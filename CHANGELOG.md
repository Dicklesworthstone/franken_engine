...
- **`bd-franken-parser-top-level-await-script-sapi0` (P1, parser team — fixed by [`a694fba6d`](https://github.com/Dicklesworthstone/franken_engine/commit/a694fba6db8b6c5b81e8214725510e65e24917b9) "feat(parser): enforce ES2020 await context rules across goals and async functions").** The first genuine drift the lockstep differential surfaced: fixture `script_await` — franken_canonical **accepted top-level `await` in a classic `script` goal** (`parse: ok`), while Node and Bun correctly rejected it (`parse: syntax_error`). Per ECMA-262, top-level `await` is only valid at the top level of a **module** (`sourceType: "module"`); in `script` it is a SyntaxError. The bead records the reproduction command + per-engine parse verdicts for posterity; with the corrected parser the default 8-fixture lockstep should return to **8/8 equivalent** once re-verified.
...
`bd-franken-parser-top-level-await-script-sapi0` (script_await parser over-permit fixed by `a694fba6d`; re-verification pending the lower_pipeline WIP build to clear).

### Representative commits

- [`365e430fd`](https://github.com/Dicklesworthstone/franken_engine/commit/365e430fd077643235a5c8c0eea352ad9b912043) — `feat(engine,cli): replace operator placeholders with sibling resolution and fail-closed env capture`
- [`5cdea2e30`](https://github.com/Dicklesworthstone/franken_engine/commit/5cdea2e30e917e5d5c49670e11f6f503c79f462a) — `fix(lints): resolve 7 clippy -D warnings failures blocking the AGENTS clippy gate`
- [`63d8b7594`](https://github.com/Dicklesworthstone/franken_engine/commit/63d8b75941cb48006b773956dc8669c00f3105f9) — `feat(lockstep,harness): add compile-only syntax verdicts to Node/Bun adapters and multi-engine harness`
- [`f876ef1c8`](https://github.com/Dicklesworthstone/franken_engine/commit/f876ef1c8b30a370cb2387af9eaafdd98a64e6e6) — `test(lockstep,harness): update multi-engine harness and lockstep integration tests for parse verdicts`
- [`9174376c0`](https://github.com/Dicklesworthstone/franken_engine/commit/9174376c051d6a588cd1ffa14a1c86476e4b93c9) — `chore(evidence): refresh all 13 OBSERVED claim receipts at current HEAD`

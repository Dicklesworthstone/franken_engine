...
/// a conservative lower quantization of `ln(num / den) * 1_000_000`, for
/// `num > 0`, `den > 0`. Integer-only (no `f64` per the determinism discipline):
...
fn ln_ratio_millionths(num: i64, den: i64) -> i64 {
    ...
    // z = (a - b) / (a + b) at high precision; z <= 1/3 because a/b < 2.
    let z = (a - b) * INTERNAL_SCALE / (a + b);
    let z_sq = z * z / INTERNAL_SCALE;
    // atanh(z) = z + z^3/3 + z^5/5 + z^7/7 + z^9/9 + z^11/11 + ...
    let mut power = z;
    let mut sum = z;
    for odd in [3u128, 5, 7, 9, 11] {
        power = power * z_sq / INTERNAL_SCALE;
        sum += power / odd;
    }
    let ln_m = 2 * sum;
    let total_internal = k * LN2_INTERNAL + ln_m;
...
mod tests {
    ...
    fn updater_legacy_json_defaults_missing_prior() {
        ...
        let mut json = serde_json::to_value(&updater).unwrap();
        json.as_object_mut()
            .expect("updater serializes as an object")
            .remove("prior");

        let restored: BayesianPosteriorUpdater = serde_json::from_value(json).unwrap();
    ...
    #[test]
    #[should_panic(expected = "ln_ratio_millionths needs positive inputs")]
    fn ln_ratio_millionths_rejects_a_zero_denominator_in_release_builds() {

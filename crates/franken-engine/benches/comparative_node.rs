...
#[cfg(target_os = "linux")]
fn run_node_with_measurement(node: &Path, path: &Path) -> std::io::Result<NodeMeasurement> {
...
fn bench_comparative_node(c: &mut Criterion) {
    ...
    for (workload, path) in WORKLOADS.iter().zip(fixtures.iter()) {
        ...
        group.bench_with_input(
            ...
            |b, path| {
                b.iter(|| {
                    let measurement = run_node_measured(&node, path);
...
fn verify_equivalent_output(frankenctl: &Path, node: &Path, workload: &Workload, path: &Path) {
    let node_measurement = run_node_measured(node, path);
    let node_stdout = stdout_trimmed(&node_measurement.output);
    assert_eq!(
    ...
    emit_lockstep_traces(
        workload.id,
        &node_measurement.output,
        node_measurement.wall_time_ns,
        node_measurement.peak_rss_bytes,
        &franken_output,
...
fn emit_lockstep_traces(
    workload_id: &str,
    node_output: &Output,
    node_wall_time_ns: u64,
    node_peak_rss_bytes: u64,
    ...
    create_benchmark_trace_from_output(
        ...
        RuntimeId::NodeJs,
        node_output,
        Duration::from_nanos(node_wall_time_ns),
        Some(node_peak_rss_bytes),
        &config,
    )
    .unwrap_or_else(|err| panic!("failed to write Node lockstep trace for {workload_id}: {err}"));
...
fn run_node_measured(node: &Path, path: &Path) -> NodeMeasurement {
    match measurement_mode() {
        #[cfg(target_os = "linux")]
        MeasurementMode::DirectLinuxWait4 => match run_node_with_measurement(node, path) {
            Ok(measurement) => measurement,
            Err(e) => panic!("Node measurement failed: {e}"),
        },
        #[cfg(not(target_os = "linux"))]
        ...
        MeasurementMode::SimpleCommand => {
            let started = Instant::now();
            let output = run_checked(node, &[path.as_os_str().to_os_string()]);
            let wall_time_ns = started.elapsed().as_nanos().min(u128::from(u64::MAX)) as u64;
            NodeMeasurement {
                wall_time_ns,

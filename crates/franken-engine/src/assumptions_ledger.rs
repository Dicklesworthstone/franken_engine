...
mod tests {
    ...
    fn test_serde_roundtrip_assumption() {
        ...
        let back: Assumption = serde_json::from_str(&json).expect("deserialize known-valid JSON");
        assert_eq!(a, back);
    }
    ...
    fn test_serde_roundtrip_ledger_error() {
        ...
        let back: LedgerError = serde_json::from_str(&json).expect("deserialize known-valid JSON");
        assert_eq!(err, back);
    }
    ...
    fn assumption_json_field_names() {
        ...
        let val: serde_json::Value =
            serde_json::to_value(&a).expect("serialization should succeed");
        let obj = val
            .as_object()
            .expect("operation should succeed for valid inputs");
    ...
    fn demotion_record_json_field_names() {
        ...
        let val: serde_json::Value =
            serde_json::to_value(&record).expect("serialization should succeed");
        let obj = val
            .as_object()
            .expect("operation should succeed for valid inputs");
    ...
    fn demotion_policy_json_field_names() {
        let policy = DemotionPolicy::default();
        let val: serde_json::Value =
            serde_json::to_value(&policy).expect("serialization should succeed");
        let obj = val
            .as_object()
            .expect("operation should succeed for valid inputs");

...
mod tests {
    ...
    fn bound_serde_roundtrip() {
        ...
        let back: RegretBound = serde_json::from_str(&json).expect("deserialize known-valid JSON");
        assert_eq!(b, back);
    }
    ...
    fn regret_entry_serde() {
        ...
        let json = serde_json::to_string(&e).expect("serialize derived Serialize");
        let back: RegretEntry = serde_json::from_str(&json).expect("deserialize known-valid JSON");
        assert_eq!(e, back);
    }

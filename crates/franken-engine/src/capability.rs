...
mod tests {
    ...
    fn capability_profile_serialization_round_trip() {
        let full = CapabilityProfile::full();
        let json = serde_json::to_string(&full).expect("serde serialization should succeed");
    ...
    fn runtime_capability_serde_all_variants() {
        ...
        for cap in &all {
            let json = serde_json::to_string(cap).expect("serde serialization should succeed");
    ...
    fn capability_denied_serialization_round_trip() {
        ...
        let json = serde_json::to_string(&denied).expect("serde serialization should succeed");
    ...
    fn capability_profile_json_contains_expected_fields() {
        let ec = CapabilityProfile::engine_core();
        let json = serde_json::to_string(&ec).expect("serde serialization should succeed");
    ...
    fn profile_serialization_deterministic() {
        ...
        let j1 = serde_json::to_string(&p1).expect("serde serialization should succeed");
        let j2 = serde_json::to_string(&p2).expect("serde serialization should succeed");
        assert_eq!(j1, j2);
    }

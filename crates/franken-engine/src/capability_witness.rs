...
mod tests {
    ...
    fn proof_obligation_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&po).expect("serde serialization should succeed");
    ...
    fn publication_pipeline_publish_emits_artifact_and_ledgers() {
        let head_signing_key =
            SigningKey::from_bytes([17u8; 32]).expect("serde serialization should succeed");
    ...
    fn publication_pipeline_rejects_missing_promotion_signature_quorum() {
        let head_signing_key =
            SigningKey::from_bytes([18u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
    ...
    fn publication_pipeline_second_publish_has_consistency_chain() {
        let head_signing_key =
            SigningKey::from_bytes([21u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
    ...
    fn publication_pipeline_revocation_appends_signed_entry() {
        let head_signing_key =
            SigningKey::from_bytes([33u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
    ...
    fn publication_pipeline_query_filters() {
        let head_signing_key =
            SigningKey::from_bytes([44u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
    ...
    fn publication_pipeline_detects_tampered_inclusion_root() {
        let head_signing_key =
            SigningKey::from_bytes([55u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
    ...
    fn denial_record_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&dr).expect("serde serialization should succeed");
    ...
    fn store_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&store).expect("serde serialization should succeed");
    ...
    fn source_capability_set_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&scs).expect("serde serialization should succeed");
    ...
    fn witness_publication_config_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&config).expect("serde serialization should succeed");
    ...
    fn witness_index_record_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&rec).expect("serde serialization should succeed");
    ...
    fn witness_index_query_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&q).expect("serde serialization should succeed");
    ...
    fn custom_theorem_extension_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&ext).expect("serde serialization should succeed");
    ...
    fn promotion_theorem_input_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&input).expect("serde serialization should succeed");
    ...
    fn witness_index_page_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&page).expect("serde serialization should succeed");
    ...
    fn witness_tree_head_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&head).expect("serde serialization should succeed");
    ...
    fn publish_draft_witness_rejected() {
        let head_key =
            SigningKey::from_bytes([60u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
    ...
    fn publish_duplicate_witness_rejected() {
        let head_key =
            SigningKey::from_bytes([61u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),
    ...
    fn revoke_empty_reason_rejected() {
        let head_key =
            SigningKey::from_bytes([62u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),
    ...
    fn revoke_unpublished_witness_rejected() {
        let head_key =
            SigningKey::from_bytes([63u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),
    ...
    fn revoke_already_revoked_rejected() {
        let head_key =
            SigningKey::from_bytes([64u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),
    ...
    fn pipeline_zero_checkpoint_interval_rejected() {
        let head_key =
            SigningKey::from_bytes([65u8; 32]).expect("serde serialization should succeed");
    ...
    fn pipeline_empty_policy_id_rejected() {
        let head_key =
            SigningKey::from_bytes([66u8; 32]).expect("serde serialization should succeed");
        let err = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig {
    ...
    fn publication_query_exclude_revoked() {
        let head_key =
            SigningKey::from_bytes([67u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),
    ...
    fn verify_publication_from_pipeline() {
        let head_key =
            SigningKey::from_bytes([68u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key.clone(),
            WitnessPublicationConfig::default(),
    ...
    fn verify_publication_not_found() {
        let head_key =
            SigningKey::from_bytes([69u8; 32]).expect("serde serialization should succeed");
    ...
    fn pipeline_events_emitted_on_publish_and_revoke() {
        let head_key =
            SigningKey::from_bytes([70u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),
    ...
    fn pipeline_checkpoints_at_interval() {
        let head_key =
            SigningKey::from_bytes([71u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig {
    ...
    fn published_witness_artifact_is_revoked_reflects_state() {
        let head_key =
            SigningKey::from_bytes([72u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),
    ...
    fn pipeline_publish_active_witness_accepted() {
        let head_key =
            SigningKey::from_bytes([73u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key.clone(),
            WitnessPublicationConfig::default(),
    ...
    fn verify_artifact_rejects_revocation_witness_id_mismatch() {
        let head_key =
            SigningKey::from_bytes([74u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key.clone(),
            WitnessPublicationConfig::default(),
    ...
    fn witness_replay_join_row_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&row).expect("serde serialization should succeed");
    ...
    fn consistency_proof_link_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&link).expect("serde serialization should succeed");
    ...
    fn pipeline_query_by_policy_id() {
        let head_signing_key =
            SigningKey::from_bytes([77u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(500),
            head_signing_key,
            WitnessPublicationConfig {
    ...
    fn pipeline_log_entries_populated_after_publish() {
        let head_key =
            SigningKey::from_bytes([90u8; 32]).expect("serde serialization should succeed");
        let mut pipeline = WitnessPublicationPipeline::new(
            SecurityEpoch::from_raw(1),
            head_key,
            WitnessPublicationConfig::default(),

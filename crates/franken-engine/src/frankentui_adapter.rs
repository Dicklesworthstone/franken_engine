...
mod tests {
    ...
    fn dashboard_severity_serde_roundtrip() {
        for s in [
            ...
            let json = serde_json::to_string(&s).expect("serialize derived Serialize");
            let restored: DashboardSeverity =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(restored, s);
        }
    }
    ...
    fn decision_outcome_kind_serde_roundtrip() {
        for k in [
            ...
            let restored: DecisionOutcomeKind =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn replay_health_status_serde_roundtrip() {
        for s in [
            ...
            let json = serde_json::to_string(&s).expect("serialize derived Serialize");
            let restored: ReplayHealthStatus =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(restored, s);
        }
    }
    ...
    fn schema_compatibility_status_serde_roundtrip() {
        for s in [
            ...
            let json = serde_json::to_string(&s).expect("serialize derived Serialize");
            let restored: SchemaCompatibilityStatus =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(restored, s);
        }
    }
    ...
    fn dashboard_alert_metric_serde_roundtrip() {
        for m in [
            ...
            let restored: DashboardAlertMetric =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn threshold_comparator_serde_roundtrip() {
        for c in [
            ...
            let restored: ThresholdComparator =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn action_candidate_view_serde_roundtrip() {
        ...
        let restored: ActionCandidateView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn driver_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&dv).expect("serialize derived Serialize");
        let restored: DriverView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn dashboard_metric_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&mv).expect("serialize derived Serialize");
        let restored: DashboardMetricView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn extension_status_row_serde_roundtrip() {
        ...
        let restored: ExtensionStatusRow =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn dashboard_refresh_policy_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&rp).expect("serialize derived Serialize");
        let restored: DashboardRefreshPolicy =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn benchmark_trend_point_serde_roundtrip() {
        ...
        let restored: BenchmarkTrendPointView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn policy_explanation_partial_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&partial).expect("serialize derived Serialize");
        let restored: PolicyExplanationPartial =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn flow_sensitivity_level_serde_roundtrip() {
        for v in [
            ...
            let restored: FlowSensitivityLevel =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn declassification_outcome_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: DeclassificationOutcome =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn confinement_status_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: ConfinementStatus =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn replacement_risk_level_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: ReplacementRiskLevel =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn proof_inventory_kind_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: ProofInventoryKind =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn proof_validity_status_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: ProofValidityStatus =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn specialization_fallback_reason_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: SpecializationFallbackReason =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn override_review_status_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: OverrideReviewStatus =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn grant_expiry_status_serde_roundtrip() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let restored: GrantExpiryStatus =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, restored);
        }
    }
    ...
    fn obligation_status_row_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&row).expect("serialize derived Serialize");
        let restored: ObligationStatusRowView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn cancellation_event_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&ev).expect("serialize derived Serialize");
        let restored: CancellationEventView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn slot_status_overview_row_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&row).expect("serialize derived Serialize");
        let restored: SlotStatusOverviewRow =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(row, restored);
    }
    ...
    fn rollback_event_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&ev).expect("serialize derived Serialize");
        let restored: RollbackEventView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(ev, restored);
    }
    ...
    fn proof_inventory_row_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&row).expect("serialize derived Serialize");
        let restored: ProofInventoryRowView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(row, restored);
    }
    ...
    fn override_rationale_view_serde_roundtrip() {
        ...
        let restored: OverrideRationaleView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn capability_delta_alert_view_serde_roundtrip() {
        ...
        let restored: CapabilityDeltaAlertView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn frankentui_view_payload_serde_roundtrip() {
        ...
        let restored: FrankentuiViewPayload =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn incident_replay_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&view).expect("serialize derived Serialize");
        let restored: IncidentReplayView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn evidence_stream_entry_view_serde_roundtrip() {
        ...
        let restored: EvidenceStreamEntryView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn dashboard_alert_rule_serde_roundtrip() {
        ...
        let restored: DashboardAlertRule =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn triggered_alert_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&alert).expect("serialize derived Serialize");
        let restored: TriggeredAlertView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(alert, restored);
    }
    ...
    fn blocked_flow_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&flow).expect("serialize derived Serialize");
        let restored: BlockedFlowView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn coverage_trend_point_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&pt).expect("serialize derived Serialize");
        let restored: CoverageTrendPoint =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn schema_compatibility_status_serde_all_variants() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let back: SchemaCompatibilityStatus =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, back);
        }
    }
    ...
    fn declassification_outcome_serde_all_variants() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let back: DeclassificationOutcome =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, back);
        }
    }
    ...
    fn proof_specialization_invalidation_reason_serde_all_variants() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let back: ProofSpecializationInvalidationReason =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, back);
        }
    }
    ...
    fn specialization_fallback_reason_serde_all_variants() {
        for v in [
            ...
            let json = serde_json::to_string(&v).expect("serialize derived Serialize");
            let back: SpecializationFallbackReason =
                serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
            assert_eq!(v, back);
        }
    }
    ...
    fn blocked_promotion_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&bpv).expect("serialize derived Serialize");
        let back: BlockedPromotionView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(bpv, back);
    }
    ...
    fn replacement_opportunity_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&rov).expect("serialize derived Serialize");
        let back: ReplacementOpportunityView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(rov, back);
    }
    ...
    fn safe_mode_activation_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&view).expect("serialize derived Serialize");
        let back: SafeModeActivationView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(view, back);
    }
    ...
    fn replay_health_panel_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&view).expect("serialize derived Serialize");
        let back: ReplayHealthPanelView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(view, back);
    }
    ...
    fn benchmark_trends_panel_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&view).expect("serialize derived Serialize");
        let back: BenchmarkTrendsPanelView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(view, back);
    }
    ...
    fn region_lifecycle_row_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&view).expect("serialize derived Serialize");
        let back: RegionLifecycleRowView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(view, back);
    }
    ...
    fn region_lifecycle_panel_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&panel).expect("serialize derived Serialize");
        let back: RegionLifecyclePanelView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
    ...
    fn decision_outcomes_panel_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: DecisionOutcomesPanelView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn obligation_status_panel_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: ObligationStatusPanelView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn schema_version_panel_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: SchemaVersionPanelView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn benchmark_trend_point_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: BenchmarkTrendPointView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn replacement_opportunity_input_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: ReplacementOpportunityInput =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn specialization_performance_impact_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: SpecializationPerformanceImpactView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn flow_decision_dashboard_view_empty_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: FlowDecisionDashboardView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn proof_specialization_lineage_dashboard_view_empty_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: ProofSpecializationLineageDashboardView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn capability_delta_dashboard_view_empty_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: CapabilityDeltaDashboardView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn replacement_progress_dashboard_view_empty_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: ReplacementProgressDashboardView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn flow_decision_alert_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: FlowDecisionAlertView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn proof_specialization_alert_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: ProofSpecializationAlertView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn capability_delta_escrow_event_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: CapabilityDeltaEscrowEventView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
    ...
    fn capability_promotion_batch_review_view_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&v).expect("serialize derived Serialize");
        let back: CapabilityPromotionBatchReviewView =
            serde_json::from_str(&json).expect("serde roundtrip should succeed in tests");
        assert_eq!(v, back);
    }
}

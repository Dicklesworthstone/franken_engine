...
mod tests {
    ...
    fn percentile_snapshot_serde_roundtrip() {
        ...
        let restored: PercentileSnapshot =
            serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn pause_budget_custom_serde_roundtrip() {
        let budget = PauseBudget::new(100, 200, 300);
        let json = serde_json::to_string(&budget).expect("serialize derived Serialize");
        let back: PauseBudget = serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn pause_record_full_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&record).expect("serialize derived Serialize");
        let back: PauseRecord = serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn tracker_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&tracker).expect("serialize derived Serialize");
        let back: PauseTracker = serde_json::from_str(&json).expect("deserialize known-valid JSON");

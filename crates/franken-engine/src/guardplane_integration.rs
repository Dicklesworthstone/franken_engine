...
mod tests {
    ...
    fn test_risk_assessment_confidence_uses_context_coverage() {
        ...
        let attributed_risk = adapter.assess_risk(&attributed_ctx).unwrap();
        // SAFETY: Test setup with valid contexts should allow assess_risk to succeed
        let sparse_risk = adapter.assess_risk(&sparse_ctx).unwrap();
    ...
    fn test_risk_assessment_confidence_penalizes_policy_violations() {
        ...
        // SAFETY: Test setup with valid contexts should allow assess_risk to succeed
        let normal_risk = adapter.assess_risk(&normal_ctx).unwrap();
        // SAFETY: Test setup with valid contexts should allow assess_risk to succeed
        let invalid_risk = adapter.assess_risk(&invalid_ctx).unwrap();

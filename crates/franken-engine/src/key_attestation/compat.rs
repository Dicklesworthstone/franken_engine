...
mod tests {
    use super::*;
    // The store and nonce registry exercised here are the shipped, hardened
    // implementations; the unhardened copies this module used to carry were
    // unreachable outside these tests.
    use crate::key_attestation::{AttestationStore, NonceRegistry};
    ...
    fn enrichment_attestation_event_type_all_variants_serde() {
        ...
        for variant in &variants {
            ...
            let restored: AttestationEventType =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");

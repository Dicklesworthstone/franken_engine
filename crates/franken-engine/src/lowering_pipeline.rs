    ...
    match statement {
        ...
        Statement::VariableDeclaration(vd) => {
            ...
            for d in &vd.declarations {
                ...
                // Discard (not Pop): a lexical/variable declaration's completion
                // is EMPTY (ECMAScript §14.3 / §14.1.1) — it must NOT record a
                // module completion value. `Pop` here emitted `Move dst:0` which
                // CLOBBERS register 0, and r0 aliases the first top-level binding
                // (e.g. a preceding `function F(){}` closure), so any later use of
                // that binding read the declaration's value instead — faulting on
                // `function F(){} let x=1; F()` with "expected function, got
                // object" (bd-jr2be). Class declarations already use `Discard`
                // here for the same reason (bd-62un6); this aligns var/let/const.
...
fn peek_lowering_value(stack: &[Reg]) -> Result<Reg, LoweringPipelineError> {
    stack
        .last()
        .copied()
        .ok_or(LoweringPipelineError::ValueStackUnderflow)
...
fn ensure_lowering_values(stack: &[Reg], needed: usize) -> Result<(), LoweringPipelineError> {
    if stack.len() < needed {
...
pub fn lower_ir2_to_ir3(
    ...
    for (op_index, op) in ir2.ops.iter().enumerate() {
        ...
        match &op.inner {
            ...
            Ir1Op::Call { arg_count } => {
                ...
                ensure_lowering_values(&value_stack, count.saturating_add(1))?;
            ...
            Ir1Op::CallMethod { arg_count } => {
                ...
                ensure_lowering_values(&value_stack, count.saturating_add(2))?;
                let mut args = Vec::with_capacity(count);
                for _ in 0..count {
                    args.push(pop_lowering_value(&mut value_stack)?);
                }
                args.reverse();
                let receiver = pop_lowering_value(&mut value_stack)?;
                let callee = pop_lowering_value(&mut value_stack)?;
            ...
            Ir1Op::Await => {
                let current = pop_lowering_value(&mut value_stack)?;
                let dst = alloc_register(&mut register_cursor);
                ir3.instructions
            ...
            Ir1Op::Yield { delegate } => {
                let value_reg = pop_lowering_value(&mut value_stack)?;
                let dst = alloc_register(&mut register_cursor);
            ...
            Ir1Op::JumpIfFalsy { label_id } => {
                let cond = pop_lowering_value(&mut value_stack)?;
            ...
            Ir1Op::NewArray { count } => {
                ...
                ensure_lowering_values(&value_stack, cnt)?;
            ...
            Ir1Op::NewObject { count } => {
                ...
                for _ in 0..cnt {
                    let val = pop_lowering_value(&mut value_stack)?;
                    let key = pop_lowering_value(&mut value_stack)?;
            ...
            Ir1Op::ArrayPush => {
                ...
                let element = pop_lowering_value(&mut value_stack)?;
            ...
            Ir1Op::SpreadIntoObject => {
                ...
                let source = pop_lowering_value(&mut value_stack)?;
                let target = pop_lowering_value(&mut value_stack)?;
                ir3.instructions
            ...
            Ir1Op::ForInNext { done_label } => {
                let iterator = peek_lowering_value(&value_stack)?;
            ...
            Ir1Op::IteratorClose { reason } => {
                let iterator = pop_lowering_value(&mut value_stack)?;
            ...
            Ir1Op::TemplateLiteral { quasi_count } => {
                ...
                for _ in 0..total {
                    part_regs.push(pop_lowering_value(&mut value_stack)?);
                }
                part_regs.reverse();
                let dst = alloc_register(&mut register_cursor);
    ...
    while deferred_idx < deferred_functions.len() {
        ...
        // (`collect_free_vars`), aligned 1:1 with `free_vars`. This replaces the
        // former fragile `binding_id >= param_count` + first-appearance-zip
        // heuristic, which misclassified body-local scoped temps (e.g. a ternary
        // result via `alloc_internal_binding`) and the function's own name —
        // mapping a temp to a free-var name and leaving the real free var on the
        // uninitialised local-register path. That broke recursion
        // (`function f(n){...f(n-1)}` -> undefined callee, bd-g0aok) and
        ...
        for body_ir1 in body_ops {
            ...
            match &ir2_op.inner {
                ...
                Ir1Op::Call { arg_count } => {
                    let count = *arg_count as usize;
                    ensure_lowering_values(&fn_value_stack, count.saturating_add(1))?;
                    let mut args = Vec::with_capacity(count);
                    for _ in 0..count {
                ...
                Ir1Op::CallMethod { arg_count } => {
                    let count = *arg_count as usize;
                    ensure_lowering_values(&fn_value_stack, count.saturating_add(2))?;
                    let mut args = Vec::with_capacity(count);
                    for _ in 0..count {
                        args.push(pop_lowering_value(&mut fn_value_stack)?);
                    }
                    args.reverse();
                    let receiver = pop_lowering_value(&mut fn_value_stack)?;
                    let callee = pop_lowering_value(&mut fn_value_stack)?;
                    let start_reg = fn_reg;
                    for arg_reg in &args {
                        let dst = alloc_register(&mut fn_reg);
                ...
                Ir1Op::NewArray { count } => {
                    let cnt = *count as usize;
                    ensure_lowering_values(&fn_value_stack, cnt)?;
                ...
                Ir1Op::NewObject { count } => {
                    ...
                    let mut properties = Vec::with_capacity(cnt);
                    for _ in 0..cnt {
                        let val = pop_lowering_value(&mut fn_value_stack)?;
                        let key = pop_lowering_value(&mut fn_value_stack)?;
                        properties.push((key, val));
                    }
                    properties.reverse();
                    let dst = alloc_register(&mut fn_reg);
                ...
                Ir1Op::ArrayPush => {
                    // Stack: [..., array, element] -> [..., array]
                    let element = pop_lowering_value(&mut fn_value_stack)?;
                ...
                Ir1Op::SpreadIntoArray => {
                    // Stack: [..., array, iterable] -> [..., array]
                    let iterable = pop_lowering_value(&mut fn_value_stack)?;
                    let array = pop_lowering_value(&mut fn_value_stack)?;
                    ir3.instructions
                        .push(Ir3Instruction::SpreadIntoArray { array, iterable });
                    fn_value_stack.push(array);
                ...
                Ir1Op::SpreadIntoObject => {
                    // Stack: [..., target, source] -> [..., target]
                    let source = pop_lowering_value(&mut fn_value_stack)?;
                    let target = pop_lowering_value(&mut fn_value_stack)?;
                    ir3.instructions
                        .push(Ir3Instruction::SpreadIntoObject { target, source });
                ...
                Ir1Op::Yield { delegate } => {
                    let value_reg = pop_lowering_value(&mut fn_value_stack)?;
                    let dst = alloc_register(&mut fn_reg);
                    ir3.instructions.push(Ir3Instruction::Yield {
                        value: value_reg,
                        delegate: *delegate,
                ...
                Ir1Op::Await => {
                    let current = pop_lowering_value(&mut fn_value_stack)?;
                    let dst = alloc_register(&mut fn_reg);
                    ir3.instructions
                        .push(Ir3Instruction::Move { dst, src: current });
                    fn_value_stack.push(dst);
...
mod tests {
    ...
    fn alien2_ir3_output_is_byte_identical_golden() {
        ...
        let cases: Vec<(&str, Ir0Module, &str, &str)> = vec![
            (
                "numeric_literal",
                script_ir0(),
                "",
            ...
            (
                ...
                "",
                "sha256:cb9f1febcc3918dc89c29c36099465b3436c9e97cbd67f6f27b9b68ea2d31397",
            ),
            (
                "const_decl",
                {
                ...
                "",
                "sha256:4c640e14cdf6103c2fd3d5834815c526df168690303716ff4d4c47434c193bfa",
            ),
        ];
        ...
        for (name, ir0, _golden_canon, _golden_hash) in cases.iter() {
            let context = LoweringContext::new("alien2-trace", "alien2-decision", "alien2-policy");
            let output =
            ...
            let again = lower_ir0_to_ir3(ir0, &context).expect("re-lowering should succeed");
            assert_eq!(
    ...
    fn lowering_pipeline_output_serde_roundtrip() {
        ...
        let ir0 = script_ir0();
        let output =
            lower_ir0_to_ir3(&ir0, &ctx).expect("operation should succeed for valid inputs");
    ...
    fn lower_conditional_expression() {
        ...
        // The current lowering emits additional cleanup Pops for branch
        // balancing alongside the test-value and statement-result Pops.
    ...
    fn oversized_ast_fails_preallocation_budget_for_bindings() {
        ...
        match result {
            ...
            // SAFETY: Test validates allocation budget exceeded error for large binding allocation
            other => panic!(
    ...
    fn statement_reordering_equivalence_independent_variable_declarations() {
        ...
        let summarize_ops = |module: &Ir1Module| {
            let mut summary = std::collections::BTreeMap::new();
            for op in &module.ops {
                let key = format!("{:?}", std::mem::discriminant(op));
        ...
        assert_eq!(
            summarize_ops(&result_original.module),
            summarize_ops(&result_permuted.module),
            "Equivalent statement orderings should preserve the same operation mix"
        );
    }
    ...
    fn statement_reordering_equivalence_mixed_independent_statements() {
        ...
        assert_eq!(
            summarize_ops(&result_original.module),
            summarize_ops(&result_permuted.module),
            "Mixed independent statements should preserve the same operation mix"
        );
    }

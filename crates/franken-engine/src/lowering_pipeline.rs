...
fn lower_expression_to_ir1(
    ...
    match expression {
        ...
        Expression::Call { callee, arguments } => {
            ...
            // Spread in call arguments (bd-hsv77): the fixed-arity Call/CallMethod
            // paths below count a `SpreadElement` as ONE positional arg (it lowers
            // to just its inner iterable), so `f(...xs)` would pass the whole array
            // as arg 0 (a=[1,2,3], b/c=undefined → NaN). When any argument is a
            // spread on a PLAIN (non-member) call, assemble the real argument list
            // into an array — mirroring the array-literal spread lowering
            // (NewArray + per-element ArrayPush / SpreadIntoArray) — and dispatch
            // through the existing `builtin:ReflectApply(target, thisArg, argsArray)`
            // with thisArg = undefined. Member-call spread (`obj.m(...xs)`) and
            // builtin spread (`Math.max(...xs)`, intercepted by the special-cases
            // above) are out of scope here and remain follow-ups.
            let has_spread_arg = arguments
                .iter()
                .any(|arg| matches!(arg, Expression::SpreadElement(_)));
            if has_spread_arg && !matches!(callee.as_ref(), Expression::Member { .. }) {
                // target = the callee function (ReflectApply slot 0)
                lower_expression_to_ir1(
                    callee,
                    ops,
                    bindings,
                ...
                // thisArg = undefined for a free call (ReflectApply slot 1)
                ops.push(Ir1Op::LoadLiteral {
                    value: Ir1Literal::Undefined,
                });
                // argsArray = [ ...expanded arguments ] (ReflectApply slot 2)

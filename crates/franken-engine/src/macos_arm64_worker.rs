...
use crate::worker_env_capture::{
    EnvCaptureError, MacOSArm64EnvCapture, WorkerEnvCapture, WorkerEnvironment,
};
...
impl MacOSArm64WorkerManager {
    ...
    pub fn mark_worker_idle(&mut self, worker_id: &str) -> Result<(), MacOSWorkerError> {
        let worker =
            self.workers
                .get_mut(worker_id)
                .ok_or_else(|| MacOSWorkerError::WorkerNotFound {
    ...
    pub fn export_worker_env(
        ...
        let json = serde_json::to_string_pretty(&worker.environment).map_err(|e| {
            MacOSWorkerError::Serialization {
                details: e.to_string(),
            }
        })?;

        std::fs::write(output_path, json).map_err(|e| MacOSWorkerError::IoError {
    ...
    pub fn execute_on_worker(
        ...
        // Prepare command
        if command.is_empty() {
            return Err(MacOSWorkerError::InvalidCommand {
    ...
    fn initialize_worker_workspace(&self, worker_id: &str) -> Result<(), MacOSWorkerError> {
        ...
        std::fs::create_dir_all(&workspace_dir).map_err(|e| MacOSWorkerError::IoError {
    ...
    fn cleanup_worker_workspace(&self, worker_id: &str) -> Result<(), MacOSWorkerError> {
        ...
        if workspace_dir.exists() {
            std::fs::remove_dir_all(&workspace_dir).map_err(|e| MacOSWorkerError::IoError {
...
mod tests {
    ...
    fn test_worker_serialization() {
        ...
        let deserialized: MacOSArm64Worker =
    ...
    fn test_create_worker_on_macos_arm64() {
        ...
        let worker_id = manager
            .create_worker()
            .expect("should create worker on macOS arm64");

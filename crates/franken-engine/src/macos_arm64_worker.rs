//! macOS ARM64 worker implementation for RCH worker pools.
//!
//! Provides worker pool management and execution capabilities for macOS ARM64
//! platforms in the RCH (Remote Compilation Host) system.

#![forbid(unsafe_code)]

use crate::worker_env_capture::{MacOSArm64EnvCapture, WorkerEnvCapture, WorkerEnvironment, EnvCaptureError};
...
/// macOS ARM64 worker instance in the RCH pool.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct MacOSArm64Worker {
...
/// Configuration for creating macOS ARM64 workers.
#[derive(Debug, Clone)]
pub struct MacOSArm64WorkerConfig {
    /// Worker capabilities.
    pub capabilities: WorkerCapabilities,
    /// Worker resource limits.
    pub resource_limits: WorkerResourceLimits,
...
impl Default for MacOSArm64WorkerConfig {
    fn default() -> Self {
        Self {
...
/// macOS ARM64 worker factory and manager.
#[derive(Debug)]
pub struct MacOSArm64WorkerManager {
    /// Configuration for creating workers.
    config: MacOSArm64WorkerConfig,
    /// Active workers in the pool.
    workers: BTreeMap<String, MacOSArm64Worker>,
    /// Environment capture utility.
    env_capture: MacOSArm64EnvCapture,
}

impl MacOSArm64WorkerManager {
    /// Create a new worker manager with the given configuration.
    pub fn new(config: MacOSArm64WorkerConfig) -> Self {
        Self {
            config,
            workers: BTreeMap::new(),
            env_capture: MacOSArm64EnvCapture::new(),
        }
    }

    /// Create a new worker manager with default configuration.
    pub fn with_defaults() -> Self {
        Self::new(MacOSArm64WorkerConfig::default())
    }

    /// Create a new worker and add it to the pool.
    pub fn create_worker(&mut self) -> Result<String, MacOSWorkerError> {
        // Generate unique worker ID
        let worker_num = NEXT_WORKER_ID.fetch_add(1, Ordering::SeqCst);
        let worker_id = format!("macos-arm64-{}", worker_num);

        // Capture environment information
        let environment = self.env_capture.capture_environment()
            .map_err(MacOSWorkerError::EnvironmentCapture)?;

        // Verify platform compatibility
        if environment.os != "macos" || environment.arch != "arm64" {
            return Err(MacOSWorkerError::PlatformMismatch {
                expected: "macos-arm64".to_string(),
    ...
    pub fn get_worker(&self, worker_id: &str) -> Option<&MacOSArm64Worker> {
        self.workers.get(worker_id)
    }

    /// Get mutable reference to a specific worker.
    pub fn get_worker_mut(&mut self, worker_id: &str) -> Option<&mut MacOSArm64Worker> {
        self.workers.get_mut(worker_id)
    }

    /// List all workers in the pool.
    pub fn list_workers(&self) -> Vec<&MacOSArm64Worker> {
        self.workers.values().collect()
    }

    /// Mark a worker as idle and available for work.
    pub fn mark_worker_idle(&mut self, worker_id: &str) -> Result<(), MacOSWorkerError> {
        let worker = self.workers.get_mut(worker_id)
            .ok_or_else(|| MacOSWorkerError::WorkerNotFound { worker_id: worker_id.to_string() })?;
    ...
    pub fn mark_worker_busy(&mut self, worker_id: &str) -> Result<(), MacOSWorkerError> {
        let worker = self.workers.get_mut(worker_id)
            .ok_or_else(|| MacOSWorkerError::WorkerNotFound { worker_id: worker_id.to_string() })?;
    ...
    pub fn remove_worker(&mut self, worker_id: &str) -> Result<MacOSArm64Worker, MacOSWorkerError> {
    ...
    pub fn export_worker_env(&self, worker_id: &str, output_path: &Path) -> Result<(), MacOSWorkerError> {
        let worker = self.workers.get(worker_id)
            .ok_or_else(|| MacOSWorkerError::WorkerNotFound { worker_id: worker_id.to_string() })?;

        let json = serde_json::to_string_pretty(&worker.environment)
            .map_err(|e| MacOSWorkerError::Serialization { details: e.to_string() })?;

        std::fs::write(output_path, json)
            .map_err(|e| MacOSWorkerError::IoError {
    ...
    pub fn execute_on_worker(
    ...
    ) -> Result<WorkerExecutionResult, MacOSWorkerError> {
        let worker = self.workers.get_mut(worker_id)
            .ok_or_else(|| MacOSWorkerError::WorkerNotFound { worker_id: worker_id.to_string() })?;

        if !worker.status.is_available() {
            return Err(MacOSWorkerError::WorkerNotAvailable {
        ...
        // Prepare command
        if command.is_empty() {
            return Err(MacOSWorkerError::InvalidCommand { details: "Empty command".to_string() });
        }
        ...
        let mut child = cmd.spawn()
            .map_err(|e| MacOSWorkerError::CommandExecution {
        ...
        let output = if let Some(timeout_secs) = timeout_seconds {
            match child.wait_timeout(Duration::from_secs(timeout_secs)) {
                ...
                Ok(None) => {
                    ...
                    worker.status = WorkerStatus::Idle;
                    return Err(MacOSWorkerError::CommandTimeout {
                ...
                Err(e) => {
                    worker.status = WorkerStatus::Idle;
                    return Err(MacOSWorkerError::CommandExecution {
                        command: command.join(" "),
                        error: e.to_string(),
                    });
                }
    ...
    fn initialize_worker_workspace(&self, worker_id: &str) -> Result<(), MacOSWorkerError> {
    ...
    fn cleanup_worker_workspace(&self, worker_id: &str) -> Result<(), MacOSWorkerError> {
        let workspace_dir = self.config.work_dir.join(worker_id);
...
/// Result of executing a command on a worker.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Errors that can occur in macOS ARM64 worker operations.
#[derive(Debug, thiserror::Error)]
pub enum MacOSWorkerError {
...
mod tests {
    ...
    fn create_test_config() -> MacOSArm64WorkerConfig {
        let temp_dir = TempDir::new().expect("create temp dir");
        MacOSArm64WorkerConfig {
            capabilities: WorkerCapabilities::default(),
            resource_limits: WorkerResourceLimits::default(),
    ...
    fn test_worker_capabilities_default() {
        ...
        assert!(caps.supported_targets.contains(&"aarch64-apple-darwin".to_string()));
    ...
    fn test_worker_manager_creation() {
        ...
        let manager = MacOSArm64WorkerManager::new(config);
    ...
    fn test_worker_serialization() {
        ...
        let deserialized: MacOSArm64Worker = serde_json::from_str(&json).expect("deserialization should work");
    ...
    fn test_create_worker_on_macos_arm64() {
        let config = create_test_config();
        let mut manager = MacOSArm64WorkerManager::new(config);

        let worker_id = manager.create_worker().expect("should create worker on macOS arm64");
        ...
        assert_eq!(worker.environment.os, "macos");
        assert_eq!(worker.environment.arch, "arm64");
    }
    ...
    fn test_create_worker_wrong_platform() {
        ...
        match result.unwrap_err() {
            MacOSWorkerError::EnvironmentCapture(EnvCaptureError::UnsupportedPlatform { .. }) => {
                // Expected error
            }

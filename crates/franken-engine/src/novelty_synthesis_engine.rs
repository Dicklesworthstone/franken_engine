...
mod tests {
    ...
    fn test_denial_reason_serde_roundtrip() {
        for r in SynthesisDenialReason::ALL {
            ...
            let back: SynthesisDenialReason =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");
            assert_eq!(*r, back);
        }
    }
    ...
    fn test_filter_all_accepted() {
        ...
        // The candidates must be semantically DISTINCT: `make_candidate_with_novelty`
        // produces comment-only source, which canonicalizes to an empty program, so two
        // such candidates collapse to the same SemanticFingerprint and the second is
        // (correctly) denied as a DuplicateCandidate. Give them distinct statement
        // structure (1 vs 2 statements — survives comment/whitespace/identifier/literal

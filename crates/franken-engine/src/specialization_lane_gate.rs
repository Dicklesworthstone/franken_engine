...
mod tests {
    ...
    fn gate_passes_with_all_criteria_met() {
        ...
        let result = evaluate_gate(&input).expect("operation should succeed for valid inputs");
    ...
    fn gate_evidence_hash_deterministic() {
        ...
        let r1 = evaluate_gate(&input).expect("operation should succeed for valid inputs");
        let r2 = evaluate_gate(&input).expect("operation should succeed for valid inputs");
    ...
    fn gate_different_runs_different_hashes() {
        ...
        let r1 = evaluate_gate(&input1).expect("operation should succeed for valid inputs");
        let r2 = evaluate_gate(&input2).expect("operation should succeed for valid inputs");
    ...
    fn serde_gate_outcome_roundtrip() {
        ...
        let json = serde_json::to_string(&val).expect("serialize derived Serialize");
        let back: GateOutcome = serde_json::from_str(&json).expect("deserialize known-valid JSON");
        assert_eq!(val, back);
    }
    ...
    fn serde_blocker_roundtrip() {
        ...
        let json = serde_json::to_string(&blocker).expect("serialize derived Serialize");

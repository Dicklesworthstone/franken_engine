...
mod tests {
    ...
    fn tee_platform_serde_round_trip() {
        for platform in TeePlatform::ALL {
            let json = serde_json::to_string(&platform).expect("serialize derived Serialize");
            let parsed: TeePlatform =
    ...
    fn measurement_algorithm_serde_round_trip() {
        for alg in [
            ...
            let parsed: MeasurementAlgorithm =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn revocation_source_type_serde_round_trip() {
        for st in [
            ...
            let parsed: RevocationSourceType =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");
            assert_eq!(st, parsed);
        }
    }
    ...
    fn trust_root_pinning_serde_round_trip() {
        for pinning in [
            ...
            let json = serde_json::to_string(&pinning).expect("serialize derived Serialize");
            let parsed: TrustRootPinning =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn revocation_probe_status_serde_round_trip() {
        for status in [
            ...
            let parsed: RevocationProbeStatus =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn policy_governance_event_serde_round_trip() {
        ...
        let parsed: PolicyGovernanceEvent =
            serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn trust_root_source_serde_all_variants() {
        ...
        for src in variants {
            let json = serde_json::to_string(&src).expect("serialize derived Serialize");
            let parsed: TrustRootSource =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn emitter_serde_roundtrip() {
        ...
        let json = serde_json::to_string(&emitter).expect("serialize derived Serialize");
        let parsed: DecisionReceiptEmitter =
            serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn freshness_window_serde_round_trip() {
        ...
        let back: AttestationFreshnessWindow =
            serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn temporary_trust_root_override_serde_round_trip() {
        ...
        let json = serde_json::to_string(&override_req).expect("serialize derived Serialize");
        let back: TemporaryTrustRootOverride =
            serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn emitter_serde_no_synced_epoch() {
        ...
        let json = serde_json::to_string(&emitter).expect("serialize derived Serialize");
        let back: DecisionReceiptEmitter =
            serde_json::from_str(&json).expect("deserialize known-valid JSON");
    ...
    fn revocation_probe_status_serde_roundtrip_all() {
        for s in [
            ...
            let back: RevocationProbeStatus =
                serde_json::from_str(&json).expect("deserialize known-valid JSON");
            assert_eq!(back, s);
        }
    }
    ...
    /// bd-7tpwa restoration of expired_quote_rejected(). The original test
    /// was deleted (its body was `unimplemented!()` after the policy types
    /// were restructured), leaving `MockTeeProvider::generate_expired_quote`
    /// with no unit-test coverage even though it produces the input to the
    /// security-critical freshness-rejection path. These drive the helper
    /// through the real policy `evaluate_quote` and assert
    /// `AttestationStale` rather than silent acceptance.
    fn mock_aligned_policy(provider: &MockTeeProvider) -> TeeAttestationPolicy {
        let mut approved = BTreeMap::new();
        approved.insert(
            provider.platform,
            vec![provider.approved_measurement.clone()],
        );
        TeeAttestationPolicy {
            schema_version: 1,
            ...
            // No revocation sources — the freshness gate fires before any
            // revocation observation is consulted, so leaving this empty
            // keeps the test scoped to the bd-7tpwa concern.
            revocation_sources: Vec::new(),
            platform_trust_roots: vec![PlatformTrustRoot {
                root_id: provider.trust_root_id.clone(),
                platform: provider.platform,
                trust_anchor_pem: "-----BEGIN CERT-----MOCK".to_string(),
                valid_from_epoch: SecurityEpoch::from_raw(0),
                valid_until_epoch: None,
                pinning: TrustRootPinning::Pinned,
                source: TrustRootSource::Policy,
    ...
    #[test]
    fn expired_quote_rejected_via_policy_evaluate() {
        for platform in TeePlatform::ALL {
            let provider = MockTeeProvider::new(platform);
            let nonce = "expired-quote-bd-7tpwa";
            let expired_mock = provider.generate_expired_quote(nonce);
            assert!(
                provider.verify_quote(&expired_mock),
                "generate_expired_quote must still produce a structurally-valid \
                 mock quote on platform {platform:?} — the stale flag lives in \
                 the *timestamp*, not in a bad signature",
            );

            // 3600s blows past the 300s standard freshness window built into
            // mock_aligned_policy.
            let policy_quote = provider.to_policy_quote(&expired_mock, 3600);
            assert_eq!(policy_quote.quote_age_secs, 3600);
            assert_eq!(policy_quote.platform, platform);

            let policy = mock_aligned_policy(&provider);
            let err = policy
                .evaluate_quote(
                    &policy_quote,
                    DecisionImpact::Standard,
                ...
                .expect_err(
                    "expired quote must be rejected by TeeAttestationPolicy::evaluate_quote",
                );
            assert!(
                matches!(
                    &err,
                ...
                "expired quote must surface as AttestationStale {{age=3600, max=300}} \
                 on platform {platform:?}; got {err:?}",
            );
        }
    }

    #[test]
    fn expired_quote_rejected_with_high_impact_window() {
        // bd-7tpwa: the 60-second high-impact window must also reject quotes
        // generated by `generate_expired_quote` (timestamp ~3600s in the past).
        let provider = MockTeeProvider::new(TeePlatform::AmdSev);
        let expired_mock = provider.generate_expired_quote("high-impact-bd-7tpwa");
        let policy_quote = provider.to_policy_quote(&expired_mock, 3600);
        let policy = mock_aligned_policy(&provider);
        let err = policy
            .evaluate_quote(
            ...
            .expect_err("expired quote must be rejected under HighImpact freshness");
        assert!(
            matches!(
                &err,
                TeeAttestationPolicyError::AttestationStale {
            ...
            "high-impact freshness must use the tighter 60s window; got {err:?}",
        );
    }

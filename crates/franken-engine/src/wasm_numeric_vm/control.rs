...
impl Validator<'_> {
    fn pop(&mut self) -> Result<Option<WasmValueType>, WasmNumericVmError> {
        let frame = self
            .frames
            .last()
            .ok_or_else(|| invalid("instruction after function end"))?;
        if self.values.len() == frame.height {
            if frame.unreachable {
                return Ok(None);
            }
            return Err(invalid(format!(
                "function {} operand underflow at control boundary",
                self.function
            )));
        }
        self.values
            .pop()
            .ok_or_else(|| invalid("operand stack underflow"))
    }
    ...
    fn push(&mut self, ty: Option<WasmValueType>) -> Result<(), WasmNumericVmError> {
        if self.values.len() >= self.vm.limits.max_stack_values {
            return Err(WasmNumericVmError::StackLimitExceeded {
                max: self.vm.limits.max_stack_values,
            });
        }
        self.values.push(ty);
        Ok(())
    ...
    fn label(&self, depth: u32) -> Result<(BlockType, Kind), WasmNumericVmError> {
        let index = self
            ...
            .ok_or_else(|| {
                invalid(format!(
                    "function {} invalid branch depth {depth}",
                    self.function
                ))
            })?;
    ...
    fn boundary(&mut self, signature: BlockType) -> Result<(), WasmNumericVmError> {
        ...
        if self.values.len() != height {
            return Err(invalid(format!(
                "function {} has excess operands at control boundary",
                self.function
            )));
        }
        Ok(())
...
pub(super) fn validate(
    vm: &WasmNumericVm,
    function: u32,
) -> Result<ControlMap, WasmNumericVmError> {
    ...
    let mut validator = Validator {
        ...
        frames: vec![ValidationFrame {
            kind: Kind::Function,
            signature: BlockType::Type(body.type_index),
            height: 0,
            unreachable: false,
            start: 0,
            body: 0,
        ...
        match opcode {
            ...
            0x02..=0x04 => {
                if validator.frames.len() > vm.limits.max_control_depth {
                    return Err(WasmNumericVmError::ControlDepthExceeded {
                        max: vm.limits.max_control_depth,
                    });
                }
            ...
            0x05 => {
                let frame = validator
                    .frames
                    .last()
                    .ok_or_else(|| invalid("else outside function"))?;
            ...
            0x0b => {
                let frame = validator
                    .frames
                    .last()
                    .ok_or_else(|| invalid("unmatched end"))?;
                let signature = frame.signature;
                if frame.kind == Kind::If
                    && frame.alternate.is_none()
                ...
                if frame.kind == Kind::Function {
                    if !reader.finished() {
                        return Err(invalid("bytes after function end"));
                    }
                    return Ok(map);
                }
                map.blocks.insert(
                    ...
                    Block {
                        kind: frame.kind,
                        params: signature.params(vm)?.len(),
                        results: signature.results(vm)?.len(),
                        body: frame.body,
            ...
            0x0f => {
                validator.pop_types(&signature.results)?;
                validator.unreachable();
            }
            0x10..=0x13 => {
                ...
                if matches!(opcode, 0x12 | 0x13) {
                    ...
                    if callee.results != signature.results {
                        return Err(invalid(
                            "tail call result types must match the enclosing function",
                        ));
                    }
                    validator.unreachable();
                } else {
            ...
            0x20..=0x22 => {
                ...
                if opcode != 0x21 {
                    validator.push(Some(ty))?;
                }
            }
            0x23..=0x40 | 0xfc => {
                let effect = vm
                    .state
                    .validate_instruction(opcode, &mut reader, function)?;
                for ty in effect.pop.into_iter().flatten() {
                    validator.expect(ty)?;
                }
                if let Some(ty) = effect.push {
                    validator.push(Some(ty))?;
                }
            }
            0x41 => {
                reader.read_i32_leb(function)?;
                validator.push(Some(WasmValueType::I32))?;
            }
            0x42 => {
                reader.read_i64_leb(function)?;
                validator.push(Some(WasmValueType::I64))?;
            }
            0x43 => {
                reader.read_u32_le(function)?;
                validator.push(Some(WasmValueType::F32))?;
            }
            0x44 => {
                reader.read_u64_le(function)?;
                validator.push(Some(WasmValueType::F64))?;
            }
            _ => {
                let (input, count, output) =
                    numeric_signature(opcode).ok_or(WasmNumericVmError::UnsupportedOpcode {
                        function_index: function,
                        opcode,
                        offset,
                    })?;
                for _ in 0..count {
                    validator.expect(input)?;
                }
    ...
    Err(invalid(format!(
        "function {function} has an unterminated control frame"
    )))
}
...
fn branch(
    ...
    let target = frames
        ...
        .checked_sub(depth as usize)
        .and_then(|index| index.checked_sub(1))
        .ok_or_else(|| invalid("invalid runtime branch depth"))?;
    let frame = &frames[target];
    let retained = stack
        .len()
        .checked_sub(frame.labels)
        .ok_or_else(|| invalid("missing branch arguments"))?;
    if retained < frame.height {
        return Err(invalid("branch consumed an enclosing operand"));
    }
...
pub(super) fn execute(
    ...
    match opcode {
        0x00 => {
            return Err(WasmNumericVmError::Unreachable {
                function_index: function,
            });
        }
        0x01 => {}
        0x02..=0x04 => {
            let block = map
                .blocks
                .get(&(reader.offset() - 1))
                .ok_or_else(|| invalid("missing validated block"))?;
            ...
            let height = stack
                .len()
                .checked_sub(block.params)
                .ok_or_else(|| invalid("missing block parameters"))?;
            frames.push(Frame {
                kind: block.kind,
                height,
                body: block.body,
                end: block.end,
                labels: if block.kind == Kind::Loop {
                    block.params
                } else {
                    block.results
                },
            ...
            reader.offset = if condition {
                block.body
            } else {
                block.alternate.unwrap_or(block.end)
            };
        }
        0x05 | 0x0b => {
            let frame = frames
                .pop()
                .ok_or_else(|| invalid("missing runtime control frame"))?;
            finish(stack, &frame)?;
            if frame.kind == Kind::Function {
                return Ok(true);
            }
...
pub(super) fn execute_numeric(
    ...
    match opcode {
        ...
        0x8b..=0x90 | 0x96..=0x9e | 0xa4..=0xa6 => execute_float_scalar(opcode, stack, function),
        ...
        0xac | 0xad => {
            ...
            stack.push(WasmBoundaryValue::I64(if opcode == 0xac {
                i64::from(value)
            } else {
        ...
        _ => Err(WasmNumericVmError::UnsupportedOpcode {
            function_index: function,
            opcode,
            offset: 0,
        }),
...
fn execute_conversion(
    ...
    let result = match opcode {
        ...
        0xb6 => F32Bits(canonical_f32_bits(
            f64::from_bits(expect_f64_bits(value, function, 0)?) as f32,
        )),
        0xb7 | 0xb8 => {
            let value = expect_i32(value, function, 0)?;
            F64Bits(
        ...
        0xbb => F64Bits(canonical_f64_bits(f64::from(f32::from_bits(
            expect_f32_bits(value, function, 0)?,
        )))),
        _ => {
            return Err(WasmNumericVmError::UnsupportedOpcode {
                function_index: function,
...
fn trunc_integer(
    ...
    if !number.is_finite() || truncated < minimum || truncated >= exclusive_maximum {
        return Err(WasmNumericVmError::IntegerOverflow {
            function_index: function,
        });
    }
...
mod tests {
    ...
    pub(super) fn module(
        ...
        for (params, results) in
            std::iter::once((params, results)).chain(extra_types.iter().copied())
        {
            types.push(0x60);
        ...
        let mut body = if local_count == 0 {
            vec![0]
        } else {
            vec![1, local_count, 0x7f]
        };
        body.extend(code);
    ...
    fn f32_rounding_is_ties_even_and_preserves_zero_sign() {
        for (value, expected) in [
            ...
            for (opcode, expected) in (0x8d..=0x90).zip(expected) {
                ...
                assert_eq!(
                    run(
                        &numeric_module(opcode, &[0x7d], 0x7d),
                        &[WasmBoundaryValue::F32Bits(value.to_bits())]
                    ),
                    [WasmBoundaryValue::F32Bits(expected.to_bits())]
                );
            }
        }
        for opcode in 0x8d..=0x90 {
            ...
            assert_eq!(
                run(
                    &numeric_module(opcode, &[0x7d], 0x7d),
                    &[WasmBoundaryValue::F32Bits(0xff80_0001)]
                ),
                [WasmBoundaryValue::F32Bits(CANONICAL_F32_NAN)]
            );
        }
    }
    ...
    fn f64_rounding_is_ties_even_and_preserves_zero_sign() {
        for (value, expected) in [
            ...
            for (opcode, expected) in (0x9b..=0x9e).zip(expected) {
                ...
                assert_eq!(
                    run(
                        &numeric_module(opcode, &[0x7c], 0x7c),
                        &[WasmBoundaryValue::F64Bits(value.to_bits())]
                    ),
                    [WasmBoundaryValue::F64Bits(expected.to_bits())]
                );
            }
        }
        for opcode in 0x9b..=0x9e {
            ...
            assert_eq!(
                run(
                    &numeric_module(opcode, &[0x7c], 0x7c),
                    &[WasmBoundaryValue::F64Bits(0xfff0_0000_0000_0001)]
                ),
                [WasmBoundaryValue::F64Bits(CANONICAL_F64_NAN)]
            );
        }
    }
    ...
    fn min_max_order_signed_zeros_and_propagate_either_nan() {
        for (op32, op64, minimum) in [(0x96, 0xa4, true), (0x97, 0xa5, false)] {
            for (left, right) in [
                ...
                assert_eq!(
                    run(
                        &numeric_module(op32, &[0x7d, 0x7d], 0x7d),
                        &[
                            WasmBoundaryValue::F32Bits((left as f32).to_bits()),
                            WasmBoundaryValue::F32Bits((right as f32).to_bits())
                        ]
                    ),
                    [WasmBoundaryValue::F32Bits((expected as f32).to_bits())]
                );
                assert_eq!(
                    run(
                        &numeric_module(op64, &[0x7c, 0x7c], 0x7c),
                        &[
                            WasmBoundaryValue::F64Bits(left.to_bits()),
                            WasmBoundaryValue::F64Bits(right.to_bits())
                        ]
                    ),
                    [WasmBoundaryValue::F64Bits(expected.to_bits())]
                );
            ...
            for (left, right) in [
                (0x7f80_0001, 0),
                (0, 0xffc1_2345),
                (0x7fc0_0000, 0xffc1_2345),
            ] {
                assert_eq!(
                    run(
                        &numeric_module(op32, &[0x7d, 0x7d], 0x7d),
                        &[
                            WasmBoundaryValue::F32Bits(left),
                            WasmBoundaryValue::F32Bits(right)
                        ]
                    ),
                    [WasmBoundaryValue::F32Bits(CANONICAL_F32_NAN)]
                );
            ...
            for (left, right) in [
                (0x7ff0_0000_0000_0001, 0),
                (0, 0xfff8_1234_5678_9abc),
                (0x7ff8_0000_0000_0000, 0xfff8_1234_5678_9abc),
            ] {
                assert_eq!(
                    run(
                        &numeric_module(op64, &[0x7c, 0x7c], 0x7c),
                        &[
                            WasmBoundaryValue::F64Bits(left),
                            WasmBoundaryValue::F64Bits(right)
                        ]
                    ),
                    [WasmBoundaryValue::F64Bits(CANONICAL_F64_NAN)]
                );
    ...
    fn float_sign_instructions_do_not_canonicalize_nan_payloads() {
        for bits in [0_u32, 0x8000_0000, 0x7f80_0001, 0xffc1_2345] {
            assert_eq!(
                run(
                    &numeric_module(0x8b, &[0x7d], 0x7d),
                    &[WasmBoundaryValue::F32Bits(bits)]
                ),
                [WasmBoundaryValue::F32Bits(bits & 0x7fff_ffff)]
            );
            assert_eq!(
                run(
                    &numeric_module(0x8c, &[0x7d], 0x7d),
                    &[WasmBoundaryValue::F32Bits(bits)]
                ),
                [WasmBoundaryValue::F32Bits(bits ^ 0x8000_0000)]
            );
            for sign in [0_u32, 0x8000_0000, 0xff80_0001] {
                assert_eq!(
                    run(
                        &numeric_module(0x98, &[0x7d, 0x7d], 0x7d),
                        &[
                            WasmBoundaryValue::F32Bits(bits),
                            WasmBoundaryValue::F32Bits(sign)
                        ]
                    ),
                    [WasmBoundaryValue::F32Bits(
                        (bits & 0x7fff_ffff) | (sign & 0x8000_0000)
                    )]
                );
            }
        }
        for bits in [
            ...
            assert_eq!(
                run(
                    &numeric_module(0x99, &[0x7c], 0x7c),
                    &[WasmBoundaryValue::F64Bits(bits)]
                ),
                [WasmBoundaryValue::F64Bits(bits & 0x7fff_ffff_ffff_ffff)]
            );
            assert_eq!(
                run(
                    &numeric_module(0x9a, &[0x7c], 0x7c),
                    &[WasmBoundaryValue::F64Bits(bits)]
                ),
                [WasmBoundaryValue::F64Bits(bits ^ 0x8000_0000_0000_0000)]
            );
            for sign in [0_u64, 0x8000_0000_0000_0000, 0xfff0_0000_0000_0001] {
                assert_eq!(
                    run(
                        &numeric_module(0xa6, &[0x7c, 0x7c], 0x7c),
                        &[
                            WasmBoundaryValue::F64Bits(bits),
                            WasmBoundaryValue::F64Bits(sign)
                        ]
                    ),
                    [WasmBoundaryValue::F64Bits(
                        (bits & 0x7fff_ffff_ffff_ffff) | (sign & 0x8000_0000_0000_0000)
                    )]
                );
            }
        }
    ...
    fn integer_to_float_conversions_respect_signedness() {
        ...
        for (opcode, input_type, output_type, input, expected) in [
            ...
            assert_eq!(
                run(
                    &numeric_module(opcode, &[input_type], output_type),
                    &[input]
                ),
                [expected]
            );
    ...
    fn integer_to_float_rounds_once_at_precision_boundaries() {
        ...
        for (opcode, input_type, output_type, input, expected) in [
            ...
            (
                ...
                0x7e,
                0x7d,
                I64(((1_u64 << 63) + (1_u64 << 39) + 1) as i64),
                F32Bits(0x5f00_0001),
            ),
            (
                0xb9,
    ...
    fn demotion_and_promotion_handle_underflow_overflow_and_nan() {
        for (value, expected) in [
            ...
            assert_eq!(
                run(
                    &numeric_module(0xb6, &[0x7c], 0x7d),
                    &[WasmBoundaryValue::F64Bits(value.to_bits())]
                ),
                [WasmBoundaryValue::F32Bits(expected)]
            );
        }
        for (bits, expected) in [
            ...
            assert_eq!(
                run(
                    &numeric_module(0xbb, &[0x7d], 0x7c),
                    &[WasmBoundaryValue::F32Bits(bits)]
                ),
                [WasmBoundaryValue::F64Bits(expected)]
            );
        }
    }
    ...
    fn truncation_to_i32_checks_truncated_not_original_value() {
        for (opcode, input_type) in [(0xa8, 0x7d), (0xa9, 0x7d), (0xaa, 0x7c), (0xab, 0x7c)] {
            ...
            for (value, expected) in [(-0.0_f64, 0), (-0.75, 0), (0.75, 0), (42.75, 42)] {
                let value = if input_type == 0x7d {
                    WasmBoundaryValue::F32Bits((value as f32).to_bits())
                } else {
                    WasmBoundaryValue::F64Bits(value.to_bits())
                };
                assert_eq!(
                    vm.call_export("f", &[value]).unwrap().results,
                    [WasmBoundaryValue::I32(expected)]
                );
            }
        }
        for (opcode, value, expected) in [
            ...
            assert_eq!(
                run(
                    &numeric_module(opcode, &[0x7c], 0x7f),
                    &[WasmBoundaryValue::F64Bits(value.to_bits())]
                ),
                [WasmBoundaryValue::I32(expected)]
            );
        ...
        for (opcode, bits, expected) in [
            ...
            assert_eq!(
                run(
                    &numeric_module(opcode, &[0x7d], 0x7f),
                    &[WasmBoundaryValue::F32Bits(bits)]
                ),
                [WasmBoundaryValue::I32(expected)]
            );
        ...
        for (opcode, bits) in [
            ...
            .unwrap();
            assert!(matches!(
                vm.call_export("f", &[WasmBoundaryValue::F32Bits(bits)]),
                Err(WasmNumericVmError::IntegerOverflow { .. })
            ));
        }
        for (opcode, value) in [
            (0xaa, -2_147_483_649.0_f64),
            (0xaa, 2_147_483_648.0),
            (0xab, -1.0),
            (0xab, 4_294_967_296.0),
        ] {
            let vm = WasmNumericVm::parse(
                &numeric_module(opcode, &[0x7c], 0x7f),
                WasmNumericLimits::default(),
            ...
            .unwrap();
            assert!(matches!(
                vm.call_export("f", &[WasmBoundaryValue::F64Bits(value.to_bits())]),
                Err(WasmNumericVmError::IntegerOverflow { .. })
            ));
        }
    }
    ...
    fn truncation_to_i64_uses_exclusive_exact_power_of_two_bounds() {
        for (opcode, input_type) in [(0xae, 0x7d), (0xaf, 0x7d), (0xb0, 0x7c), (0xb1, 0x7c)] {
            let vm = WasmNumericVm::parse(
                &numeric_module(opcode, &[input_type], 0x7e),
                WasmNumericLimits::default(),
            )
            .unwrap();
            ...
            let at_upper = if input_type == 0x7d {
                WasmBoundaryValue::F32Bits((upper as f32).to_bits())
            } else {
                WasmBoundaryValue::F64Bits(upper.to_bits())
            };
            assert!(matches!(
                vm.call_export("f", &[at_upper]),
                Err(WasmNumericVmError::IntegerOverflow { .. })
            ));
            let (below_upper, expected) = match (input_type, unsigned) {
                (0x7d, false) => (
                    WasmBoundaryValue::F32Bits(0x5eff_ffff),
                    i64::MAX - (1_i64 << 39) + 1,
                ),
                (0x7d, true) => (WasmBoundaryValue::F32Bits(0x5f7f_ffff), -(1_i64 << 40)),
                (0x7c, false) => (
                    WasmBoundaryValue::F64Bits(0x43df_ffff_ffff_ffff),
                    i64::MAX - 1023,
                ),
            ...
            assert_eq!(
                vm.call_export("f", &[below_upper]).unwrap().results,
                [WasmBoundaryValue::I64(expected)]
            );
            ...
            let lower = if input_type == 0x7d {
                WasmBoundaryValue::F32Bits((lower as f32).to_bits())
            } else {
                WasmBoundaryValue::F64Bits(lower.to_bits())
            };
            assert_eq!(
                vm.call_export("f", &[lower]).unwrap().results,
                [WasmBoundaryValue::I64(if unsigned { 0 } else { i64::MIN })]
            );
    ...
    fn all_trapping_conversions_reject_nan_and_infinity_without_saturation() {
        for (opcode, input_type, output_type) in [
            ...
            for number in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
                let input = if input_type == 0x7d {
                    WasmBoundaryValue::F32Bits((number as f32).to_bits())
                } else {
                    WasmBoundaryValue::F64Bits(number.to_bits())
                };
                assert!(matches!(
                    vm.call_export("f", &[input]),
                    Err(WasmNumericVmError::IntegerOverflow { .. })
                ));
            }
            let input = if input_type == 0x7d {
                WasmBoundaryValue::F32Bits(42.75_f32.to_bits())
            } else {
                WasmBoundaryValue::F64Bits(42.75_f64.to_bits())
            };
            assert_eq!(
                vm.call_export("f", &[input]).unwrap().results,
                [if output_type == 0x7f {
                    WasmBoundaryValue::I32(42)
                } else {
                    WasmBoundaryValue::I64(42)
                }]
            );
        }
    }
    ...
    fn numeric_validation_covers_all_unprefixed_scalar_opcodes() {
        for opcode in 0x45..=0xc4 {
            let (input, count, output) =
                numeric_signature(opcode).expect("unprefixed scalar opcode");
            let encode = |ty| match ty {
            ...
            let vm = WasmNumericVm::parse(&bytes, WasmNumericLimits::default()).unwrap();
            let one = match input {
                WasmValueType::I32 => WasmBoundaryValue::I32(1),
                WasmValueType::I64 => WasmBoundaryValue::I64(1),
                WasmValueType::F32 => WasmBoundaryValue::F32Bits(1.0_f32.to_bits()),
                WasmValueType::F64 => WasmBoundaryValue::F64Bits(1.0_f64.to_bits()),
            };
        ...
        for code in [
            vec![0x00, 0x41, 1, 0x90, 0x0b],
            vec![0x00, 0x42, 1, 0xaa, 0x0b],
            vec![0x41, 1, 0xbb, 0x0b],
        ] {
            assert!(
                WasmNumericVm::parse(
                    &module(&[], &[0x7f], 0, &code, &[]),
    ...
    fn conversion_is_metered_before_it_can_trap() {
        ...
        assert_eq!(
            vm.call_export("f", &[WasmBoundaryValue::F64Bits(f64::NAN.to_bits())]),
        ...
        let vm = WasmNumericVm::parse(&bytes, limits).unwrap();
        let result = vm
            .call_export("f", &[WasmBoundaryValue::F64Bits(42.75_f64.to_bits())])
            .unwrap();
    ...
    fn i32_bit_counts_cover_zero_sign_bit_and_all_ones() {
        for (value, expected) in [
            ...
            for (opcode, expected) in (0x67..=0x69).zip(expected) {
                assert_eq!(
                    run(
                        &numeric_module(opcode, &[0x7f], 0x7f),
    ...
    fn i64_bit_counts_return_i64_not_i32() {
        for (value, expected) in [
            ...
            for (opcode, expected) in (0x79..=0x7b).zip(expected) {
                assert_eq!(
                    run(
                        &numeric_module(opcode, &[0x7e], 0x7e),
    ...
    fn i32_bitwise_shift_and_rotate_preserve_width_and_mask_counts() {
        ...
        for (opcode, expected) in (0x71..=0x78).zip(expected) {
            ...
            assert_eq!(
                run(
                    &bytes,
                    &[WasmBoundaryValue::I32(left), WasmBoundaryValue::I32(1)]
                ),
                [WasmBoundaryValue::I32(expected as i32)]
            );
        }
        for count in [0, 32, 64, i32::MIN] {
            for opcode in 0x74..=0x78 {
                ...
                assert_eq!(
                    run(
                        &bytes,
                        &[WasmBoundaryValue::I32(left), WasmBoundaryValue::I32(count)]
                    ),
                    [WasmBoundaryValue::I32(left)]
                );
            }
        }
        for (opcode, expected) in
            (0x74..=0x78).zip([0x8000_0000_u32, 0xffff_ffff, 1, 0xc000_0001, 7])
        {
            let bytes = numeric_module(opcode, &[0x7f, 0x7f], 0x7f);
            assert_eq!(
                run(
    ...
    fn i64_bitwise_shift_and_rotate_preserve_high_bits() {
        ...
        for (opcode, expected) in (0x83..=0x8a).zip(expected) {
            ...
            assert_eq!(
                run(
                    &bytes,
                    &[WasmBoundaryValue::I64(left), WasmBoundaryValue::I64(1)]
                ),
                [WasmBoundaryValue::I64(expected as i64)]
            );
        }
        for count in [0, 64, 128, i64::MIN, 1_i64 << 32] {
            for opcode in 0x86..=0x8a {
                ...
                assert_eq!(
                    run(
                        &bytes,
                        &[WasmBoundaryValue::I64(left), WasmBoundaryValue::I64(count)]
                    ),
                    [WasmBoundaryValue::I64(left)]
                );
            }
        }
        for (opcode, expected) in (0x86..=0x8a).zip([
    ...
    fn narrow_sign_extensions_discard_high_bits() {
        for (opcode, value, expected) in [
            (0xc0, 0x1234_ff80, -128),
            (0xc0, 0x1234_ff7f, 127),
            (0xc1, 0x1234_8000, -32768),
            (0xc1, 0x1234_7fff, 32767),
        ] {
            assert_eq!(
                run(
                    &numeric_module(opcode, &[0x7f], 0x7f),
        ...
        for (opcode, value, expected) in [
            (0xc2, 0x1234_5678_ffff_ff80, -128),
            (0xc2, 0x1234_5678_ffff_ff7f, 127),
            (0xc3, 0x1234_5678_ffff_8000, -32768),
            (0xc3, 0x1234_5678_ffff_7fff, 32767),
            (0xc4, 0x1234_5678_8000_0000, -2_147_483_648),
            (0xc4, 0x1234_5678_7fff_ffff, 2_147_483_647),
        ] {
            assert_eq!(
                run(
                    &numeric_module(opcode, &[0x7e], 0x7e),
    ...
    fn reinterpretation_keeps_exact_nan_payloads_and_signed_zero() {
        for bits in [
            ...
            assert_eq!(
                run(
                    &numeric_module(0xbc, &[0x7d], 0x7f),
                    &[WasmBoundaryValue::F32Bits(bits)]
                ),
                [WasmBoundaryValue::I32(bits as i32)]
            );
            assert_eq!(
                run(
                    &numeric_module(0xbe, &[0x7f], 0x7d),
                    &[WasmBoundaryValue::I32(bits as i32)]
                ),
                [WasmBoundaryValue::F32Bits(bits)]
            );
        ...
        for bits in [
            ...
            assert_eq!(
                run(
                    &numeric_module(0xbd, &[0x7c], 0x7e),
                    &[WasmBoundaryValue::F64Bits(bits)]
                ),
                [WasmBoundaryValue::I64(bits as i64)]
            );
            assert_eq!(
                run(
                    &numeric_module(0xbf, &[0x7e], 0x7c),
                    &[WasmBoundaryValue::I64(bits as i64)]
                ),
                [WasmBoundaryValue::F64Bits(bits)]
            );
    ...
    fn xorshift_workload_executes_and_replays_under_the_instruction_meter() {
        let bytes = module(
            ...
            0,
            &[
                0x20, 0, 0x20, 0, 0x41, 13, 0x74, 0x73, 0x21, 0, 0x20, 0, 0x20, 0, 0x41, 17, 0x76,
                0x73, 0x21, 0, 0x20, 0, 0x20, 0, 0x41, 5, 0x74, 0x73, 0x0b,
            ],
            &[],
        );
        let limits = WasmNumericLimits {
        ...
        let vm = WasmNumericVm::parse(&bytes, limits).unwrap();
        assert_eq!(
            vm.call_export("f", &[WasmBoundaryValue::I32(1)]),
    ...
    fn factorial_executes_nested_loop_and_conditional_exit() {
        let bytes = module(
            ...
            1,
            &[
                0x41, 1, 0x21, 1, 0x02, 0x40, 0x03, 0x40, 0x20, 0, 0x45, 0x0d, 1, 0x20, 1, 0x20, 0,
                0x6c, 0x21, 1, 0x20, 0, 0x41, 1, 0x6b, 0x21, 0, 0x0c, 0, 0x0b, 0x0b, 0x20, 1, 0x0b,
            ],
            &[],
        );
        for (argument, expected) in [(0, 1), (1, 1), (5, 120), (10, 3_628_800)] {
            assert_eq!(
                run(&bytes, &[WasmBoundaryValue::I32(argument)]),
                [WasmBoundaryValue::I32(expected)]
            );
        }
        let vm = WasmNumericVm::parse(&bytes, WasmNumericLimits::default()).unwrap();
        ...
        assert_eq!(
            a,
            vm.call_export("f", &[WasmBoundaryValue::I32(5)]).unwrap()
        );
    }
    ...
    fn if_else_selects_exactly_one_result_arm() {
        ...
        for (condition, expected) in [(0, 22), (1, 11), (-1, 11)] {
            assert_eq!(
                run(&bytes, &[WasmBoundaryValue::I32(condition)]),
                [WasmBoundaryValue::I32(expected)]
            );
        }
    }
    ...
    fn branch_table_handles_index_and_unsigned_default() {
        let bytes = module(
            ...
            0,
            &[
                0x02, 0x7f, 0x02, 0x7f, 0x41, 42, 0x20, 0, 0x0e, 1, 0, 1, 0x0b, 0x41, 1, 0x6a,
                0x0b, 0x0b,
            ],
            &[],
        );
        for (index, expected) in [(0, 43), (1, 42), (-1, 42)] {
            assert_eq!(
                run(&bytes, &[WasmBoundaryValue::I32(index)]),
                [WasmBoundaryValue::I32(expected)]
            );
        }
    }
    ...
    fn indexed_block_types_preserve_multiple_values() {
        let bytes = module(
            ...
            &[0x7f, 0x7e],
            0,
            &[0x41, 7, 0x42, 9, 0x02, 1, 0x0c, 0, 0x0b, 0x0b],
            &[(&[0x7f, 0x7e], &[0x7f, 0x7e])],
        );
        assert_eq!(
            run(&bytes, &[]),
            [WasmBoundaryValue::I32(7), WasmBoundaryValue::I64(9)]
        );
    }
    ...
    fn loop_branches_carry_parameters_not_results() {
        let bytes = module(
            ...
            1,
            &[
                0x41, 3, 0x03, 1, 0x41, 1, 0x6b, 0x22, 0, 0x20, 0, 0x0d, 0, 0x1a, 0x42, 9, 0x0b,
                0x0b,
            ],
            &[(&[0x7f], &[0x7e])],
        );
    ...
    fn if_without_else_preserves_block_parameters() {
        let bytes = module(
            ...
            &[0x7f],
            0,
            &[0x41, 7, 0x20, 0, 0x04, 1, 0x41, 1, 0x6a, 0x0b, 0x0b],
            &[(&[0x7f], &[0x7f])],
        );
        assert_eq!(
            run(&bytes, &[WasmBoundaryValue::I32(0)]),
            [WasmBoundaryValue::I32(7)]
        );
        assert_eq!(
            run(&bytes, &[WasmBoundaryValue::I32(1)]),
            [WasmBoundaryValue::I32(8)]
        );
    }
    ...
    fn branch_table_rejects_mismatched_label_types() {
        let bytes = module(
            ...
            0,
            &[
                0x02, 0x7f, 0x02, 0x7e, 0x42, 1, 0x41, 0, 0x0e, 1, 0, 1, 0x0b, 0x1a, 0x41, 1, 0x0b,
                0x0b,
            ],
            &[],
        );
    ...
    fn loop_instruction_and_control_depth_limits_fail_closed() {
        ...
        assert!(matches!(
            ...
            Err(WasmNumericVmError::ControlDepthExceeded { max: 0 })
        ));
    }
    ...
    fn signed_full_width_and_padded_block_immediates_decode() {
        assert_eq!(
            read_i32_leb(&[0x80, 0x80, 0x80, 0x80, 0x78]).unwrap(),
            (i32::MIN, 5)
        );
        assert_eq!(
            read_i64_leb(&[0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x7f]).unwrap(),
            (i64::MIN, 10)
        );
        let bytes = module(
            &[],
            &[],
            0,
            &[0x02, 0xc0, 0xff, 0xff, 0xff, 0x7f, 0x0b, 0x0b],
            &[],
        );

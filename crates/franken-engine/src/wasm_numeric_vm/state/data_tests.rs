...
fn module(
    ...
    if let Some(start) = start { section(&mut bytes, 8, &leb(start as usize)); }
    if let Some(count) = count { section(&mut bytes, 12, &leb(count as usize)); }
    section(&mut bytes, 10, &bodies);
    if let Some(segments) = segments { section(&mut bytes, 11, segments); }
    bytes
}
...
fn drop_data(index: u8) -> Vec<u8> { vec![0xfc, 9, index, 0x0b] }
...
fn program(contents: &[u8]) -> Vec<u8> {
    module(
        ...
        Some(&data(&[(None, contents)])), Some(1), Some(1), None,
    )
}
...
fn reorder(bytes: &[u8], order: &[u8]) -> Vec<u8> {
    ...
    for id in order { result.extend_from_slice(sections[id]); }
    result
}
...
fn passive_payloads_do_not_initialize_memory_and_need_no_memory_declaration() {
    ...
    for count in [None, Some(1)] {
        let vm = parse(&module(&[(&[], &[0x0b])], Some(&segments), count, None, None));
        assert!(vm.instantiate().unwrap().memory_export("m").is_none());
        let vm = parse(&module(&[(&[], &[0x0b])], Some(&segments), count, Some(0), None));
        assert!(vm.instantiate().unwrap().memory_export("m").unwrap().is_empty());
    }
...
fn data_drop_is_idempotent_and_instance_local_with_shared_immutable_payloads() {
    ...
    assert!(Arc::ptr_eq(a.state.data[0].as_ref().unwrap(), b.state.data[0].as_ref().unwrap()));
    a.call_export("f1", &[]).unwrap();
    a.call_export("f1", &[]).unwrap();
    assert!(a.state.data[0].is_none());
    for values in [[0, 0, 1], [0, 1, 0], [0, -1, 0]] {
        assert!(matches!(a.call_export("f0", &args(values)), Err(WasmNumericVmError::State(WasmStateError::DataSourceOutOfBounds { data_bytes: 0, .. }))));
    }
    a.call_export("f0", &args([65_536, 0, 0])).unwrap();
    b.call_export("f0", &args([0, 0, 3])).unwrap();
    assert_eq!(&b.memory_export("m").unwrap()[..3], b"abc");
    vm.instantiate().unwrap().call_export("f0", &args([0, 0, 3])).unwrap();
}

#[test]
fn active_segments_keep_indices_but_are_dropped_before_export_calls() {
    let bytes = module(
        &[(&[0x7f; 3], &init(0)), (&[], &drop_data(0)), (&[0x7f; 3], &init(1))],
        Some(&data(&[(Some(0), b"active"), (None, b"passive")])), Some(2), Some(1), None,
    );
    let vm = parse(&bytes);
    let mut instance = vm.instantiate().unwrap();
    assert_eq!(&instance.memory_export("m").unwrap()[..6], b"active");
    assert!(matches!(instance.call_export("f0", &args([0, 0, 1])), Err(WasmNumericVmError::State(WasmStateError::DataSourceOutOfBounds { data_index: 0, data_bytes: 0, .. }))));
    instance.call_export("f1", &[]).unwrap();
    instance.call_export("f0", &args([6, 0, 0])).unwrap();
    instance.call_export("f2", &args([6, 0, 7])).unwrap();
    assert_eq!(&instance.memory_export("m").unwrap()[..13], b"activepassive");
}

#[test]
fn entire_unsigned_source_and_destination_ranges_are_checked_before_mutation() {
    let vm = parse(&program(b"abcdef"));
    for values in [[65_535, 0, 2], [0, 5, 2], [-1, 0, 1], [0, -1, 1], [0, 0, -1], [65_537, 0, 0], [0, 7, 0]] {
        let mut instance = vm.instantiate().unwrap();
        let before = instance.memory_export("m").unwrap().to_vec();
        assert!(matches!(instance.call_export("f0", &args(values)), Err(WasmNumericVmError::State(WasmStateError::MemoryOutOfBounds { .. } | WasmStateError::DataSourceOutOfBounds { .. }))));
...
fn work_budget_refusal_is_atomic_but_later_refusal_preserves_completed_copy() {
    ...
    for (budget, length) in [(4, 1), (5, 65)] {
        ...
        let mut instance = vm.instantiate().unwrap();
        assert!(matches!(instance.call_export("f0", &args([0, 0, length])), Err(WasmNumericVmError::InstructionBudgetExceeded { .. })));
        assert!(instance.memory_export("m").unwrap().iter().all(|byte| *byte == 0));
        assert!(instance.state.data[0].is_some());
    }
    let vm = WasmNumericVm::parse(&bytes, WasmNumericLimits { max_instructions: 5, ..WasmNumericLimits::default() }).unwrap();
    let mut instance = vm.instantiate().unwrap();
    assert!(matches!(instance.call_export("f0", &args([0, 0, 1])), Err(WasmNumericVmError::InstructionBudgetExceeded { .. })));
    assert_eq!(instance.memory_export("m").unwrap()[0], 0x5a);
    let vm = parse(&bytes);
    assert_eq!(vm.call_export("f0", &args([0, 0, 65])).unwrap().instructions_executed, 7);
}
...
fn data_drop_obeys_the_opcode_budget_and_survives_a_later_trap() {
    ...
    for (budget, dropped) in [(0, false), (1, true)] {
        let vm = WasmNumericVm::parse(&bytes, WasmNumericLimits { max_instructions: budget, ..WasmNumericLimits::default() }).unwrap();
        let mut instance = vm.instantiate().unwrap();
        assert!(matches!(instance.call_export("f1", &[]), Err(WasmNumericVmError::InstructionBudgetExceeded { .. })));
        assert_eq!(instance.state.data[0].is_none(), dropped);
    }
    let bytes = module(&[(&[], &[0xfc, 9, 0, 0x00, 0x0b])], Some(&data(&[(None, b"abc")])), Some(1), None, None);
    let vm = parse(&bytes);
    let mut instance = vm.instantiate().unwrap();
    assert!(matches!(instance.call_export("f0", &[]), Err(WasmNumericVmError::Unreachable { .. })));
...
fn start_can_initialize_and_drop_passive_data_before_instance_publication() {
    ...
    let bytes = module(&[(&[0x7f; 3], &init(0)), (&[], &start)], Some(&data(&[(None, b"abc")])), Some(1), Some(1), Some(1));
    let vm = parse(&bytes);
    for _ in 0..2 {
        let mut instance = vm.instantiate().unwrap();
        assert_eq!(&instance.memory_export("m").unwrap()[4..7], b"abc");
        assert_eq!(instance.start_execution().unwrap().instructions_executed, 7);
        assert!(matches!(instance.call_export("f0", &args([0, 0, 1])), Err(WasmNumericVmError::State(WasmStateError::DataSourceOutOfBounds { data_bytes: 0, .. }))));
    }
    // The same start body cannot re-read an active segment: it is already gone.
    let bytes = module(&[(&[], &start)], Some(&data(&[(Some(0), b"abc")])), Some(1), Some(1), Some(0));
    assert!(matches!(parse(&bytes).instantiate(), Err(WasmNumericVmError::State(WasmStateError::DataSourceOutOfBounds { data_bytes: 0, .. }))));
}

#[test]
fn direct_calls_share_passive_data_and_completed_copies_survive_traps() {
    let caller = [0x20, 0, 0x20, 1, 0x20, 2, 0x10, 0, 0x00, 0x0b];
    let bytes = module(&[(&[0x7f; 3], &init(0)), (&[0x7f; 3], &caller)], Some(&data(&[(None, b"abc")])), Some(1), Some(1), None);
    let vm = parse(&bytes);
    let mut instance = vm.instantiate().unwrap();
    assert!(matches!(instance.call_export("f1", &args([0, 0, 3])), Err(WasmNumericVmError::Unreachable { .. })));
...
fn data_drop_requires_no_memory_and_does_not_pop_an_operand() {
    let bytes = module(&[(&[], &[0x41, 7, 0xfc, 9, 0, 0x1a, 0x0b])], Some(&data(&[(None, b"abc")])), Some(1), None, None);
    let vm = parse(&bytes);
    let mut instance = vm.instantiate().unwrap();
...
fn data_count_must_match_even_without_a_data_section() {
    for count in [0, 2] {
        let bytes = module(&[(&[], &[0x0b])], Some(&data(&[(None, b"abc")])), Some(count), None, None);
        assert!(matches!(WasmNumericVm::parse(&bytes, WasmNumericLimits::default()), Err(WasmNumericVmError::InvalidModule { detail }) if detail.contains("data count section")));
    }
    let bytes = module(&[(&[], &[0x0b])], None, Some(1), None, None);
    assert!(WasmNumericVm::parse(&bytes, WasmNumericLimits::default()).is_err());
    parse(&module(&[(&[], &[0x0b])], None, Some(0), None, None)).instantiate().unwrap();
}
...
fn data_count_ordering_preserves_duplicate_and_out_of_order_rejection() {
    ...
    parse(&bytes);
    for order in [
        vec![1, 3, 7, 10, 12, 11], vec![1, 3, 7, 12, 11, 10],
        vec![1, 3, 7, 12, 12, 10, 11], vec![1, 3, 7, 12, 10, 11, 11],
    ] {
        assert!(WasmNumericVm::parse(&reorder(&bytes, &order), WasmNumericLimits::default()).is_err(), "{order:?}");
    }
...
fn data_indices_count_and_memory_are_validated_in_unreachable_code_too() {
    for code in [vec![0x00, 0xfc, 8, 0, 0, 0x0b], vec![0x00, 0xfc, 9, 0, 0x0b]] {
        let bytes = module(&[(&[], &code)], Some(&data(&[(None, b"a")])), None, Some(1), None);
        assert!(WasmNumericVm::parse(&bytes, WasmNumericLimits::default()).is_err());
        // Polymorphic unreachable operands are valid when the indices are real.
        parse(&module(&[(&[], &code)], Some(&data(&[(None, b"a")])), Some(1), Some(1), None));
    }
    for code in [vec![0x00, 0xfc, 8, 1, 0, 0x0b], vec![0x00, 0xfc, 9, 1, 0x0b]] {
        let bytes = module(&[(&[], &code)], Some(&data(&[(None, b"a")])), Some(1), Some(1), None);
        assert!(matches!(WasmNumericVm::parse(&bytes, WasmNumericLimits::default()), Err(WasmNumericVmError::State(WasmStateError::UnknownDataSegment { data_index: 1 }))));
    }
    let bytes = module(&[(&[], &[0x00, 0xfc, 8, 0, 0, 0x0b])], Some(&data(&[(None, b"a")])), Some(1), None, None);
    assert!(WasmNumericVm::parse(&bytes, WasmNumericLimits::default()).is_err());
...
fn padded_subopcodes_data_indices_and_memory_indices_are_decoded_completely() {
    let code = [0x20, 0, 0x20, 1, 0x20, 2, 0xfc, 0x88, 0, 0x80, 0, 0x80, 0, 0x0b];
    let drop = [0xfc, 0x89, 0, 0x80, 0, 0x0b];
    let bytes = module(&[(&[0x7f; 3], &code), (&[], &drop)], Some(&[1, 0x81, 0, 1, 42]), Some(1), Some(1), None);
    let vm = parse(&bytes);
    let mut instance = vm.instantiate().unwrap();
...
fn segment_counts_and_payload_boundaries_remain_bounded() {
    let bytes = module(&[(&[], &[0x0b])], Some(&data(&[(None, b"a")])), Some(1), None, None);
    let limits = WasmNumericLimits { max_state_entries: 1, ..WasmNumericLimits::default() };
    WasmNumericVm::parse(&bytes, limits).unwrap().instantiate().unwrap();
    ...
    for segments in [vec![1, 1, 4, b'a'], vec![1, 3, 0], vec![1, 2, 1, 0x41, 0, 0x0b, 0]] {
    ...
    let bytes = module(&[(&[], &[0x0b])], Some(&[0xff, 0xff, 0xff, 0xff, 0x0f]), None, None, None);
    let limits = WasmNumericLimits { max_state_entries: usize::MAX, ..WasmNumericLimits::default() };
    assert!(matches!(WasmNumericVm::parse(&bytes, limits), Err(WasmNumericVmError::InvalidModule { detail }) if detail == "truncated data segment vector"));
    let vm = parse(&module(&[(&[0x7f; 3], &init(0))], Some(&data(&[(None, b"")])), Some(1), Some(0), None));
    vm.instantiate().unwrap().call_export("f0", &args([0, 0, 0])).unwrap();
}

        ...
        for padded in [false, true] {
            ...
            for &(number, i32_s, i32_u, i64_s, i64_u) in &rows {
                ...
                assert_eq!(
                    result.results,
                    [expected],
                    "subopcode {subopcode}, {number}, padded={padded}"
                );
...
fn adjacent_f64_integer_boundaries_do_not_saturate_early() {
    let rows = [
        ...
        (
            6,
            f64::from_bits(9_223_372_036_854_775_808.0_f64.to_bits() - 1),
            WasmBoundaryValue::I64(9_223_372_036_854_774_784),
        ),
        (
            6,
            f64::from_bits((-9_223_372_036_854_775_808.0_f64).to_bits() - 1),
        ...
        (
            7,
            f64::from_bits(18_446_744_073_709_551_616.0_f64.to_bits() - 1),
            WasmBoundaryValue::I64(-2048),
        ),
    ];
    for (subopcode, value, expected) in rows {
        let vm = WasmNumericVm::parse(
            &conversion_module(subopcode, false),
            WasmNumericLimits::default(),
        )
        .unwrap();
        assert_eq!(
            vm.call_export("f", &[input(subopcode, value)])
                .unwrap()
                .results,
            [expected]
        );
...
fn nan_sign_and_payload_do_not_change_the_zero_result() {
    for subopcode in 0..8 {
        ...
        let values = if subopcode & 2 == 0 {
            ...
            .into_iter()
            .map(WasmBoundaryValue::F64Bits)
            .collect::<Vec<_>>()
        };
        let zero = if subopcode < 4 {
            WasmBoundaryValue::I32(0)
        } else {
            WasmBoundaryValue::I64(0)
        };
        for value in values {
            assert_eq!(
...
fn saturating_conversion_keeps_the_existing_instruction_budget() {
    for subopcode in 0..8 {
        ...
        assert!(matches!(
            vm.call_export("f", &[input(subopcode, f64::INFINITY)]),
...
fn memory_copy_is_memmove_in_both_overlap_directions() {
    ...
    let mut instance = vm.instantiate().unwrap();
    instance
        .call_export("f", &arguments([65_532, 0, 4]))
        .unwrap();
...
fn memory_fill_uses_only_the_low_byte_and_preserves_neighbors() {
    ...
    for (value, byte) in [(0x1ff, 255), (-1, 255), (0x100, 0), (0x142, 0x42)] {
        let mut instance = vm.instantiate().unwrap();
        instance
            .call_export("f", &arguments([3, value, 5]))
            .unwrap();
...
fn bulk_memory_bounds_traps_do_not_partially_write() {
    for (instruction, cases) in [
        ...
        (
            &[PREFIX, 11, 0][..],
            vec![[65_535, 42, 2], [-1, 42, 1], [0, 42, -1], [65_537, 42, 0]],
        ),
    ] {
        ...
        for args in cases {
            ...
            assert!(matches!(
                instance.call_export("f", &arguments(args)),
...
fn zero_length_bulk_operations_allow_exactly_one_past_the_end() {
    for instruction in [&[PREFIX, 10, 0, 0][..], &[PREFIX, 11, 0][..]] {
        ...
        let mut instance = vm.instantiate().unwrap();
        instance
            .call_export("f", &arguments([65_536, 65_536, 0]))
            .unwrap();
        assert_eq!(
            &instance.memory_export("m").unwrap()[..16],
            b"0123456789abcdef"
        );
    }
}
...
fn bulk_work_is_precharged_and_budget_refusal_is_atomic() {
    for instruction in [&[PREFIX, 10, 0, 0][..], &[PREFIX, 11, 0][..]] {
        ...
        let before = instance.memory_export("m").unwrap().to_vec();
        assert!(matches!(
            instance.call_export("f", &arguments([2, 1, 1])),
...
fn bulk_failure_retains_earlier_stores_but_never_leaks_between_instances() {
    ...
    let vm = WasmNumericVm::parse(
        &bulk_module(&[PREFIX, 10, 0, 0], &prefix),
        WasmNumericLimits::default(),
    )
    .unwrap();
    ...
    assert_eq!(
        &first.memory_export("m").unwrap()[1..16],
        b"123456789abcdef"
    );
}
...
fn bulk_memory_immediates_and_all_operand_types_are_validated() {
    ...
    for instruction in [
        ...
        code.extend([0x0b, 0x0b]);
        assert!(
            WasmNumericVm::parse(&module(&[], &[], &code, true), WasmNumericLimits::default())
                .is_err()
        );
    }
}

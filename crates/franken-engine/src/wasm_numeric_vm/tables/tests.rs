...
fn fixture(operation: &[u8], params: &[u8], results: &[u8], elements: &[u8]) -> Vec<u8> {
    ...
    section(
        ...
        7,
        &[
            5, 1, b'f', 0, 3, 1, b'a', 0, 4, 1, b'b', 0, 5, 1, b't', 1, 0, 1, b'u', 1, 1,
        ],
    );
...
fn active_elements() -> Vec<u8> {
    vec![1, 0, 0x41, 0, 0x0b, 3, 0, 1, 2]
}
...
fn cross_table_copy_changes_real_indirect_dispatch_and_preserves_nulls() {
    ...
    assert_eq!(
        instance.table_export("u").unwrap(),
        &[None, Some(0), Some(1), Some(2), None, None]
    );
    for (slot, expected) in [(1, 10), (2, 20), (3, 30)] {
        assert_eq!(
            instance
                .call_export("b", &[WasmBoundaryValue::I32(slot)])
                .unwrap()
                .results,
    ...
    assert!(matches!(
        instance.call_export("b", &[WasmBoundaryValue::I32(4)]),
...
fn table_copy_checks_entire_unsigned_ranges_before_any_mutation() {
    for (to, from) in [(0, 0), (1, 0), (0, 1)] {
        ...
        for values in [
            ...
            assert!(matches!(
                instance.call_export("f", &args(values)),
                Err(WasmNumericVmError::State(
                    WasmStateError::TableElementOutOfBounds { .. }
                ))
            ));
...
fn table_copy_precharges_reference_work_and_refuses_atomically() {
    ...
    let mut instance = vm.instantiate().unwrap();
    assert_eq!(
        instance.call_export("f", &args([0, 0, 3])),
    ...
    for length in 0..=6 {
        ...
        assert_eq!(
            vm.call_export("f", &args([0, 0, length]))
                .unwrap()
...
fn completed_table_copy_survives_later_trap_without_cross_instance_leaks() {
    ...
    assert!(matches!(
        first.call_export("f", &args([0, 0, 3])),
        Err(WasmNumericVmError::Unreachable { .. })
    ));
    assert_eq!(
        first.table_export("u").unwrap(),
...
fn table_bulk_immediates_and_types_are_validated_even_in_dead_code() {
    for instruction in [
        ...
        vec![0xfc, 16, 2],
        vec![0xfc, 14, 0, 0x80],
        vec![0xfc, 16, 0x80, 0x80, 0x80, 0x80, 0x10],
    ] {
    ...
    for wrong in 0..3 {
        let mut code = vec![0x00];
        for operand in 0..3 {
            code.extend([if operand == wrong { 0x42 } else { 0x41 }, 0]);
        }
...
fn padded_table_copy_encodings_replay_the_same_results_and_work() {
    ...
    let padded = vm(
        &[
            0x20, 0, 0x20, 1, 0x20, 2, 0xfc, 0x8e, 0, 0x81, 0, 0x80, 0, 0x0b,
        ],
        WasmNumericLimits::default(),
    );
    ...
    assert_eq!(
        a.call_export("f", &args([1, 0, 3])).unwrap(),
        b.call_export("f", &args([1, 0, 3])).unwrap()
    );
...
fn elements(mode: u8, functions: &[u8]) -> Vec<u8> {
    ...
    if mode & 3 != 0 {
        bytes.push(if mode & 4 == 0 { 0 } else { 0x70 });
    }
    bytes.extend(leb(functions.len()));
    for function in functions {
        if mode & 4 == 0 {
            bytes.push(*function);
        } else if *function == 255 {
            bytes.extend([0xd0, 0x70, 0x0b]);
        } else {
            bytes.extend([0xd2, *function, 0x0b]);
        }
    }
    bytes
}
...
fn passive_index_and_expression_elements_initialize_real_dispatch() {
    for (mode, functions, expected) in [
        ...
        assert_eq!(
            instance
                .call_export("b", &[WasmBoundaryValue::I32(1)])
                .unwrap()
                .results,
            [WasmBoundaryValue::I32(10)]
        );
        assert_eq!(
            instance
                .call_export("b", &[WasmBoundaryValue::I32(3)])
                .unwrap()
                .results,
            [WasmBoundaryValue::I32(30)]
        );
        if mode == 5 {
            assert!(matches!(
                instance.call_export("b", &[WasmBoundaryValue::I32(2)]),
                Err(WasmNumericVmError::State(
                    WasmStateError::UninitializedTableElement { .. }
                ))
            ));
...
fn element_drop_is_idempotent_instance_local_and_keeps_copied_references() {
    ...
    first.call_export("f", &args([0, 0, 0])).unwrap();
    assert!(matches!(
        first.call_export("f", &args([1, 0, 1])),
    ...
    assert_eq!(
        first
            .call_export("b", &[WasmBoundaryValue::I32(2)])
            .unwrap()
            .results,
        [WasmBoundaryValue::I32(20)]
    );
...
fn all_active_and_declarative_encodings_are_dropped_before_guest_execution() {
    for mode in [0, 2, 3, 4, 6, 7] {
        ...
        assert!(matches!(
            instance.call_export("f", &args([0, 0, 1])),
            Err(WasmNumericVmError::State(WasmStateError::LimitExceeded {
                actual: 1,
                max: 0,
                ..
...
fn table_init_checks_source_and_destination_extents_before_writes() {
    ...
    for values in [
        ...
        instance.call_export("f", &args([0, 0, 3])).unwrap();
        assert_eq!(
            &instance.table_export("u").unwrap()[..3],
    ...
    assert!(
        matches!(instance.call_export("f", &args([0, 2, 2])), Err(WasmNumericVmError::State(WasmStateError::LimitExceeded { actual: 4, max: 3, resource })) if resource == "element segment 0 source range")
    );
}
...
fn completed_element_drop_survives_later_traps_but_not_across_instances() {
    ...
    let mut second = vm.instantiate().unwrap();
    assert!(matches!(
        first.call_export("f", &args([0, 0, 0])),
        Err(WasmNumericVmError::Unreachable { .. })
    ));
...
fn table_init_binary_indices_and_padded_immediates_are_not_reversed() {
    ...
    for code in [
        init_code(1, 0),
        vec![
            0x20, 0, 0x20, 1, 0x20, 2, 0xfc, 0x8c, 0, 0x81, 0, 0x80, 0, 0x0b,
        ],
    ] {
        ...
        assert_eq!(
            instance
                .call_export("a", &[WasmBoundaryValue::I32(0)])
                .unwrap()
                .results,
            [WasmBoundaryValue::I32(30)]
        );
    ...
    for instruction in [
        ...
        vec![0xfc, 12, 0, 2],
        vec![0xfc, 13, 2],
        vec![0xfc, 13, 0x80, 0x80, 0x80, 0x80, 0x10],
    ] {
        let mut code = vec![0x00];
        code.extend(instruction);
        code.push(0x0b);
    ...
    for wrong in 0..3 {
        let mut code = vec![0x00];
        for operand in 0..3 {
            code.extend([if wrong == operand { 0x42 } else { 0x41 }, 0]);
        }
...
fn all_element_modes_validate_references_types_and_shared_resource_limits() {
    for mode in [1, 3, 5, 7] {
        ...
        assert!(matches!(
            WasmNumericVm::parse(&bytes, WasmNumericLimits::default()),
            Err(WasmNumericVmError::UnknownFunction { function_index: 99 })
        ));
    }
    for bad in [
        vec![1, 8],
        ...
        vec![1, 5, 0x6f, 0],
        vec![1, 3, 0, 2, 0],
        vec![1, 7, 0x70, 1, 0xd2, 0, 0xd2, 1, 0x0b],
    ] {
        assert!(
            WasmNumericVm::parse(
                &fixture(&[0x0b], &[], &[], &bad),
...
fn passive_initialization_does_not_bypass_indirect_signature_checks() {
    ...
    instance.call_export("f", &args([0, 0, 1])).unwrap();
    assert!(matches!(
        instance.call_export("b", &[WasmBoundaryValue::I32(0)]),
        Err(WasmNumericVmError::State(

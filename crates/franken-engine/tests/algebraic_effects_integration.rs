...
fn test_console_effect_round_trip() {
    ...
    let result = stack.handle_effect(&effect).expect("Console effect should be handled");
...
fn test_fs_effect_round_trip() {
    ...
    let write_result = stack.handle_effect(&write_effect).expect("Write should succeed");
    let bytes_written: u64 = write_result.downcast().expect("Write should return byte count");
    ...
    let read_result = stack.handle_effect(&read_effect).expect("Read should succeed");
...
fn test_fs_range_reading() {
    ...
    let result = stack.handle_effect(&read_effect).expect("Range read should succeed");
...
fn test_fs_append_mode() {
    ...
    stack.handle_effect(&append_effect).expect("Append should succeed");
    ...
    let result = stack.handle_effect(&read_effect).expect("Read should succeed");
    let data: Vec<u8> = result.downcast().expect("Should return data");
...
fn test_proc_spawn_effect() {
    ...
    assert!(effect.required_capabilities().custom_caps.contains("proc:spawn"));

    let params = effect.parameters();
    let (cmd, args, env_params, cwd) = params.downcast_ref::<(String, Vec<String>, BTreeMap<String, String>, Option<String>)>().unwrap();
    assert_eq!(*cmd, "echo");
...
fn test_capability_requirements() {
    ...
    assert_eq!(console_effect.required_capabilities(), EffectCapabilities::none());
    ...
    assert!(net_caps.runtime_caps.contains(&RuntimeCapability::NetworkEgress));
    ...
    assert!(policy_caps.runtime_caps.contains(&RuntimeCapability::PolicyRead));
    ...
    assert!(builtin_caps.runtime_caps.contains(&RuntimeCapability::VmDispatch));
}
...
fn test_migration_adapter_console_round_trip() {
    ...
    let result = adapter.dispatch_hostcall(
        "console:error",
        &["Critical".to_string(), "error".to_string(), "occurred".to_string()],
...
fn test_migration_adapter_file_round_trip() {
    ...
    let write_result = adapter.dispatch_hostcall(
        "fs:write",
        &["/migration/test.dat".to_string(), "Binary data content".to_string()],
...
fn test_effect_set_operations() {
    let console_set = EffectSet::from_effects([
...
fn test_circular_dependency_detection() {
    ...
    assert!(matches!(result, Err(EffectError::CircularDependency { .. })));
}
...
impl Handler for TestHandler {
    ...
    fn handle(&self, _effect: &dyn Effect) -> Result<Option<EffectResult>, EffectError> {

...
fn suspended_module_cancellation_preserves_shared_host_operation_and_other_waiter() {
    let mut runtime =
        AsyncModuleRuntime::with_defaults(&[node("a", true, &[]), node("b", true, &[])]).unwrap();
    ...
    assert!(
        runtime
            .cancel_module("a", JsValue::Int(9), Label::Confidential)
            .unwrap()
    );
    assert_eq!(
        runtime.promise_result(host).unwrap().0,
    ...
    assert!(runtime.next_task().unwrap().is_none());
    runtime
        .complete_task(&resume, JsValue::Int(43), Label::Secret)
        .unwrap();
...
fn dependency_blocked_module_can_be_cancelled_without_cancelling_provider() {
    ...
    assert!(
        runtime
            .cancel_module("blocked", JsValue::Int(9), Label::Secret)
            .unwrap()
    );
    let provider = runtime.next_task().unwrap().unwrap();
    assert_eq!(provider.module_specifier, "provider");
    runtime
        .complete_task(&provider, JsValue::Int(42), Label::Public)
        .unwrap();
    assert!(runtime.next_task().unwrap().is_none());
    assert_eq!(
...
fn in_flight_cancellation_revokes_every_guest_completion_operation() {
    ...
    assert!(
        runtime
            .cancel_module("app", JsValue::Int(9), Label::Secret)
            .unwrap()
    );
    let before = state(&runtime);
    assert!(
        runtime
            .complete_task(&task, JsValue::Int(999), Label::Public)
            .is_err()
    );
    assert!(
        runtime
            .reject_task(&task, JsValue::Int(999), Label::Public)
            .is_err()
    );
...
fn cancel_all_breaks_runtime_wait_cycles_and_preserves_prior_terminal_results() {
    ...
    runtime
        .complete_task(&done, JsValue::Int(7), Label::Public)
        .unwrap();
    let failed = runtime.next_task().unwrap().unwrap();
    assert_eq!(failed.module_specifier, "b-failed");
    runtime
        .reject_task(&failed, JsValue::Int(8), Label::Confidential)
        .unwrap();
    ...
    assert_eq!(
        runtime.cancel_all(JsValue::Int(99), Label::Secret).unwrap(),
        3
    );
    assert_eq!(runtime.snapshot().ready_tasks, 0);
    assert_eq!(runtime.snapshot().in_flight_tasks, 0);
    ...
    assert_eq!(
        runtime
            .promise_result(runtime.evaluation_promise("a-done").unwrap())
            .unwrap(),
        (&PromiseState::Fulfilled(JsValue::Int(7)), &Label::Public)
    );
    assert_eq!(
        runtime
            .promise_result(runtime.evaluation_promise("b-failed").unwrap())
            .unwrap(),
        (
    ...
    let before = state(&runtime);
    assert_eq!(
        runtime.cancel_all(JsValue::Int(0), Label::Public).unwrap(),
        0
    );
    assert!(
        !runtime
            .cancel_module("c", JsValue::Int(0), Label::Public)
            .unwrap()
    );
    assert_eq!(state(&runtime), before);
}
...
fn cancelling_an_unknown_module_is_read_only() {
    ...
    assert!(
        runtime
            .cancel_module("missing", JsValue::Int(9), Label::Secret)
            .is_err()
    );
    assert_eq!(state(&runtime), before);
    assert_eq!(
...
fn cancellation_removes_a_queued_fulfilled_await_continuation() {
    ...
    assert!(
        runtime
            .cancel_module("app", JsValue::Int(9), Label::Confidential)
            .unwrap()
    );
    assert!(runtime.next_task().unwrap().is_none());
    assert_eq!(
...
fn forged_sequence_cannot_complete_reject_or_suspend_a_live_lease() {
    ...
    assert!(
        runtime
            .complete_task(&forged, JsValue::Int(999), Label::Public)
            .is_err()
    );
    assert!(
        runtime
            .reject_task(&forged, JsValue::Int(999), Label::Public)
            .is_err()
    );

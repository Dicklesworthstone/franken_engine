...
fn cell_signing_key_from_seed(seed: u8) -> SigningKey {
...
fn cell_signing_key_for(cell_id: &str) -> SigningKey {
    // Stable per-cell-id seed so tests issuing two clients with the same id
    // get the same key.
    let mut seed: u8 = 0xA5;
    for byte in cell_id.as_bytes() {
        seed = seed.wrapping_add(*byte).wrapping_mul(31);
    }
    cell_signing_key_from_seed(seed)
}

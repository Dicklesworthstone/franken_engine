...
impl ConformanceTest for ManifestSchemaTest {
    fn name(&self) -> &str { "manifest_schema_version_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-1.1" }
    fn description(&self) -> &str { "Manifest MUST use franken-engine.proof-artifact-manifest.v1 schema" }
        ...
        match fixtures.create_valid_bundle() {
            Ok(bundle) => {
                ...
                let content = match fs::read_to_string(&manifest_path) {
                    Ok(content) => content,
                    Err(e) => return TestResult::Fail { reason: format!("Failed to read manifest: {}", e) }
                };

                let manifest: Value = match serde_json::from_str(&content) {
                    Ok(manifest) => manifest,
                    Err(e) => return TestResult::Fail { reason: format!("Invalid JSON: {}", e) }
                };
                ...
                match manifest.get("schema_version").and_then(|v| v.as_str()) {
                    ...
                    Some(version) => TestResult::Fail {
                        reason: format!("Wrong schema version: expected '{}', got '{}'", PROOF_MANIFEST_SCHEMA_VERSION, version)
                    },
                    None => TestResult::Fail { reason: "Missing schema_version field".to_string() }
                }
            }
            Err(e) => TestResult::Fail { reason: format!("Failed to create test bundle: {}", e) }
        }
    }
}
...
impl ConformanceTest for EventSchemaTest {
    fn name(&self) -> &str { "event_schema_version_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-1.2" }
    fn description(&self) -> &str { "Events MUST use franken-engine.proof-artifact-event.v1 schema" }
        ...
        match fixtures.create_valid_bundle() {
            Ok(bundle) => {
                ...
                match validate_events_jsonl_file(&events_path) {
                    ...
                    Err(e) => TestResult::Fail { reason: format!("Event validation failed: {}", e) }
                }
            }
            Err(e) => TestResult::Fail { reason: format!("Failed to create test bundle: {}", e) }
        }
...
impl ConformanceTest for ReportSchemaTest {
    fn name(&self) -> &str { "report_schema_version_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-1.3" }
    fn description(&self) -> &str { "Report MUST use franken-engine.proof-artifact-report.v1 schema" }
        ...
        match fixtures.create_valid_bundle() {
            Ok(bundle) => {
                ...
                let content = match fs::read_to_string(&report_path) {
                    Ok(content) => content,
                    Err(e) => return TestResult::Fail { reason: format!("Failed to read report: {}", e) }
                };
                ...
                match report.get("schema_version").and_then(|v| v.as_str()) {
                    ...
                    Some(version) => TestResult::Fail {
                        reason: format!("Wrong schema version: expected '{}', got '{}'", PROOF_REPORT_SCHEMA_VERSION, version)
                    },
                    None => TestResult::Fail { reason: "Missing schema_version field".to_string() }
                }
            }
...
impl ConformanceTest for RedactionSchemaTest {
    fn name(&self) -> &str { "redaction_schema_version_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-1.4" }
    fn description(&self) -> &str { "Redaction policy MUST use franken-engine.proof-artifact-redaction-policy.v1 schema" }
        ...
        match fixtures.create_valid_bundle() {
            Ok(bundle) => {
                ...
                let content = match fs::read_to_string(&policy_path) {
                    Ok(content) => content,
                    Err(e) => return TestResult::Fail { reason: format!("Failed to read redaction policy: {}", e) }
                };
                ...
                match policy.get("schema_version").and_then(|v| v.as_str()) {
                    ...
                    Some(version) => TestResult::Fail {
                        reason: format!("Wrong schema version: expected '{}', got '{}'", REDACTION_POLICY_SCHEMA_VERSION, version)
                    },
                    None => TestResult::Fail { reason: "Missing schema_version field".to_string() }
                }
            }
...
impl ConformanceTest for ManifestRequiredFieldsTest {
    fn name(&self) -> &str { "manifest_required_fields_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-2.1" }
    fn description(&self) -> &str { "Manifest MUST contain bundle_id, gate_name, status, generated_utc, source_revision, artifact_paths, and freshness" }
        ...
        match fixtures.create_valid_bundle() {
            Ok(bundle) => {
                ...
                let manifest: ProofManifest = match serde_json::from_str(&content) {
                    Ok(manifest) => manifest,
                    Err(e) => return TestResult::Fail { reason: format!("Invalid manifest JSON: {}", e) }
                };

                // Use the contract's validation function
                match manifest.validate() {
                    Ok(()) => TestResult::Pass,
                    Err(e) => TestResult::Fail { reason: format!("Manifest validation failed: {}", e) }
                }
            }
            Err(e) => TestResult::Fail { reason: format!("Failed to create test bundle: {}", e) }
        }
...
impl ConformanceTest for EventRequiredFieldsTest {
    fn name(&self) -> &str { "event_required_fields_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-2.2" }
    fn description(&self) -> &str { "Events MUST contain schema_version, event_name, severity, step_id, and decision" }
        ...
        match fixtures.create_valid_bundle() {
            Ok(bundle) => {
                ...
                match validate_events_jsonl_file(&events_path) {
                    ...
                    Err(e) => TestResult::Fail { reason: format!("Events JSONL validation failed: {}", e) }
                }
            }
            Err(e) => TestResult::Fail { reason: format!("Failed to create test bundle: {}", e) }
        }
...
impl ConformanceTest for ArtifactPathsTest {
    fn name(&self) -> &str { "artifact_paths_structure_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-2.3" }
    fn description(&self) -> &str { "Artifact paths MUST contain run_dir, manifest_json, commands_txt, events_jsonl, report_json, report_md, redaction_policy_json" }
...
impl ConformanceTest for PathNormalizationTest {
    fn name(&self) -> &str { "path_normalization_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-3.1" }
    fn description(&self) -> &str { "All artifact paths MUST be normalized, relative, and contain no .. or absolute references" }
...
impl ConformanceTest for Sha256ValidationTest {
    fn name(&self) -> &str { "sha256_format_validation" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-4.1" }
    fn description(&self) -> &str { "SHA256 hashes MUST be 64 character lowercase hex strings" }
...
impl ConformanceTest for HashChainIntegrityTest {
    fn name(&self) -> &str { "hash_chain_integrity_validation" }
    fn category(&self) -> TestCategory { TestCategory::Integration }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Must }
    fn requirement_id(&self) -> &str { "CD3D2B4D-4.2" }
    fn description(&self) -> &str { "Generated artifact SHA256 hashes MUST match actual file contents" }
    ...
    fn run(&self, ctx: &TestContext) -> TestResult {
        ...
        match fixtures.create_valid_bundle() {
            Ok(bundle) => {
                ...
                let artifacts = match manifest.get("generated_artifacts").and_then(|v| v.as_array()) {
                    Some(artifacts) => artifacts,
                    None => return TestResult::Fail { reason: "Missing generated_artifacts array".to_string() }
                };

                for (i, artifact) in artifacts.iter().enumerate() {
                    let path = match artifact.get("path").and_then(|v| v.as_str()) {
                        Some(path) => path,
                        None => return TestResult::Fail { reason: format!("Artifact {} missing path", i) }
                    };
...
impl ArtifactHashTest { pub fn new() -> Self { Self } }
impl ConformanceTest for ArtifactHashTest {
    fn name(&self) -> &str { "artifact_hash_test_placeholder" }
    fn category(&self) -> TestCategory { TestCategory::Unit }
    fn requirement_level(&self) -> RequirementLevel { RequirementLevel::Should }
    fn requirement_id(&self) -> &str { "CD3D2B4D-4.3" }
    fn description(&self) -> &str { "Placeholder for artifact hash test" }
...
macro_rules! placeholder_test {
    ($name:ident, $test_name:expr, $req_id:expr, $level:expr, $desc:expr) => {
        ...
        impl ConformanceTest for $name {
            fn name(&self) -> &str { $test_name }
            fn category(&self) -> TestCategory { TestCategory::Unit }
            fn requirement_level(&self) -> RequirementLevel { $level }
            fn requirement_id(&self) -> &str { $req_id }
...
placeholder_test!(JsonDepthLimitTest, "json_depth_limit_test", "CD3D2B4D-5.1", RequirementLevel::Must, "JSON nesting MUST not exceed MAX_JSON_DEPTH");
placeholder_test!(JsonSizeLimitTest, "json_size_limit_test", "CD3D2B4D-5.2", RequirementLevel::Must, "JSON values MUST not exceed MAX_JSON_VALUE_SIZE");
placeholder_test!(JsonStringLengthTest, "json_string_length_test", "CD3D2B4D-5.3", RequirementLevel::Must, "JSON strings MUST not exceed MAX_JSON_STRING_LENGTH");
placeholder_test!(BundleStructureTest, "bundle_structure_test", "CD3D2B4D-6.1", RequirementLevel::Must, "Bundles MUST have proper directory structure");
placeholder_test!(RequiredArtifactRolesTest, "required_artifact_roles_test", "CD3D2B4D-6.2", RequirementLevel::Must, "Bundles MUST contain required artifact roles");
placeholder_test!(RedactionPolicyTest, "redaction_policy_test", "CD3D2B4D-7.1", RequirementLevel::Must, "Redaction policy MUST be properly configured");
placeholder_test!(SecretDetectionTest, "secret_detection_test", "CD3D2B4D-7.2", RequirementLevel::Must, "Secrets MUST be properly redacted");
placeholder_test!(EmptyBundleTest, "empty_bundle_test", "CD3D2B4D-8.1", RequirementLevel::Should, "Empty bundles should be handled gracefully");
placeholder_test!(LargeBundleTest, "large_bundle_test", "CD3D2B4D-8.2", RequirementLevel::Should, "Large bundles should be handled efficiently");
placeholder_test!(CorruptedArtifactTest, "corrupted_artifact_test", "CD3D2B4D-8.3", RequirementLevel::Should, "Corrupted artifacts should be detected");
placeholder_test!(ManifestRoundTripTest, "manifest_round_trip_test", "CD3D2B4D-9.1", RequirementLevel::Must, "Manifest MUST survive round-trip serialization");
placeholder_test!(EventRoundTripTest, "event_round_trip_test", "CD3D2B4D-9.2", RequirementLevel::Must, "Events MUST survive round-trip serialization");
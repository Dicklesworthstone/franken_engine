...
fn trusted_context() -> GuardplaneExtensionContext {
    ...
    metadata.insert("guardplane.enable_instruction_hooks".to_string(), "true".to_string());
    metadata.insert("capability_witness.trust_level".to_string(), "trusted".to_string());
    metadata.insert("capability_witness.confidence_millionths".to_string(), "990000".to_string());
    metadata.insert("capability_witness.required_capabilities".to_string(), "module.import,network.fetch".to_string());
...
fn suspicious_context() -> GuardplaneExtensionContext {
    let mut metadata = BTreeMap::new();
    metadata.insert("guardplane.enable_instruction_hooks".to_string(), "true".to_string());
    metadata.insert("capability_witness.trust_level".to_string(), "suspicious".to_string());
    metadata.insert("capability_witness.confidence_millionths".to_string(), "150000".to_string());
    metadata.insert("capability_witness.denied_capabilities".to_string(), "module.import,system.exec".to_string());
...
fn compromised_context() -> GuardplaneExtensionContext {
    let mut metadata = BTreeMap::new();
    metadata.insert("guardplane.enable_instruction_hooks".to_string(), "true".to_string());
    metadata.insert("capability_witness.trust_level".to_string(), "compromised".to_string());
    metadata.insert("capability_witness.confidence_millionths".to_string(), "50000".to_string());
    metadata.insert("capability_witness.denied_capabilities".to_string(), "module.import,network.fetch,system.exec,filesystem.write".to_string());
...
fn provisional_context_with_capability_witness() -> GuardplaneExtensionContext {
    let mut metadata = BTreeMap::new();
    metadata.insert("guardplane.enable_instruction_hooks".to_string(), "true".to_string());
    metadata.insert("capability_witness.trust_level".to_string(), "provisional".to_string());
    metadata.insert("capability_witness.confidence_millionths".to_string(), "750000".to_string());
    metadata.insert("capability_witness.required_capabilities".to_string(), "network.fetch".to_string());
    metadata.insert("capability_witness.denied_capabilities".to_string(), "system.exec".to_string());

    GuardplaneExtensionContext::new("ext:provisional-witness", BTreeSet::new(), metadata)
}

fn create_adapter(context: GuardplaneExtensionContext, loss_matrix: LossMatrix) -> GuardplaneAdapter {
...
fn containment_threshold_enforcement_under_adversarial_conditions() {
    ...
    assert!(escalation_triggered, "Expected containment action under adversarial conditions");
    assert!(!matches!(final_action, HookAction::Allow), "Should not allow continued adversarial behavior");
    ...
    assert_ne!(last_record.threshold_action, ThresholdContainmentAction::Allow);

    // Verify risk accumulation shows escalation
    let summary = adapter.summary();
    assert!(summary.decision_count >= 3, "Should have multiple decisions recorded");
    assert!(summary.last_posterior_delta_millionths.unwrap() > 100_000, "Should show significant risk increase");
}
...
fn bayesian_posterior_integration_with_capability_witness_metadata() {
    ...
    let provisional_adapter = create_adapter(provisional_context_with_capability_witness(), LossMatrix::balanced());
    ...
    assert!(suspicious_escalated, "Suspicious context should eventually escalate");
    ...
    assert!(
        suspicious_summary.last_posterior_delta_millionths.unwrap_or(0) >
...
fn hook_policy_activation_deactivation_scenarios() {
    ...
    trusted_metadata.insert("capability_witness.trust_level".to_string(), "signed".to_string());
    trusted_metadata.insert("capability_witness.confidence_millionths".to_string(), "995000".to_string());

    let trusted_no_hooks = GuardplaneExtensionContext::new("ext:trusted-no-hooks", BTreeSet::new(), trusted_metadata);
    assert!(!trusted_no_hooks.instruction_hooks_enabled(), "Trusted contexts should not auto-enable hooks");

    // Suspicious context should auto-enable hooks
    let auto_enabled = suspicious_context();
    assert!(auto_enabled.instruction_hooks_enabled(), "Suspicious contexts should auto-enable hooks");

    // Explicit enablement should override trust level defaults
    let mut explicit_metadata = BTreeMap::new();
    explicit_metadata.insert("guardplane.enable_instruction_hooks".to_string(), "true".to_string());
    explicit_metadata.insert("capability_witness.trust_level".to_string(), "trusted".to_string());

    let explicit_enabled = GuardplaneExtensionContext::new("ext:explicit", BTreeSet::new(), explicit_metadata);
    assert!(explicit_enabled.instruction_hooks_enabled(), "Explicit enablement should override defaults");
...
fn expected_loss_selector_integration() {
    ...
    let balanced_adapter = create_adapter(provisional_context_with_capability_witness(), LossMatrix::balanced());
    ...
    let is_restrictive = |action: &HookAction| matches!(
        action,
        HookAction::Suspend | HookAction::Terminate(_) | HookAction::Quarantine(_)
    );
    ...
    if let (Some(aggressive_delta), Some(conservative_delta)) = (
        ...
        assert!(aggressive_delta >= 0 && conservative_delta >= 0, "Risk deltas should be non-negative");
    }
    ...
    assert!(!aggressive_records.is_empty(), "Should have decision records for aggressive adapter");
...
fn full_security_enforcement_pipeline_integration() {
    ...
    assert_eq!(risk_progression.len(), 5, "Should track risk for all operations");
    ...
    assert!(final_risk >= initial_risk, "Risk should not decrease with repeated suspicious operations");

    // At least one operation should trigger some containment action
    let has_containment = actions.iter().any(|action| !matches!(action, HookAction::Allow));
    assert!(has_containment, "Repeated suspicious operations should trigger containment");

    // Verify full pipeline state
    let final_summary = adapter.summary();
    assert_eq!(final_summary.decision_count, 5, "Should record all decisions");
    assert!(final_summary.decision_count > 0, "Should have made decisions");

    let final_records = adapter.decision_records();
    assert_eq!(final_records.len(), 5, "Should have decision records for all operations");

    // Verify the pipeline maintains coherent state throughout
    for (i, record) in final_records.iter().enumerate() {
        assert!(record.operation_index <= i + 1, "Operation index should be consistent");
        assert!(record.posterior_delta_millionths >= 0, "Posterior delta should be non-negative");
    }
}
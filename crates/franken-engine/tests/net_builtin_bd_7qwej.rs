...
// =========================================================================
// bd-asw4m.5: implicit net callbacks publish failure-atomic once wrappers,
// and socket.end(completion) lives outside the finish listener set.
// =========================================================================

#[test]
fn listen_and_connect_callbacks_expose_stable_wrappers_to_rawlisteners() {
    // The scheduled loopback tasks drain after the synchronous registration
    // block, so each introspection below observes the pre-fire publication
    // state: rawListeners returns the stable wrapper (never the original
    // callable), its .listener property is the original, and listeners()
    // reports the original through the wrapper's truthy .listener.
    let source = r#"
        const net = require('net');
...
fn socket_end_completion_carrier_is_invisible_and_fires_after_finish() {
    ...
    let source = r#"
        ...
        const onEnd = function () { this.endCompletionReceiver = null; };
        const onConnect = () => {

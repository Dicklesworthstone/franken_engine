...
use frankenengine_engine::ir_contract::{Instruction, OpCode, RegisterAddress, Value, ObjectId};
...
fn test_own_property_descriptor() {
    ...
    interpreter.heap.push(frankenengine_engine::baseline_interpreter::HeapObject {
    ...
    interpreter.set_object_property(obj_id, "ownProp".to_string(), Value::Number(42.0)).unwrap();
    ...
    let args = frankenengine_engine::baseline_interpreter::FunctionArgs {
        start: 0,
        count: 2,
    };
    let result = interpreter.builtin_function_call("builtin:ObjectGetOwnPropertyDescriptor", args).unwrap();
...
fn test_inherited_property_descriptor() {
    ...
    interpreter.set_object_property(parent_id, "inheritedProp".to_string(), Value::String("inherited".to_string())).unwrap();
    ...
    let own_result = interpreter.builtin_function_call("builtin:ObjectGetOwnPropertyDescriptor", args).unwrap();
    assert_eq!(own_result, Value::Undefined);

    // getPropertyDescriptor should find inherited property
    let inherited_result = interpreter.builtin_function_call("builtin:ObjectGetPropertyDescriptor", args).unwrap();
...
fn test_accessor_property_descriptor() {
    ...
    interpreter.set_object_property(obj_id, "accessorProp_getter".to_string(), Value::Object(getter_id)).unwrap();
    ...
    let result = interpreter.builtin_function_call("builtin:ObjectGetPropertyDescriptor", args).unwrap();
...
fn test_frozen_object_descriptor_flags() {
    ...
    interpreter.set_object_property(obj_id, "frozenProp".to_string(), Value::Number(123.0)).unwrap();
    ...
    match result {
        Value::Object(desc_id) => {
            let writable = interpreter.get_object_property(desc_id, "writable").unwrap();
            let configurable = interpreter.get_object_property(desc_id, "configurable").unwrap();
...
fn test_sealed_object_descriptor_flags() {
    ...
    interpreter.set_object_property(obj_id, "sealedProp".to_string(), Value::Number(456.0)).unwrap();
...
fn test_deep_prototype_chain() {
    ...
    interpreter.set_object_property(level1_id, "deepProp".to_string(), Value::String("deep".to_string())).unwrap();
...
fn test_shadowed_property_descriptor() {
    ...
    interpreter.set_object_property(parent_id, "shadowProp".to_string(), Value::String("parent".to_string())).unwrap();
    ...
    interpreter.set_object_property(child_id, "shadowProp".to_string(), Value::String("child".to_string())).unwrap();
    ...
    match (own_result, inherited_result) {
        (Value::Object(own_desc), Value::Object(inherited_desc)) => {
            ...
            let inherited_value = interpreter.get_object_property(inherited_desc, "value").unwrap();

...
fn red_team_compromise_rate_script_detects_stubs_and_refuses_observed_status() {
    ...
    env::set_var("CARGO_TARGET_DIR", test_target_dir);
    ...
    env::set_var("RED_TEAM_COMPROMISE_RATE_METRIC_ARTIFACT_ROOT", test_artifact_dir);
    env::set_var("RED_TEAM_COMPROMISE_RATE_METRIC_RUN_ID", "test_bd_12vhs");
    ...
    let output = Command::new("bash")
        ...
        .env("RED_TEAM_COMPROMISE_RATE_METRIC_ARTIFACT_ROOT", test_artifact_dir)
    ...
    let metric: Value = serde_json::from_str(&metric_content)
        .expect("Failed to parse metric artifact JSON");
    ...
    let report_content = std::fs::read_to_string(&report_path)
        .expect("Failed to read metric report");

    let report: Value = serde_json::from_str(&report_content)
        .expect("Failed to parse metric report JSON");
    ...
    if metric["has_placeholder_data"].as_bool().unwrap_or(false) {
        assert!(
            ...
            || report["reason"].as_str().unwrap().contains("awaiting_real_scenario_measurements"),
    ...
    println!("✓ Red team compromise rate script correctly detects stubs and applies defensive pattern");
}
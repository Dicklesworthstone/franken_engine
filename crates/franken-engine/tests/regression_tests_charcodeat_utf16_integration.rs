...
fn test_charcodeat_utf16_basic_regression() {
    ...
    if let Ok(Value::Number(n)) = result {
        ...
        panic!(
            "charCodeAt(0) for 'A' should return Number(65), got: {:?}",
            result
        );
    }
    ...
    if let Ok(Value::Number(n)) = result {
        ...
        panic!(
            "charCodeAt(0) for 'a' should return Number(97), got: {:?}",
            result
        );
    }
}
...
fn test_charcodeat_utf16_out_of_bounds_regression() {
    ...
    if let Ok(Value::Number(n)) = result {
        assert!(
            n.is_nan(),
            "charCodeAt(5) on 3-char string should return NaN"
        );
    } else {
...
fn test_charcodeat_utf16_surrogate_pairs_regression() {
    ...
    if let Ok(Value::Number(n)) = result {
        ...
        assert!(
            n >= 55296.0 && n <= 56319.0,
            "First code unit of emoji should be high surrogate, got: {}",
            n
        );
    } else {
        ...
        assert!(
            result.is_err(),
            "charCodeAt with UTF-16 may fail-closed, got: {:?}",
            result
        );
    }
}
...
fn test_charcodeat_no_args_regression() {
    ...
    if let Ok(Value::Number(n)) = result {
        assert_eq!(
            n, 116.0,
            "charCodeAt() without args should use index 0, 't' = 116"
        );
    } else {
...
fn test_charcodeat_type_coercion_regression() {
    ...
    if let Ok(Value::Number(n)) = result {
        assert_eq!(
            n, 101.0,
            "charCodeAt('1') should coerce to charCodeAt(1), 'e' = 101"
        );
    } else {
    ...
    if let Ok(Value::Number(n)) = result {
        assert_eq!(
            n, 101.0,
            "charCodeAt(true) should coerce to charCodeAt(1), 'e' = 101"
        );
    } else {
        // Type coercion might not be implemented, which is acceptable
        // Test documents expected behavior for when coercion is added
...
fn test_charcodeat_integration_pipeline_regression() {
    ...
    let result =
        interpreter.evaluate_expression("var s = 'world'; var code = s.charCodeAt(0); code");
    if let Ok(Value::Number(n)) = result {
        assert_eq!(
            n, 119.0,
            "charCodeAt through variable should return 'w' = 119"
        );
    } else {
        panic!(
            "charCodeAt integration should return 119, got: {:?}",
            result
        );
    }

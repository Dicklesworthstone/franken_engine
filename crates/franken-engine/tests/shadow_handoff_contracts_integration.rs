...
fn test_panel_bundle_schema_validation() {
    ...
    assert!(
        parsed.get("shadow_status").is_some(),
        "Missing shadow_status field"
    );
    assert!(
        parsed.get("source_freshness").is_some(),
        "Missing source_freshness field"
    );
    assert!(
        parsed.get("degraded_gates").is_some(),
        "Missing degraded_gates field"
    );
    assert!(
        parsed.get("replay_drift").is_some(),
        "Missing replay_drift field"
    );
    assert!(
        parsed.get("recommended_actions").is_some(),
        "Missing recommended_actions field"
    );
    assert!(
    ...
    assert!(
        parsed.get("bundle_version").is_some(),
        "Missing bundle_version field"
    );

    // Check shadow_status structure
    let shadow_status = parsed.get("shadow_status").unwrap();
    assert!(
        shadow_status.get("title").is_some(),
        "Missing shadow_status.title"
    );
    assert!(
        shadow_status.get("daemon_health").is_some(),
        "Missing shadow_status.daemon_health"
    );
    assert!(
        shadow_status.get("active_journals").is_some(),
        "Missing shadow_status.active_journals"
    );
    assert!(
        shadow_status.get("uptime_seconds").is_some(),
        "Missing shadow_status.uptime_seconds"
    );

    // Check source_freshness structure
    let source_freshness = parsed.get("source_freshness").unwrap();
    assert!(
        source_freshness.get("title").is_some(),
        "Missing source_freshness.title"
    );
    assert!(
        source_freshness.get("sources").is_some(),
        "Missing source_freshness.sources"
    );
    assert!(
        source_freshness.get("stale_source_count").is_some(),
        "Missing source_freshness.stale_source_count"
    );

    // Check degraded_gates structure
    let degraded_gates = parsed.get("degraded_gates").unwrap();
    assert!(
        degraded_gates.get("title").is_some(),
        "Missing degraded_gates.title"
    );
    assert!(
        degraded_gates.get("gates").is_some(),
        "Missing degraded_gates.gates"
    );
    assert!(
        degraded_gates.get("degraded_count").is_some(),
        "Missing degraded_gates.degraded_count"
    );

    // Check replay_drift structure
    let replay_drift = parsed.get("replay_drift").unwrap();
    assert!(
        replay_drift.get("title").is_some(),
        "Missing replay_drift.title"
    );
    assert!(
        replay_drift.get("drift_entries").is_some(),
        "Missing replay_drift.drift_entries"
    );
    assert!(
        replay_drift.get("total_drift_count").is_some(),
        "Missing replay_drift.total_drift_count"
    );

    // Check recommended_actions structure
    let recommended_actions = parsed.get("recommended_actions").unwrap();
    assert!(
        recommended_actions.get("title").is_some(),
        "Missing recommended_actions.title"
    );
    assert!(
        recommended_actions.get("actions").is_some(),
        "Missing recommended_actions.actions"
    );
    assert!(
        recommended_actions.get("priority_action_count").is_some(),
        "Missing recommended_actions.priority_action_count"
    );
...
fn test_missing_source_rendering_state() {
    ...
    let json =
        serde_json::to_string_pretty(&missing_panel).expect("Should serialize missing panel");
    ...
    assert_eq!(
        parsed.get("retry_in_seconds").unwrap().as_u64().unwrap(),
        30
    );

    // Test missing source panel without last fetch
    let missing_panel_no_history =
        create_missing_source_panel("Unknown Source", "No connection history available", None);
...
fn test_no_mutation_command_surfaces() {
    ...
    for action in &bundle.recommended_actions.actions {
        ...
        assert!(
            !action.command_preview.is_empty(),
            "Action should have command preview"
        );
        assert!(
            action.command_preview.starts_with("shadow-daemon"),
            "Commands should use shadow-daemon prefix"
        );

        // Commands should not contain dangerous flags
        assert!(
            !action.command_preview.contains("--force"),
            "Commands should not contain --force"
        );
        assert!(
            !action.command_preview.contains("--auto"),
            "Commands should not contain --auto"
        );
        assert!(
            !action.command_preview.contains("--yes"),
            "Commands should not contain --yes"
        );
    }
...
fn test_panel_bundle_serialization_roundtrip() {
    ...
    assert_eq!(
        original_bundle.bundle_version,
        restored_bundle.bundle_version
    );
    assert_eq!(
        original_bundle.shadow_status.active_journals,
        restored_bundle.shadow_status.active_journals
    );
    assert_eq!(
        original_bundle.shadow_status.uptime_seconds,
        restored_bundle.shadow_status.uptime_seconds
    );

    if let (DaemonHealth::Degraded { reason: orig }, DaemonHealth::Degraded { reason: restored }) = (
        &original_bundle.shadow_status.daemon_health,
        &restored_bundle.shadow_status.daemon_health,
    ) {
    ...
    assert_eq!(
        original_bundle.source_freshness.sources.len(),
        restored_bundle.source_freshness.sources.len()
    );
    assert_eq!(
        original_bundle.degraded_gates.gates.len(),
        restored_bundle.degraded_gates.gates.len()
    );
    assert_eq!(
        original_bundle.replay_drift.drift_entries.len(),
        restored_bundle.replay_drift.drift_entries.len()
    );
    assert_eq!(
        original_bundle.recommended_actions.actions.len(),
        restored_bundle.recommended_actions.actions.len()
    );
...
fn test_service_interface_contract_compliance() {
    ...
    let bundle = service
        .get_panel_bundle()
        .expect("Should provide panel bundle");
    assert!(
        !bundle.bundle_version.is_empty(),
        "Bundle version should not be empty"
    );
    assert!(
        !bundle.shadow_status.title.is_empty(),
        "Panel titles should not be empty"
    );
    ...
    let filtered_request = FilteredPanelsRequest {
        panels: panel_types,
    };
    let filtered_response = service
        .get_filtered_panels(filtered_request)
        .expect("Should filter panels");

    assert!(
        filtered_response.shadow_status.is_some(),
        "Requested shadow_status should be present"
    );
    assert!(
        filtered_response.source_freshness.is_some(),
        "Requested source_freshness should be present"
    );
    assert!(
        filtered_response.degraded_gates.is_none(),
        "Non-requested degraded_gates should be absent"
    );
    assert!(
        filtered_response.replay_drift.is_none(),
        "Non-requested replay_drift should be absent"
    );
    assert!(
        filtered_response.recommended_actions.is_none(),
        "Non-requested recommended_actions should be absent"
    );
    ...
    assert!(
        error_result.is_err(),
        "Should error for non-existent action"
    );
...
fn test_accessibility_and_scannability() {
    ...
    assert!(
        !bundle.shadow_status.title.is_empty(),
        "Shadow status panel should have title"
    );
    assert!(
        !bundle.source_freshness.title.is_empty(),
        "Source freshness panel should have title"
    );
    assert!(
        !bundle.degraded_gates.title.is_empty(),
        "Degraded gates panel should have title"
    );
    assert!(
        !bundle.replay_drift.title.is_empty(),
        "Replay drift panel should have title"
    );
    assert!(
        !bundle.recommended_actions.title.is_empty(),
        "Recommended actions panel should have title"
    );

    // Check that titles are descriptive and readable
    assert!(
        bundle.shadow_status.title.contains("Status"),
        "Status panel title should be descriptive"
    );
    assert!(
        bundle.source_freshness.title.contains("Freshness"),
        "Freshness panel title should be descriptive"
    );
    ...
    let degraded_json =
        serde_json::to_string(&degraded_entry).expect("Should serialize degraded entry");
    ...
    assert!(
        degraded_json.contains("Critical"),
        "Degraded severity should be serialized"
    );
    assert!(
        drift_json.contains("Major"),
        "Drift severity should be serialized"
    );
    assert!(
        action_json.contains("High"),
        "Action priority should be serialized"
    );

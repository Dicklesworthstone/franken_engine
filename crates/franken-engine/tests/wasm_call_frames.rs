...
use frankenengine_engine::wasm_runtime_lane::numeric::{
    WasmHostError, WasmHostImports, WasmNumericLimits, WasmNumericVm, WasmNumericVmError,
    WasmStateError,
};
...
const LARGE_LOCALS: &str =
    "0061736d010000000105016000017f0302010007060102663000000a08010601417f412a0b";
...
fn exact_call_depth_limits_remain_guest_faults() {
    ...
    assert_eq!(
        vm.call_export("f0", &[I32(64)]),
        Err(WasmNumericVmError::CallDepthExceeded { max: 64 })
    );
    let zero = WasmNumericVm::parse(
        &bytes(COUNTDOWN),
        WasmNumericLimits {
            max_call_depth: 0,
            ..WasmNumericLimits::default()
...
fn suspended_operands_and_locals_share_one_live_value_ceiling() {
    ...
    assert!(matches!(
        tight.call_export("f0", &[I32(10)]),
        Err(WasmNumericVmError::LiveValueLimitExceeded {
            actual: 22,
            max: 21
        })
    ));
    ...
    assert_eq!(
        tight.call_export("f0", &[I32(0)]).unwrap().results,
        [I32(0)]
    );
}
...
fn returning_discards_only_the_callee_control_and_operand_state() {
    let result = vm(EARLY_RETURN, WasmNumericLimits::default())
        .call_export("f1", &[])
        .unwrap();
...
fn trapped_call_chains_unwind_without_undoing_completed_guest_writes() {
    ...
    for expected in [1, 2] {
        ...
        assert_eq!(
            instance.call_export("f2", &[]).unwrap().results,
            [I32(expected)]
        );
...
fn imported_calls_resume_the_correct_frame_and_keep_revocation_checks() {
    ...
    let grants = [RuntimeCapability::VmDispatch, RuntimeCapability::Builtin]
        .into_iter()
        .collect();
    ...
    assert!(matches!(
        instance.call_export("f1", &[I32(34)]),
...
mod continuation_regressions {
    ...
    impl Program {
        fn bytes(&self) -> Vec<u8> {
            ...
            for function in &self.functions {
                declarations.extend(leb(function.ty as usize));
            }
            section(&mut bytes, 3, &declarations);
            if let Some(entries) = &self.table {
                let mut tables = vec![1, 0x70, 0];
                tables.extend(leb(entries.len()));
            ...
            for index in 0..self.functions.len() {
                ...
                exports.push(0);
                exports.extend(leb(index + self.imports.len()));
            }
            ...
            if let Some(entries) = &self.table {
                ...
                for entry in entries {
                    elements.extend(leb(*entry as usize));
                }
    ...
    fn triangle_code(call: &[u8]) -> Vec<u8> {
        let mut code = vec![
            0x20, 0, 0x45, 0x04, 0x7f, 0x41, 0, 0x05, 0x20, 0, 0x20, 0, 0x41, 1, 0x6b,
        ];
    ...
    fn mutual_recursion_restores_each_distinct_function_continuation() {
        ...
        program
            .functions
            .push(function(0, &triangle_code(&[0x10, 0])));
    ...
    fn depth_refusal_precedes_callee_writes_and_retains_completed_parent_writes() {
        let mut program = triangle(false);
        let mut code = vec![
            0x23, 0, 0x41, 1, 0x6a, 0x24, 0, 0x41, 0, 0x23, 0, 0x36, 2, 0,
        ];
        ...
        assert_eq!(vm.instantiate().unwrap().global_export("g"), Some(&I32(0)));
        assert_eq!(
            instance.call_export("f0", &[I32(0)]).unwrap().results,
            [I32(0)]
        );
    ...
    fn budget_refusal_unwinds_suspended_frames_without_poisoning_the_next_call() {
        ...
        let mut instance = vm.instantiate().unwrap();
        assert_eq!(
            instance.call_export("f0", &[I32(10)]),
            Err(WasmNumericVmError::InstructionBudgetExceeded { max: 8 })
        );
    ...
    fn nested_loop_labels_and_mutated_locals_resume_after_each_call() {
        let program = Program {
            ...
            functions: vec![
                Function {
                    ...
                    locals: vec![0x7f],
                    code: vec![
                        0x41, 0, 0x21, 1, 0x02, 0x40, 0x03, 0x40, 0x20, 0, 0x45, 0x0d, 1, 0x20, 1,
                        0x20, 0, 0x10, 1, 0x6a, 0x21, 1, 0x20, 0, 0x41, 1, 0x6b, 0x21, 0, 0x0c, 0,
        ...
        for n in [0, 1, 5, 50] {
            assert_eq!(
                vm.call_export("f0", &[I32(n)]).unwrap().results,
                [I32(n * (n + 1))]
            );
        }
    }
        ...
        let program = Program {
            ...
            functions: vec![
                function(
                    0,
                    &[
                        0x20, 0, 0x45, 0x04, 1, 0x20, 1, 0x20, 2, 0x20, 3, 0x05, 0x20, 0, 0x41, 1,
                        0x6b, 0x20, 1, 0x20, 2, 0x20, 3, 0x10, 0, 0x0b, 0x0b,
                    ],
                ),
                function(
                    2,
                    &[0x41, 7, 0x20, 0, 0x20, 1, 0x20, 2, 0x20, 3, 0x10, 0, 0x0b],
                ),
            ],
            ..Program::default()
        };
    ...
    fn a_guest_trap_does_not_resume_any_suspended_caller() {
        ...
        assert_eq!(
            ...
            Err(WasmNumericVmError::Unreachable { function_index: 1 })
        );
    ...
    fn host_program() -> Program {
        Program {
            ...
            functions: vec![function(
                1,
                &[
                    0x20, 0, 0x45, 0x04, 0x7f, 0x10, 0, 0x05, 0x20, 0, 0x41, 1, 0x6b, 0x10, 1,
                    0x41, 1, 0x6a, 0x0b, 0x0b,
                ],
            )],
            ..Program::default()
        }
    ...
    fn startup_uses_the_same_nonrecursive_executor_and_runs_only_once() {
        ...
        program
            .functions
            .push(function(1, &[0x41, 50, 0x10, 0, 0x24, 0, 0x0b]));
    ...
    fn tail_sum_code(call: &[u8]) -> Vec<u8> {
        let mut code = vec![
            0x20, 0, 0x45, 0x04, 0x7f, 0x20, 1, 0x05, 0x20, 0, 0x41, 1, 0x6b, 0x20, 1, 0x20, 0,
            0x6a,
        ];
        code.extend_from_slice(call);
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    assert_eq!(
                        ...
                        if indirect { 600_006 } else { 500_006 }
                    );
                    assert_eq!(result.peak_stack_values, 3);
                })
                .unwrap()
    ...
    fn mutual_tail_calls_switch_functions_without_growing_depth() {
        ...
        program
            .functions
            .push(function(0, &tail_sum_code(&[0x12, 0])));
        let vm = program.vm(WasmNumericLimits {
            max_call_depth: 1,
            ..WasmNumericLimits::default()
        });
        ...
        let program = Program {
            ...
            functions: vec![
                Function {
                    ty: 0,
                    locals: vec![0x7f],
                    code: vec![0x41, 9, 0x21, 0, 0x41, 7, 0x41, 42, 0x12, 1, 0x00, 0x0b],
                },
                Function {
                    ty: 1,
                    locals: vec![0x7f],
                    code: vec![0x20, 0, 0x20, 1, 0x6a, 0x0b],
                },
            ],
            ..Program::default()
        };
    ...
    fn tail_call_result_contracts_are_validated_even_in_unreachable_code() {
        for (params, results, code) in [
            ...
            let callee = if results.is_empty() {
                vec![0x0b]
            } else {
                vec![0x42, 0, 0x0b]
            };
    ...
    fn an_infinite_tail_cycle_exhausts_work_not_native_stack_or_call_depth() {
        ...
        for _ in 0..2 {
            assert_eq!(
                instance.call_export("f0", &[]),
                Err(WasmNumericVmError::InstructionBudgetExceeded { max: 10_000 })
            );
        }
    }
    ...
    fn full_leb_tail_indices_execute_without_becoming_spurious_opcodes() {
        for instruction in [vec![0x12, 0x80, 0], vec![0x41, 0, 0x13, 0x80, 0, 0x80, 0]] {
            ...
            let result = program
                .vm(WasmNumericLimits {
                    max_call_depth: 1,
                    ..WasmNumericLimits::default()
                })
                .call_export("f0", &[I32(100), I32(0)])
                .unwrap();
    ...
    fn tail_loops_reuse_live_value_capacity_without_bypassing_the_limit() {
        for indirect in [false, true] {
            ...
            assert!(matches!(
                tight.call_export("f0", &[I32(1), I32(0)]),
                Err(WasmNumericVmError::LiveValueLimitExceeded { actual: 5, max: 4 })
            ));
            assert_eq!(
                tight.call_export("f0", &[I32(0), I32(7)]).unwrap().results,
                [I32(7)]
            );
        }
    }
        ...
        let program = Program {
            ...
            functions: vec![
                Function {
                    ty: 0,
                    locals: vec![0x7f; 2],
                    code: vec![0x41, 7, 0x24, 0, 0x41, 7, 0x41, 42, 0x12, 1, 0x0b],
                },
                Function {
                    ty: 1,
                    locals: vec![0x7f; 4],
                    code: vec![0x41, 9, 0x24, 0, 0x20, 0, 0x0b],
                },
            ],
            ..Program::default()
        };
    ...
    fn zero_result_host_tails_resume_older_callers_and_cannot_bypass_revocation() {
        ...
        for indirect in [false, true] {
            let tail: &[u8] = if indirect {
                &[0x41, 0, 0x13, 1, 0, 0x0b]
            } else {
                &[0x12, 0, 0x0b]
            };
            let program = Program {
    ...
    fn startup_tail_calls_and_multivalue_tails_keep_the_existing_boundaries() {
        let startup = Program {
            types: vec![(vec![], vec![])],
            functions: vec![
                function(0, &[0x12, 1, 0x0b]),
                function(0, &[0x41, 7, 0x24, 0, 0x0b]),
            ],
            start: Some(0),
            ..Program::default()
        };
        let vm = startup.vm(WasmNumericLimits {
            max_call_depth: 1,
            ..WasmNumericLimits::default()
        });
        ...
        for indirect in [false, true] {
            ...
            code.extend_from_slice(if indirect {
                &[0x41, 0, 0x13, 0, 0, 0x0b]
            } else {
                &[0x12, 1, 0x0b]
            });
            ...
            let program = Program {
                ...
                functions: vec![
                    function(0, &code),
                    function(0, &[0x20, 0, 0x20, 1, 0x20, 2, 0x0b]),
                ],
                table: Some(vec![1]),
                ..Program::default()
            };

...
const SUM_DOWN: &[u8] = &[
    0x20, 0, 0x04, 0x7f, 0x41, 1, 0x20, 0, 0x41, 1, 0x6b, 0x10, 0, 0x6a, 0x05, 0x41, 0, 0x0b, 0x0b,
];
const FILL: &[u8] = &[0x41, 0, 0x41, 42, 0x20, 0, 0xfc, 11, 0, 0x0b];
const LOOP: &[u8] = &[
    0x02, 0x40, 0x03, 0x40, 0x20, 0, 0x45, 0x0d, 1, 0x23, 0, 0x41, 1, 0x6a, 0x24, 0, 0x20, 0, 0x41,
    1, 0x6b, 0x21, 0, 0x0c, 0, 0x0b, 0x0b, 0x23, 0, 0x0b,
];
...
fn fixture(params: &[u8], results: &[u8], bodies: &[&[u8]], host: bool) -> Vec<u8> {
    ...
    ty.extend(leb(params.len()));
    ty.extend_from_slice(params);
    ty.extend(leb(results.len()));
    ...
    for _ in 0..100_000 {
        match call.resume(quantum(work))? {
            ...
            WasmCallStep::Pending(next) => {
                assert!(
                    next.instructions_executed() > before,
                    "yield made no work progress"
                );
...
fn nested_calls_keep_suspended_operands_locals_and_depth() {
    ...
    for work in [1, 2, 7, 100] {
        let mut instance = vm.instantiate().unwrap();
        assert_eq!(
            finish(instance.begin_call("f", &[I32(40)]).unwrap(), work).unwrap(),
            expected
        );
    }
}
...
fn hard_instruction_budget_is_shared_across_all_slices() {
    ...
    assert_eq!(
        expected,
        WasmNumericVmError::InstructionBudgetExceeded { max: 31 }
    );
    for work in [1, 2, 5, 31, u64::MAX] {
        let mut sliced = vm.instantiate().unwrap();
        assert_eq!(
            finish(sliced.begin_call("f", &[I32(0)]).unwrap(), work).unwrap_err(),
            expected
        );
    ...
    for limits in [
        ...
        assert!(matches!(
            ...
            WasmNumericVmError::LiveValueLimitExceeded { .. }
                | WasmNumericVmError::CallDepthExceeded { .. }
        ));
        for work in [1, 3, 13] {
            let mut instance = vm.instantiate().unwrap();
            assert_eq!(
                finish(instance.begin_call("f", &[I32(40)]).unwrap(), work).unwrap_err(),
                expected
            );
        }
    }
...
fn arguments_are_owned_and_preparation_does_not_execute_guest_code() {
    ...
    for name in ["missing", "m", "g", "t"] {
        assert!(instance.begin_call(name, &[I32(0)]).is_err());
    }
    assert!(matches!(
        instance.begin_call("f", &[I64(0)]),
        Err(WasmNumericVmError::TypeMismatch { .. })
    ));
    assert!(matches!(
        instance.begin_call("f", &[]),
        Err(WasmNumericVmError::ArityMismatch { .. })
    ));
...
fn cancelling_a_pending_callee_does_not_enter_it() {
    let vm = WasmNumericVm::parse(
        &fixture(&[0x7f], &[0x7f], &[HOST_CALL], true),
        WasmNumericLimits::default(),
    )
    .unwrap();
    let calls = Arc::new(AtomicUsize::new(0));
    let mut instance = vm
        .instantiate_with_imports(provider(calls.clone()))
        .unwrap();
    ...
    assert_eq!(
        instance.call_export("f", &[I32(42)]).unwrap().results,
        [I32(42)]
    );
...
fn cancelling_after_an_atomic_host_call_keeps_its_effect_exactly_once() {
    ...
    let mut instance = vm
        .instantiate_with_imports(provider(calls.clone()))
        .unwrap();
    let WasmCallStep::Pending(call) = instance
        .begin_call("f", &[I32(42)])
        .unwrap()
        .resume(quantum(2))
        .unwrap()
    else {
        panic!("expected pending host");
    };
    let WasmCallStep::Pending(call) = call.resume(quantum(1)).unwrap() else {
        panic!("callback must yield before caller end");
    };
    assert_eq!(calls.load(Ordering::SeqCst), 1);
    assert!(
        call.instructions_executed() > 3,
        "atomic host cost overran the soft quantum"
    );
    drop(call);
    assert_eq!(
    ...
    assert_eq!(
        instance.call_export("h", &[I32(7)]).unwrap().results,
        [I32(7)]
    );
...
fn direct_indirect_and_exported_host_calls_match_blocking_execution() {
    for code in [HOST_CALL, INDIRECT_HOST_CALL, TWO_HOST_CALLS] {
        let vm = WasmNumericVm::parse(
            &fixture(&[0x7f], &[0x7f], &[code], true),
            WasmNumericLimits::default(),
        )
        .unwrap();
        for export in ["f", "h"] {
            let mut blocking = vm
                .instantiate_with_imports(provider(Arc::new(AtomicUsize::new(0))))
                .unwrap();
            ...
            for work in [1, 2, 4, 64] {
                ...
                let mut instance = vm
                    ...
                    .unwrap();
                assert_eq!(
                    finish(instance.begin_call(export, &[I32(-7)]).unwrap(), work).unwrap(),
                    expected
                );
                assert_eq!(instance.memory_export("m"), blocking.memory_export("m"));
                assert_eq!(
                    calls.load(Ordering::SeqCst),
                    if code == TWO_HOST_CALLS && export == "f" {
                        2
                    } else {
                        1
                    }
...
fn bulk_instruction_is_not_split_and_still_refuses_on_the_hard_budget() {
    ...
    let mut instance = vm.instantiate().unwrap();
    let WasmCallStep::Pending(call) = instance
        .begin_call("f", &[I32(4096)])
        .unwrap()
        .resume(quantum(4))
        .unwrap()
    else {
        panic!("fill should yield before end");
    };
    assert_eq!(call.instructions_executed(), 68); // 4 opcodes + 4096 / 64
    call.cancel();
    assert!(
        instance.memory_export("m").unwrap()[..4096]
            .iter()
            .all(|byte| *byte == 42)
    );
    let limited = self::vm(
    ...
    assert_eq!(
        finish(instance.begin_call("f", &[I32(4096)]).unwrap(), 1).unwrap_err(),
...
fn sliced_and_blocking_host_transcripts_are_identical_and_replay_with_new_quanta() {
    let vm = WasmNumericVm::parse(
        &fixture(&[0x7f], &[0x7f], &[TWO_HOST_CALLS], true),
        WasmNumericLimits::default(),
    )
    .unwrap();
    ...
    for work in [1, 2, 9, 1024, u64::MAX] {
        ...
        assert_eq!(
            finish(live.begin_call("f", &[I32(42)]).unwrap(), work).unwrap(),
            expected
        );
        assert_eq!(calls.load(Ordering::SeqCst), 2);
        ...
        assert_eq!(
            finish(replayed.begin_call("f", &[I32(42)]).unwrap(), work).unwrap(),
            expected
        );
...
fn multivalue_returns_retain_exact_float_and_integer_bits_across_call_boundaries() {
    ...
    for work in [1, 2, 3] {
        let mut instance = vm.instantiate().unwrap();
        assert_eq!(
            finish(instance.begin_call("f", &args).unwrap(), work).unwrap(),
            expected
        );
    }
}
...
mod resolved {
    ...
    fn context() -> ResolutionContext {
        ResolutionContext::new("slice-trace", "slice-decision", "current")
    }
    ...
    fn module(
        ...
        resolver
            .register_workspace_module(
                "/app/main.mjs",
                ModuleDefinition::new(ModuleSyntax::EsModule, "import './task.wasm';"),
            )
            .unwrap();
        resolver
            .register_workspace_module(
        ...
        resolver
            .load_wasm(
                &ModuleRequest::new("./task.wasm", ImportStyle::Import)
    ...
    fn revocation_at_a_yield_prevents_pending_host_entry_but_retains_prior_guest_writes() {
        ...
        for capability in [
            ...
            let mut instance = module
                .instantiate_with_imports(&context(), &grants, provider(calls.clone()))
                .unwrap();
            let call = instance
                .begin_call("f", &[I32(42)], &context(), &grants)
                .unwrap();
            let WasmNativeCallStep::Pending(call) =
                call.resume(quantum(4), &context(), &grants).unwrap()
            else {
                panic!("expected pending callback");
            };
            assert_eq!(calls.load(Ordering::SeqCst), 0);
            ...
            let fresh = instance
                .begin_call("f", &[I32(7)], &context(), &grants)
                .unwrap();
    ...
    fn request_and_canonical_deny_lists_are_rechecked_on_resume() {
        ...
        for name in ["./task.wasm", "/app/task.wasm"] {
            let mut instance = module.instantiate(&context(), &grants).unwrap();
            let WasmNativeCallStep::Pending(call) = instance
                .begin_call("f", &[I32(42)], &context(), &grants)
                .unwrap()
                .resume(quantum(1), &context(), &grants)
                .unwrap()
            else {
                panic!("expected yield before end");
            };
            denied(call.resume(quantum(1), &context(), &grants.clone().deny_specifier(name)));
            assert_eq!(
                instance
                    .call_export("f", &[I32(7)], &context(), &grants)
                    .unwrap()
                    .results,
                [I32(7)]
            );
    ...
    fn denied_preparation_cannot_acquire_a_continuation_or_mutate_guest_state() {
        ...
        instance
            .begin_call("f", &[I32(99)], &context(), &grants)
            .unwrap()
            .cancel();
        assert_eq!(
            instance.global_export("g", &context(), &grants).unwrap(),
    ...
    fn resumed_native_calls_keep_hard_limits_and_release_the_instance_after_failure() {
        ...
        let mut instance = module.instantiate(&context(), &grants).unwrap();
        let call = instance
            .begin_call("f", &[I32(0)], &context(), &grants)
            .unwrap();
        assert!(matches!(
            finish(call, 1, &grants),
        ...
        assert!(
            instance
                .global_export("g", &context(), &grants)
                .unwrap()
                .is_some()
        );
        let call = instance
    ...
    fn cancellation_releases_exclusive_state_for_permanent_host_revocation() {
        ...
        let WasmNativeCallStep::Pending(call) = instance
            .begin_call("f", &[I32(42)], &context(), &grants)
            .unwrap()
            .resume(quantum(2), &context(), &grants)
            .unwrap()
        else {
            panic!("expected pending host");
        };
...
fn an_independent_instance_can_finish_while_another_call_is_suspended() {
    ...
    let WasmCallStep::Pending(call) = first
        .begin_call("f", &[I32(20)])
        .unwrap()
        .resume(quantum(7))
        .unwrap()
    else {
        panic!("expected a live loop continuation");
    };
    assert_eq!(
        second.call_export("f", &[I32(3)]).unwrap().results,
        [I32(3)]
    );
...
fn setup_work_alone_can_end_a_slice_before_any_guest_opcode() {
    ...
    let WasmCallStep::Pending(call) = instance
        ...
        panic!("frame setup should consume the first quantum");
    };
...
fn u64_work_counter_endpoint_traps_instead_of_yielding_forever() {
    ...
    let WasmCallStep::Pending(call) = instance
        .begin_call("f", &[I32(42)])
        .unwrap()
        .resume(quantum(u64::MAX))
        .unwrap()
    else {
        panic!("caller end must still be pending");
    };
    assert_eq!(call.instructions_executed(), u64::MAX);
    assert!(matches!(
        call.resume(quantum(1)),
...
fn cancellation_signal_between_slices_is_observed_before_pending_host_entry() {
    ...
    for (code, work) in [(HOST_CALL, 2), (INDIRECT_HOST_CALL, 4)] {
        ...
        imports
            .bind_cancellation(token.clone(), "between-slices")
            .unwrap();
        let mut instance = vm.instantiate_with_imports(imports).unwrap();
        let WasmCallStep::Pending(call) = instance
            .begin_call("f", &[I32(42)])
...
fn live_service_revocation_between_slices_preserves_only_the_completed_callback() {
    ...
    let mut imports = provider(calls.clone());
    imports
        .bind_capability_revocation(RuntimeCapability::Builtin, token.clone(), "between-slices")
        .unwrap();
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    let WasmCallStep::Pending(call) = instance
        .begin_call("f", &[I32(42)])
        ...
        panic!("first host entry should be pending");
    };
    let WasmCallStep::Pending(call) = call.resume(quantum(1)).unwrap() else {
        panic!("caller should resume after the first callback");
    };
    assert_eq!(calls.load(Ordering::SeqCst), 1);
    token.cancel();
    token.reset();
...
fn tail_replacements_keep_one_activation_across_direct_and_indirect_slices() {
    for tail in [&[0x12, 0][..], &[0x41, 0, 0x13, 0, 0][..]] {
        ...
        for work in [1, 2, 7] {
            let mut instance = vm.instantiate().unwrap();
            assert_eq!(
                finish(instance.begin_call("f", &[I32(100)]).unwrap(), work).unwrap(),
                expected
            );
        }
    }
...
fn yielding_before_a_host_tail_neither_repeats_nor_resurrects_its_caller() {
    ...
    let WasmCallStep::Pending(call) = instance
        ...
        panic!("tail callback should be pending");
    };
    assert_eq!(calls.load(Ordering::SeqCst), 0);
    let WasmCallStep::Complete(result) = call.resume(quantum(1)).unwrap() else {
        panic!("a host tail has no caller left to resume");
    };
    assert_eq!(result.results, [I32(42)]);

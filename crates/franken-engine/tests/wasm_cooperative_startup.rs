...
use frankenengine_engine::wasm_runtime_lane::{WasmBoundaryValue::I32, WasmFunctionSignature, WasmValueType};
...
fn quantum(n: u64) -> NonZeroU64 { NonZeroU64::new(n).unwrap() }
...
fn run<F: Future + Unpin>(mut future: F) -> F::Output {
    ...
    for _ in 0..100_000 {
        ...
        match poll(&mut future, &wakes) {
            Poll::Ready(result) => {
                assert_eq!(wakes.0.load(Ordering::SeqCst), before, "Ready must not self-schedule");
                return result;
            }
...
fn program(code: &[u8], imports: u8, start: Option<u32>) -> Vec<u8> {
    ...
    section(&mut bytes, 7, &[5, 1, b's', 0, imports, 1, b'g', 3, 0,
        1, b'f', 0, imports + 1, 1, b'm', 2, 0, 1, b't', 1, 0]);
    ...
    bodies.extend(leb(code.len() + 1)); bodies.push(0); bodies.extend_from_slice(code);
...
const STORES: &[u8] = &[0x41, 7, 0x24, 0, 0x41, 16, 0x41, 9, 0x36, 2, 0, 0x41, 42, 0x24, 0, 0x0b];
...
where F: FnMut(&mut WasmHostCaller<'_, '_>, &[frankenengine_engine::wasm_runtime_lane::WasmBoundaryValue])
        -> Result<Vec<frankenengine_engine::wasm_runtime_lane::WasmBoundaryValue>, WasmNumericVmError>
        + Send + Sync + 'static {
    ...
    imports.define("h", "f", WasmFunctionSignature { params: vec![], results: vec![] },
        BTreeSet::from([Builtin]), 3, callback).unwrap();
    imports
}
...
fn future_is_lazy_and_dropping_it_before_poll_has_no_effects() {
    ...
    drop(future);
    assert_eq!(calls.load(Ordering::SeqCst), 0);
    assert!(run(vm.instantiate_cooperatively_with_imports(counting(calls.clone()), quantum(1))).is_ok());
...
fn no_start_and_empty_start_preserve_distinct_metadata() {
    ...
    let wakes = Arc::new(Wakes::default());
    let Poll::Ready(Ok(instance)) = poll(&mut future, &wakes) else { panic!("no start must be immediately ready"); };
    assert_eq!(instance.start_execution(), None);
    assert_eq!(instance.global_export("g"), Some(&I32(0)));
    assert_eq!(&instance.memory_export("m").unwrap()[..3], &[0xff, 0x80, 0xfe]);
    assert_eq!(wakes.0.load(Ordering::SeqCst), 0);
    let empty = vm(&[0x0b], 0, Some(0));
    assert_eq!(run(empty.instantiate_cooperatively(quantum(1))).unwrap()
        .start_execution().unwrap().instructions_executed, 1);
}
...
fn dropping_pending_start_keeps_completed_external_effects_without_running_later_calls() {
    ...
    let wakes = Arc::new(Wakes::default());
    for _ in 0..3 { assert!(poll(&mut future, &wakes).is_pending()); }
    assert_eq!(calls.load(Ordering::SeqCst), 0, "pending callee has not entered");
...
fn linking_rejects_all_missing_or_mismatched_imports_before_allocation() {
    ...
    assert!(matches!(run(vm.instantiate_cooperatively_with_imports(imports, quantum(1))),
        Err(WasmNumericVmError::State(WasmStateError::Host(WasmHostError::MissingBinding { name, .. }))) if name == "x"));
    let mut imports = WasmHostImports::new(BTreeSet::from([Builtin, VmDispatch]));
    imports.define("h", "f", WasmFunctionSignature { params: vec![WasmValueType::I32], results: vec![] },
        BTreeSet::from([Builtin]), 1, |_, _| panic!("wrong ABI")).unwrap();
    assert!(matches!(run(vm.instantiate_cooperatively_with_imports(imports, quantum(1))),
...
fn default_route_does_not_require_unused_imports_or_install_host_authority() {
    ...
    assert!(matches!(run(start.instantiate_cooperatively(quantum(1))),
...
fn one_start_budget_is_shared_across_all_polls_and_later_exports_get_their_own() {
    for work in [1, 2, 128] {
        ...
        let low = WasmNumericVm::parse(&bytes, WasmNumericLimits {
            max_instructions: 7, ..WasmNumericLimits::default()
        }).unwrap();
        assert!(matches!(run(low.instantiate_cooperatively(quantum(work))),
            Err(WasmNumericVmError::InstructionBudgetExceeded { max: 7 })));
        let exact = WasmNumericVm::parse(&bytes, WasmNumericLimits {
            max_instructions: 8, ..WasmNumericLimits::default()
        }).unwrap();
...
fn nested_and_tail_start_calls_keep_the_existing_activation_limits() {
    ...
    assert_eq!(result.start_execution(), nested.instantiate().unwrap().start_execution());
    assert_eq!(result.start_execution().unwrap().max_call_depth, 2);
    for tail in [vec![0x12, 0], vec![0x41, 0, 0x13, 0, 0]] {
        let mut code = vec![0x23, 0, 0x41, 1, 0x6a, 0x24, 0, 0x23, 0, 0x41, 10, 0x48, 0x04, 0x40];
        code.extend(tail); code.extend([0x0b, 0x0b]);
        let vm = WasmNumericVm::parse(&program(&code, 0, Some(0)), WasmNumericLimits {
        ...
        assert_eq!(instance.start_execution(), vm.instantiate().unwrap().start_execution());
...
fn imported_start_uses_the_same_host_abi_and_records_once() {
    ...
    let recorder = imports.record_calls(WasmHostTraceLimits::default()).unwrap();
...
fn startup_recordings_replay_across_different_quanta_without_provider_entry() {
    ...
    for work in [1, 100_000] {
        ...
        let replay = imports.replay_calls(tape.clone(), WasmHostTraceLimits::default()).unwrap();
        let reproduced = run(vm.instantiate_cooperatively_with_imports(imports, quantum(work))).unwrap();
...
fn failed_start_keeps_recorded_host_effects_but_cannot_publish_or_restart() {
    ...
    let wakes = Arc::new(Wakes::default());
    assert!(matches!(poll(&mut future, &wakes), Poll::Ready(Err(WasmNumericVmError::Unreachable { .. }))));
    assert_eq!(calls.load(Ordering::SeqCst), 1);
    assert_eq!(recorder.snapshot().unwrap().call_count(), 1);
    assert!(std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| poll(&mut future, &wakes))).is_err());
    assert_eq!(calls.load(Ordering::SeqCst), 1);
}
...
fn cancellation_before_first_poll_wins_before_memory_refusal_even_after_reset() {
    ...
    imports.bind_execution_cancellation(token.clone(), "lazy-start").unwrap();
    let vm = WasmNumericVm::parse(&program(STORES, 0, Some(0)), WasmNumericLimits {
        max_memory_pages: 0, ..WasmNumericLimits::default()
    }).unwrap();
    let future = vm.instantiate_cooperatively_with_imports(imports, quantum(1));
    token.cancel(); token.reset();
    assert!(matches!(run(future),
...
fn cancellation_between_start_polls_never_resurrects_unpublished_state() {
    ...
    let token = CancellationToken::new();
    let mut imports = WasmHostImports::new(BTreeSet::new());
    imports.bind_execution_cancellation(token.clone(), "between-start-polls").unwrap();
    let mut future = vm.instantiate_cooperatively_with_imports(imports, quantum(1));
    let wakes = Arc::new(Wakes::default());
    assert!(poll(&mut future, &wakes).is_pending());
    token.cancel(); token.reset();
    assert!(matches!(poll(&mut future, &wakes),
        Poll::Ready(Err(WasmNumericVmError::State(WasmStateError::Host(WasmHostError::ExecutionCancelled))))));
...
fn service_revocation_between_polls_blocks_the_pending_start_host_call() {
    ...
    let calls = Arc::new(AtomicUsize::new(0));
    let mut imports = counting(calls.clone());
    imports.bind_capability_revocation(Builtin, token.clone(), "start-revoke").unwrap();
    let mut future = vm.instantiate_cooperatively_with_imports(imports, quantum(1));
    let wakes = Arc::new(Wakes::default());
    for _ in 0..3 { assert!(poll(&mut future, &wakes).is_pending()); }
    token.cancel(); token.reset();
    assert!(matches!(poll(&mut future, &wakes), Poll::Ready(Err(WasmNumericVmError::State(

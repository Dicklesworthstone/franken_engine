...
fn control(token: &CancellationToken) -> WasmHostImports {
    ...
    imports
        .bind_execution_cancellation(token.clone(), "guest-execution")
        .unwrap();
    imports
}
    ...
    F: FnMut(&mut WasmHostCaller<'_, '_>, &[WasmBoundaryValue]) -> Outcome + Send + Sync + 'static,
{
    ...
    imports
        .bind_execution_cancellation(token.clone(), "guest-and-host")
        .unwrap();
    imports
        .define(
            "h",
...
fn cancelled(error: WasmNumericVmError) {
    assert_eq!(
        error,
        WasmNumericVmError::State(WasmStateError::Host(WasmHostError::ExecutionCancelled))
    );
}
...
fn hostless_control_preserves_results_state_and_work_accounting_when_not_cancelled() {
    for start in [false, true] {
        ...
        let mut controlled = vm
            .instantiate_with_imports(control(&CancellationToken::new()))
            .unwrap();
        assert_eq!(controlled.start_execution(), baseline.start_execution());
        for name in ["step", "indirect", "nested", "step"] {
            assert_eq!(
                controlled.call_export(name, &[]).unwrap(),
                baseline.call_export(name, &[]).unwrap()
            );
...
fn execution_signal_cannot_be_replaced_to_erase_a_pending_request() {
    ...
    token.reset();
    assert_eq!(
        imports.bind_execution_cancellation(CancellationToken::new(), "replacement"),
...
fn shared_execution_signal_stops_existing_instances_but_not_a_fresh_authorized_scope() {
    ...
    assert_eq!(
        unrelated.call_export("step", &[]).unwrap().results,
        [I32(1)]
    );
...
fn host_only_and_service_signals_remain_distinct_from_whole_instance_cancellation() {
    ...
    imports
        .bind_cancellation(host_signal.clone(), "host-only")
        .unwrap();
    imports
    ...
    assert_eq!(
        instance.call_export("nested", &[]).unwrap().results,
        [I32(1)]
    );
...
fn cancellation_inside_nested_host_leaf_retains_prior_stores_and_unwinds_guest_frames() {
    for name in ["step", "indirect", "nested"] {
        ...
        let mut instance = vm
            .instantiate_with_imports(host_control(&token, move |caller, _| {
        ...
        assert_eq!(instance.global_export("g"), Some(&I32(1)));
        assert_eq!(
            &instance.memory_export("m").unwrap()[..12],
            &[0, 0, 0, 0, 0, 0, 0, 0, 9, 8, 7, 6]
        );
        cancelled(instance.call_export("step", &[]).unwrap_err());
        let mut fresh = vm
            .instantiate_with_imports(host_control(&CancellationToken::new(), |_, _| {
                Ok(vec![I32(42)])
            }))
            .unwrap();
...
fn return_boundary_cancellation_is_recorded_without_losing_completed_host_effects() {
    ...
    for start in [false, true] {
        ...
        let mut bindings = host_control(&CancellationToken::new(), |_, _| {
...
fn inactive_execution_control_does_not_replace_budget_or_depth_refusals() {
    ...
    let mut instance = limited
        ...
        .unwrap();
    assert!(matches!(
        instance.call_export("nested", &[]),
        Err(WasmNumericVmError::CallDepthExceeded { max: 2 })
    ));
    assert!(matches!(
        instance.call_export("spin", &[]),
        Err(WasmNumericVmError::InstructionBudgetExceeded { max: 12 })
    ));
...
fn resolved_hostless_module_keeps_the_execution_subscription_after_loading() {
    ...
    resolver
        .register_workspace_module(
            "/app/pure.wasm",
            ModuleDefinition::wasm_binary(&program(false, false), &limits).unwrap(),
        )
        .unwrap();
    ...
    let module = resolver
        .load_wasm(
            &ModuleRequest::new("/app/pure.wasm", ImportStyle::Import),
            &ctx,
            &policy,
            limits,
        )
    ...
    let mut instance = module
        .instantiate_with_imports(&ctx, &policy, control(&signal))
        .unwrap();
    assert_eq!(
        instance
            .call_export("nested", &[], &ctx, &policy)
            .unwrap()
            .results,
        [I32(1)]
    );
...
fn direct_and_indirect_host_tail_calls_preserve_execution_cancellation() {
    for indirect in [false, true] {
        ...
        section(
            &mut bytes,
            7,
            &[2, 4, b's', b't', b'e', b'p', 0, 1, 1, b'm', 2, 0],
        );
        section(&mut bytes, 9, &[1, 0, 0x41, 0, 0x0b, 1, 0]);
        let body = if indirect {
            vec![0, 0x41, 0, 0x13, 0, 0, 0x0b]
        } else {
            vec![0, 0x12, 0, 0x0b]
        };
        let mut code = vec![1];
        ...
        let mut first = true;
        let mut instance = vm
            .instantiate_with_imports(host_control(&signal, move |caller, _| {
                if first {
                    first = false;
                    return Ok(vec![I32(42)]);
                }

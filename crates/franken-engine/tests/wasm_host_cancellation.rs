...
fn program(start: bool) -> Vec<u8> {
    ...
    section(
        &mut module,
        2,
        &[1, 3, b'e', b'n', b'v', 4, b's', b't', b'e', b'p', 0, 0],
    );
        ...
        + 'static,
{
    ...
    imports
        .bind_cancellation(token.clone(), "wasm-cancel-test")
        .unwrap();
    imports
        .define(
            "env",
            ...
            [RuntimeCapability::Builtin].into(),
            3,
            callback,
        )
        .unwrap();
...
fn add(
    _: &mut WasmHostCaller<'_, '_>,
    arguments: &[WasmBoundaryValue],
) -> Result<Vec<WasmBoundaryValue>, WasmNumericVmError> {
    let [I32(value)] = arguments else {
        panic!("validated argument type")
    };
...
fn already_cancelled_scope_refuses_before_state_allocation_or_startup() {
    ...
    .unwrap();
    let imports = imports(&token, |_, _| {
        panic!("cancelled startup must not call a provider")
    });
    token.reset();
...
fn cancellation_binding_cannot_be_replaced_with_a_fresh_signal() {
    ...
    assert_eq!(
        imports.bind_cancellation(CancellationToken::new(), "replacement"),
...
fn direct_indirect_and_exported_imports_all_observe_the_same_scope() {
    ...
    let mut instance = vm
        .instantiate_with_imports(imports(&token, move |caller, args| {
            observed.fetch_add(1, Ordering::SeqCst);
            add(caller, args)
        }))
        .unwrap();
    assert_eq!(
        instance.call_export("direct", &[I32(2)]).unwrap().results,
        [I32(42)]
    );
...
fn ignored_callback_checkpoint_refusal_cannot_publish_results_or_continue_guest() {
    ...
    let mut instance = vm
        .instantiate_with_imports(imports(&token, move |caller, _| {
...
fn post_callback_checkpoint_observes_a_request_even_when_provider_never_polls() {
    ...
    let signal = token.clone();
    let mut instance = vm
        .instantiate_with_imports(imports(&token, move |_, _| {
            signal.cancel();
            signal.reset();
            Ok(vec![I32(99)])
        }))
...
fn cancellation_is_sticky_for_one_instance_but_reset_allows_a_new_scope() {
    ...
    assert_eq!(
        fresh.call_export("host", &[I32(2)]).unwrap().results,
        [I32(42)]
    );
...
fn separate_tokens_keep_unrelated_instances_running() {
    ...
    assert_eq!(
        b.call_export("direct", &[I32(2)]).unwrap().results,
        [I32(42)]
    );
}
...
fn provider_checkpoints_do_not_spend_or_replenish_the_shared_work_budget() {
    ...
    let token = CancellationToken::new();
    let mut instance = vm
        .instantiate_with_imports(imports(&token, |caller, args| {
...
fn latched_budget_failure_is_not_replaced_by_the_new_checkpoint_path() {
    ...
    let token = CancellationToken::new();
    let mut instance = vm
        .instantiate_with_imports(imports(&token, |caller, _| {
        ...
        .unwrap();
    assert!(matches!(
        instance.call_export("direct", &[I32(1)]),
...
fn host_scope_cancellation_does_not_claim_to_preempt_guest_only_execution() {
    ...
    token.cancel();
    assert_eq!(
        instance.call_export("pure", &[I32(0)]).unwrap().results,
        [I32(42)]
    );
...
fn resolved_loader_preserves_the_bound_cancellation_scope_without_a_raw_vm_escape() {
    ...
    let definition = ModuleDefinition::wasm_binary(&program(false), &limits)
        .unwrap()
        .require_capability(RuntimeCapability::Builtin);
    let mut resolver = DeterministicModuleResolver::new("/app");
    resolver
        .register_workspace_module("/app/cancel.wasm", definition)
        .unwrap();
    let module = resolver
        .load_wasm(
            &ModuleRequest::new("/app/cancel.wasm", ImportStyle::Import),
            &context,
            &policy,
            limits,
        )
    ...
    let mut instance = module
        .instantiate_with_imports(&context, &policy, imports(&token, add))
        .unwrap();
    assert_eq!(
        instance
            .call_export("direct", &[I32(2)], &context, &policy)
            .unwrap()
            .results,
        [I32(42)]
    );
...
fn cancellation_records_completed_effects_and_replays_without_the_provider() {
    ...
    for start in [false, true] {
        ...
        let mut bindings = imports(&CancellationToken::new(), |_, _| {
            panic!("replay invoked provider")
        });
        let replay = bindings
        ...
        let actual = if start {
            ...
            assert_eq!(
                &replayed.memory_export("m").unwrap()[..8],
                &[1, 2, 3, 4, 0, 0, 0, 0]
            );
            error
        };
...
fn a_live_cancellation_cannot_be_bypassed_with_a_successful_replay_tape() {
    use frankenengine_engine::wasm_runtime_lane::host_replay::{
        WasmHostTraceError, WasmHostTraceLimits,
    };
    let vm = vm(false);
...
fn service_imports(granted: BTreeSet<RuntimeCapability>) -> WasmHostImports {
    ...
    for (name, capability) in [
        ("read", RuntimeCapability::FsRead),
        ("compute", RuntimeCapability::Builtin),
    ] {
        imports
            .define(
                "env",
...
fn revoked(error: WasmNumericVmError, capability: RuntimeCapability, name: &str) {
    assert_eq!(
        error,
        WasmNumericVmError::State(WasmStateError::Host(WasmHostError::CapabilityDenied {
...
fn service_revocation_blocks_only_the_bindings_that_require_it() {
    ...
    imports
        .bind_capability_revocation(RuntimeCapability::FsRead, signal.clone(), "read-authority")
        .unwrap();
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    assert_eq!(
        instance.call_export("read", &[I32(2)]).unwrap().results,
        [I32(42)]
    );
    signal.cancel();
    signal.reset();
    revoked(
        instance.call_export("read", &[I32(3)]).unwrap_err(),
        RuntimeCapability::FsRead,
        "read",
    );
    assert_eq!(
        instance.call_export("compute", &[I32(4)]).unwrap().results,
        [I32(44)]
    );
    revoked(
        instance.call_export("read", &[I32(5)]).unwrap_err(),
        RuntimeCapability::FsRead,
        "read",
    );
    assert_eq!(
        instance.call_export("compute", &[I32(6)]).unwrap().results,
        [I32(46)]
    );
}
...
fn vm_dispatch_revocation_covers_all_host_services() {
    ...
    for name in ["read", "compute"] {
        revoked(
            instance.call_export(name, &[I32(2)]).unwrap_err(),
            RuntimeCapability::VmDispatch,
            name,
        );
    }
...
fn an_already_revoked_required_service_prevents_linking_before_startup() {
    let vm = vm(true);
    let signal = CancellationToken::new();
    let mut imports = imports(&CancellationToken::new(), |_, _| {
        panic!("revoked startup callback")
    });
    signal.cancel();
    imports
        .bind_capability_revocation(
    ...
    signal.reset();
    revoked(
        vm.instantiate_with_imports(imports).unwrap_err(),
        RuntimeCapability::Builtin,
        "step",
    );
}
...
fn an_unused_revocation_signal_neither_grants_nor_blocks_other_services() {
    ...
    let mut imports = service_imports(service_grants());
    imports
        .bind_capability_revocation(RuntimeCapability::NetworkEgress, signal, "unused-network")
        .unwrap();
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    assert_eq!(
        instance.call_export("read", &[I32(2)]).unwrap().results,
    ...
    assert_eq!(
        instance.call_export("compute", &[I32(3)]).unwrap().results,
        [I32(43)]
    );
}
...
fn binding_a_signal_cannot_grant_an_absent_capability_or_replace_an_old_signal() {
    ...
    imports
        .bind_capability_revocation(RuntimeCapability::FsRead, signal.clone(), "missing-grant")
        .unwrap();
    revoked(
        vm.instantiate_with_imports(imports).unwrap_err(),
        RuntimeCapability::FsRead,
    ...
    let mut imports = service_imports(service_grants());
    imports
        .bind_capability_revocation(RuntimeCapability::FsRead, signal.clone(), "original")
        .unwrap();
    assert_eq!(
        imports.bind_capability_revocation(
            RuntimeCapability::FsRead,
            CancellationToken::new(),
            "replacement"
        ),
        Err(WasmHostError::CapabilityRevocationAlreadyBound {
...
fn in_callback_service_revocation_latches_buffer_and_result_refusal() {
    ...
    let mut imports = imports(&CancellationToken::new(), move |caller, _| {
        ...
        revoked(
            caller.write_memory(0, &[99; 4]).unwrap_err(),
            RuntimeCapability::Builtin,
            "step",
        );
        revoked(
            caller.read_memory(8, 3).unwrap_err(),
            RuntimeCapability::Builtin,
            "step",
        );
        revoked(
            caller.charge_work(0).unwrap_err(),
            RuntimeCapability::Builtin,
            "step",
        );
    ...
    imports
        .bind_capability_revocation(RuntimeCapability::Builtin, signal, "live-builtin")
        .unwrap();
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    revoked(
        instance.call_export("direct", &[I32(1)]).unwrap_err(),
        RuntimeCapability::Builtin,
        "step",
    );
    assert_eq!(
    ...
    revoked(
        instance.call_export("indirect", &[I32(1)]).unwrap_err(),
        RuntimeCapability::Builtin,
        "step",
    );
}
...
fn shared_service_revocation_reaches_every_existing_instance_after_reset() {
    ...
    let build_imports = || {
        let mut imports = service_imports(service_grants());
        imports
            .bind_capability_revocation(RuntimeCapability::FsRead, signal.clone(), "shared-read")
            .unwrap();
        imports
    };
    ...
    for instance in [&mut a, &mut b] {
        revoked(
            instance.call_export("read", &[I32(2)]).unwrap_err(),
            RuntimeCapability::FsRead,
            "read",
        );
        assert_eq!(
            instance.call_export("compute", &[I32(2)]).unwrap().results,
            [I32(42)]
        );
    }
    ...
    assert_eq!(
        fresh.call_export("read", &[I32(2)]).unwrap().results,
        [I32(42)]
    );
    revoked(
        a.call_export("read", &[I32(2)]).unwrap_err(),
        RuntimeCapability::FsRead,
        "read",
    );
}
    ...
    for capability in [RuntimeCapability::Builtin, RuntimeCapability::FsRead] {
        ...
        imports
            .define(
                ...
                [RuntimeCapability::Builtin, RuntimeCapability::FsRead].into(),
                3,
                move |caller, _| {
                    callback_signal.cancel();
            ...
            .unwrap();
        imports
            .bind_capability_revocation(capability, signal, "multi-capability")
            .unwrap();
        let mut instance = vm.instantiate_with_imports(imports).unwrap();
        revoked(
            instance.call_export("direct", &[I32(1)]).unwrap_err(),
...
fn whole_scope_cancellation_still_covers_services_without_revocation_subscriptions() {
    ...
    let mut imports = service_imports(service_grants());
    imports
        .bind_cancellation(whole_scope.clone(), "whole-scope")
        .unwrap();
    imports
        .bind_capability_revocation(RuntimeCapability::FsRead, read_scope.clone(), "read-only")
        .unwrap();
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    read_scope.cancel();
    revoked(
        instance.call_export("read", &[I32(1)]).unwrap_err(),
        RuntimeCapability::FsRead,
        "read",
    );
    assert_eq!(
...
fn a_successful_tape_cannot_regrant_a_live_revoked_service() {
    ...
    vm.instantiate_with_imports(bindings)
        .unwrap()
        .call_export("read", &[I32(2)])
        .unwrap();
    let signal = CancellationToken::new();
    let mut bindings = service_imports(service_grants());
    bindings
        .bind_capability_revocation(RuntimeCapability::FsRead, signal.clone(), "read-replay")
        .unwrap();
    let replay = bindings
        .replay_calls(
            recording.snapshot().unwrap(),
    ...
    signal.reset();
    revoked(
        replayed.call_export("read", &[I32(2)]).unwrap_err(),
        RuntimeCapability::FsRead,
        "read",
    );
    assert_eq!(
...
fn service_revocation_on_return_retains_a_replayable_effect_record() {
    ...
    bindings
        .bind_capability_revocation(RuntimeCapability::Builtin, signal, "record-builtin")
        .unwrap();
    let recording = bindings
        .record_calls(WasmHostTraceLimits::default())
        .unwrap();
    ...
    let mut replayed = vm.instantiate_with_imports(bindings).unwrap();
    assert_eq!(
        replayed.call_export("host", &[I32(2)]).unwrap_err(),
        expected
    );

...
impl Fixture {
    fn bytes(&self) -> Vec<u8> {
        ...
        types.extend(leb(self.params.len()));
        types.extend(&self.params);
        types.extend(leb(self.results.len()));
        types.extend(&self.results);
        types.extend([0x60, 0, 0]);
        section(&mut bytes, 1, &types);
        let mut imports = leb(self.imports.len());
        for (module, name) in &self.imports {
            imports.extend(leb(module.len()));
            imports.extend(module.as_bytes());
            imports.extend(leb(name.len()));
        ...
        let mut exports = vec![4, 1, b'f', 0];
        exports.extend(leb(self.imports.len()));
        exports.extend([1, b'h', 0, 0, 1, b'g', 3, 0, 1, b't', 1, 0]);
        section(&mut bytes, 7, &exports);
        if self.start.is_some() {
            section(&mut bytes, 8, &leb(self.imports.len() + 1));
        }
...
fn add(
    ...
    let [I32(a), I32(b)] = args else {
        panic!("typed arguments");
    };
...
fn default_instantiation_never_acquires_ambient_host_bindings() {
    ...
    for export in ["h", "f"] {
        assert!(matches!(
            vm.call_export(export, &[I32(20), I32(22)]),
...
fn all_imports_are_linked_before_startup_can_perform_any_host_effects() {
    ...
    assert!(
        matches!(result, Err(WasmNumericVmError::State(WasmStateError::Host(WasmHostError::MissingBinding { ref module, .. }))) if module == "missing")
    );
...
fn exact_names_and_structural_signatures_are_required_at_link_time() {
    for (module, name) in [("H", "f"), ("h", "F"), ("h.f", "")] {
        ...
        bindings
            ...
            .unwrap();
        assert!(matches!(
            Fixture::default().vm().instantiate_with_imports(bindings),
    ...
    for signature in [
        WasmFunctionSignature {
            params: vec![WasmValueType::I64; 2],
            results: vec![WasmValueType::I32],
        },
        WasmFunctionSignature {
            params: vec![WasmValueType::I32; 2],
...
fn revocation_cannot_be_bypassed_by_exported_imports_or_table_dispatch() {
    for indirect in [false, true] {
        ...
        let observed = calls.clone();
        let mut instance = vm
            .instantiate_with_imports(imports(grants(), move |caller, args| {
                observed.fetch_add(1, Ordering::SeqCst);
                add(caller, args)
            }))
            .unwrap();
        ...
        for export in ["f", "h"] {
            assert!(matches!(
                instance.call_export(export, &[I32(20), I32(22)]),
                Err(WasmNumericVmError::State(WasmStateError::Host(
                    WasmHostError::CapabilityDenied {
...
fn incorrect_arguments_and_host_results_never_enter_later_guest_code() {
    ...
    for results in [vec![], vec![I64(42)], vec![I32(42), I32(9)]] {
        let mut instance = vm
            .instantiate_with_imports(imports(grants(), move |_, _| Ok(results.clone())))
            .unwrap();
        assert!(matches!(
            instance.call_export("f", &[I32(20), I32(22)]),
            Err(WasmNumericVmError::ResultStackMismatch { .. }
    ...
    let mut instance = vm
        ...
        .unwrap();
    assert!(matches!(
        instance.call_export("h", &[I32(1)]),
        Err(WasmNumericVmError::ArityMismatch { .. })
    ));
    assert!(matches!(
        instance.call_export("h", &[I32(1), I64(2)]),
...
fn callbacks_preserve_exact_multivalue_bits() {
    let mut fixture = Fixture::default();
    fixture.params = vec![0x7d, 0x7e, 0x7c, 0x7f];
    fixture.results = fixture.params.clone();
...
fn host_startup_runs_once_and_shares_owned_callback_state_with_later_calls() {
    ...
    assert_eq!(
        first.call_export("f", &[I32(0), I32(0)]).unwrap().results,
        [I32(2)]
    );
    assert_eq!(
        first.call_export("h", &[I32(0), I32(0)]).unwrap().results,
        [I32(3)]
    );
    assert_eq!(
        second.call_export("f", &[I32(0), I32(0)]).unwrap().results,
        [I32(2)]
    );
    assert_eq!(first.global_export("g"), Some(&I32(1)));
}
...
fn host_budget_is_precharged_and_ignored_provider_refusals_remain_fatal() {
    ...
    let mut instance = vm
        ...
        .unwrap();
    assert!(matches!(
        instance.call_export("f", &[I32(1), I32(2)]),
        Err(WasmNumericVmError::InstructionBudgetExceeded { max: 6 })
    ));
    assert_eq!(calls.load(Ordering::SeqCst), 0);
    let vm = fixture.vm();
    let mut instance = vm
        .instantiate_with_imports(imports(grants(), |caller, _| {
    ...
    let mut instance = vm
        .instantiate_with_imports(imports(grants(), |caller, args| {
...
fn registration_rejects_missing_authority_free_calls_and_duplicate_replacement() {
    ...
    assert_eq!(
        ...
        Err(WasmHostError::DuplicateBinding {
            module: "h".into(),
            name: "f".into()
        })
    );
    assert_eq!(
        ...
        Err(WasmHostError::ZeroCallCost)
    );
    assert_eq!(
        bindings.define("h", "ambient", signature(), BTreeSet::new(), 1, add),
        Err(WasmHostError::MissingAuthority)
    );
    let vm = Fixture::default().vm();
    assert_eq!(
        vm.instantiate_with_imports(bindings)
            .unwrap()
            .call_export("h", &[I32(20), I32(22)])
            .unwrap()
            .results,
        [I32(42)]
    );
...
fn host_traps_retain_completed_guest_effects_and_do_not_publish_failed_starts() {
    ...
    let mut instance = vm
        .instantiate_with_imports(imports(grants(), |_, _| {
            Err(WasmHostError::trap("provider failure").into())
        }))
        .unwrap();
    assert!(matches!(
        instance.call_export("f", &[I32(1), I32(2)]),
    ...
    assert!(matches!(
        fixture
            .vm()
            .instantiate_with_imports(imports(grants(), |_, _| Err(WasmHostError::trap(
                "startup failure"
            )
            .into()))),
        Err(WasmNumericVmError::State(WasmStateError::Host(
...
fn cost_overflow_and_call_depth_refuse_before_host_entry() {
    ...
    bindings
        .define(
            ...
            [RuntimeCapability::Builtin].into_iter().collect(),
            u64::MAX,
            |_, _| panic!("cost overflow must refuse before entry"),
        )
        .unwrap();
    assert!(matches!(
        vm.instantiate_with_imports(bindings)
            .unwrap()
            .call_export("h", &[I32(1), I32(2)]),
        Err(WasmNumericVmError::InstructionBudgetExceeded { .. })
    ));
    let vm = WasmNumericVm::parse(
        &fixture.bytes(),
    ...
    let mut instance = vm
        .instantiate_with_imports(imports(grants(), |_, _| {
            panic!("depth refusal before host")
        }))
        .unwrap();
    assert!(matches!(
        instance.call_export("f", &[I32(1), I32(2)]),
        Err(WasmNumericVmError::CallDepthExceeded { max: 1 })
    ));
}
...
fn host_instances_preserve_send_and_sync_embedding_contracts() {
    ...
    require_send_sync::<
        frankenengine_engine::wasm_runtime_lane::numeric::WasmNumericInstance<'static>,
    >();
}

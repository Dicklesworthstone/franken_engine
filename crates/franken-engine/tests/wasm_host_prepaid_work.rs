...
fn fixture(start: bool) -> Vec<u8> {
    ...
    section(
        ...
        7,
        &[
            6, 1, b'h', 0, 0, 1, b'f', 0, 1, 1, b'i', 0, 2, 1, b't', 0, 3, 1, b'u', 0, 4, 1, b'm',
            2, 0,
        ],
    );
    if start {
...
fn spend_unreserved(pool: &WasmWorkPool) -> u64 {
    ...
    let mut other = vm
        .instantiate_with_imports(imports(pool, |_, _| panic!("unused import")))
        .unwrap();
    for _ in 0..before {
        other.call_export("u", &[]).unwrap();
    }
...
fn exact_prepaid_accesses_do_not_double_charge_or_require_spare_fuel() {
    ...
    let registry = imports(&pool, |caller, _| {
        caller.with_prepaid_work(4, |caller| {
...
fn competing_instance_cannot_steal_work_between_completed_buffer_writes() {
    ...
    let registry = imports(&pool, move |caller, _| {
        caller.with_prepaid_work(2, |caller| {
            caller.write_memory(0, b"A")?;
    ...
    assert!(matches!(
        host_error(instance.call_export("u", &[])),
...
fn scope_overrun_cannot_spill_into_available_parent_or_shared_balance() {
    ...
    let registry = imports(&pool, |caller, _| {
        caller.with_prepaid_work(4, |caller| {
            let refusal: Result<(), WasmNumericVmError> =
                caller.with_prepaid_work(5, |_| panic!("overdrawn scope entered"));
            assert_eq!(
                host_error(refusal),
                WasmHostError::PrepaidWorkExceeded {
                    requested: 5,
                    remaining: 4
                }
            );
    ...
    let mut instance = vm.instantiate_with_imports(registry).unwrap();
    assert_eq!(
        host_error(instance.call_export("h", &[])),
        WasmHostError::PrepaidWorkExceeded {
            requested: 5,
            remaining: 4
        }
...
fn unused_credit_is_not_available_to_later_invocations() {
    ...
    let pool = WasmWorkPool::new(5);
    let registry = imports(&pool, |caller, _| {
        caller.with_prepaid_work(4, |_| Ok(vec![]))
    });
    let mut instance = vm.instantiate_with_imports(registry).unwrap();
    assert_eq!(
        instance
...
fn zero_scope_is_not_unlimited_and_maximum_reservation_cannot_wrap() {
    ...
    for units in [0, u64::MAX] {
        ...
        let registry = imports(&pool, move |caller, _| {
            caller.with_prepaid_work(units, |caller| {
        ...
        if units == 0 {
            ...
            assert!(matches!(
                error,
                WasmNumericVmError::InstructionBudgetExceeded { max: u64::MAX }
            ));
        }
...
fn live_cancellation_and_service_revocation_still_stop_prepaid_access() {
    for scope in 0..3 {
        ...
        let mut registry = imports(&pool, move |caller, _| {
            caller.with_prepaid_work(4, |caller| {
                caller.write_memory(0, b"a")?;
                signal.cancel();
        ...
        match scope {
            ...
            1 => registry
                .bind_execution_cancellation(token, "prepaid")
                .unwrap(),
            _ => registry
                .bind_capability_revocation(RuntimeCapability::Builtin, token, "prepaid")
                .unwrap(),
        }
        let mut instance = vm.instantiate_with_imports(registry).unwrap();
...
fn caught_scope_unwind_is_latched_even_after_accounting_is_restored() {
    ...
    let registry = imports(&pool, |caller, _| {
        ...
        assert_eq!(
            host_error(caller.write_memory(1, b"bad")),
            WasmHostError::PrepaidWorkInterrupted
        );
        Ok(vec![])
    });
    let mut instance = vm.instantiate_with_imports(registry).unwrap();
...
fn recording_and_replay_charge_the_complete_reservation_including_unused_credit() {
    ...
    let mut registry = imports(&pool, |caller, _| {
        caller.with_prepaid_work(4, |caller| {
            caller.write_memory(0, b"a")?;
            caller.write_memory(1, b"b")?;
            Ok(vec![])
...
fn failed_recorded_operation_keeps_debit_and_replays_only_its_completed_effects() {
    ...
    let mut registry = imports(&WasmWorkPool::new(1029), |_, _| {
        panic!("replay entered failed provider")
    });
    let replay = registry
        .replay_calls(
...
fn imported_start_direct_indirect_and_tail_calls_use_the_same_prepaid_boundary() {
    for export in ["h", "f", "i", "t"] {
        ...
        let mut reference = vm
            .instantiate_with_imports(imports(&WasmWorkPool::new(100), |caller, _| {
    ...
    let instance = vm
        .instantiate_with_imports(imports(&pool, |caller, _| {
            caller.with_prepaid_work(2, |caller| {
                caller.write_memory(0, b"a")?;
                caller.write_memory(1, b"b")?;
                Ok(vec![])
...
fn ordinary_provider_error_restores_outer_accounting_without_refund() {
    ...
    impl From<WasmNumericVmError> for Failure {
    ...
    let registry = imports(&pool, |caller, _| {
        ...
        match outcome {
            Err(Failure::Errno) => {}
            Err(Failure::Vm(error)) => return Err(error),
            Ok(()) => panic!("lost errno"),
        }
        assert_eq!(caller.remaining_work(), 5);
        caller.write_memory(0, b"a")?;

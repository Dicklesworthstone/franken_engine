...
use frankenengine_engine::wasm_runtime_lane::host_replay::{
    WasmHostTraceError, WasmHostTraceLimits, WasmHostTranscript,
};
...
fn fixture(start: bool, trap_after_call: bool) -> Vec<u8> {
    ...
    section(
        ...
        7,
        &[
            6, 1, b'f', 0, 1, 1, b'i', 0, 2, 1, b'h', 0, 0, 3, b's', b'e', b't', 0, 3, 1, b'm', 2,
            0, 1, b't', 1, 0,
        ],
    );
    if start {
        section(&mut bytes, 8, &[4]);
    }
    ...
    if start {
        functions.push(vec![0, 0x41, 0, 0x41, 4, 0x10, 0, 0x1a, 0x0b]);
    }
...
where
    F: FnMut(&mut WasmHostCaller<'_, '_>, &[WasmBoundaryValue]) -> Outcome + Send + Sync + 'static,
{
    ...
    imports
        .define(
            ...
            [RuntimeCapability::Builtin].into_iter().collect(),
            3,
            callback,
        )
        .unwrap();
...
fn transform(caller: &mut WasmHostCaller<'_, '_>, arguments: &[WasmBoundaryValue]) -> Outcome {
    let [I32(address), I32(length)] = arguments else {
        panic!("checked ABI");
    };
    let input = caller
...
fn records_real_buffers_and_replays_without_calling_the_provider() {
    for export in ["f", "i", "h"] {
        ...
        if export == "f" {
            assert_eq!(expected.instructions_executed, 1041);
        }
        ...
        let decoded =
            WasmHostTranscript::from_json(&bytes, WasmHostTraceLimits::default()).unwrap();
        assert_eq!(decoded, transcript);
        let mut bindings = imports(forbidden);
        let replay = bindings
            .replay_calls(decoded, WasmHostTraceLimits::default())
            .unwrap();
        assert_eq!(
            replay.verify_complete(),
            Err(WasmHostTraceError::Incomplete { remaining: 1 })
        );
        let mut reproduced = vm.instantiate_with_imports(bindings).unwrap();
        assert_eq!(
            reproduced.call_export(export, &[I32(0), I32(4)]).unwrap(),
            expected
        );
...
fn ordered_overlapping_writes_and_stateful_host_results_replay_exactly() {
    ...
    assert_eq!(
        &live.memory_export("m").unwrap()[8..12],
        &[0xff, 0xaa, 0xbb, 0xfe]
    );
    let mut bindings = imports(forbidden);
    let replay = bindings
    ...
    let mut reproduced = vm.instantiate_with_imports(bindings).unwrap();
    assert_eq!(
        reproduced.call_export("f", &[I32(0), I32(0)]).unwrap(),
        first
    );
    assert_eq!(
        replay.verify_complete(),
...
fn startup_and_later_invocations_share_one_trace_without_rerunning_start() {
    ...
    assert_eq!(
        reproduced.call_export("f", &[I32(0), I32(4)]).unwrap(),
        expected
    );
    replay.verify_complete().unwrap();
}
...
fn provider_budget_faults_are_replayed_with_preceding_writes_retained() {
    ...
    let mut reproduced = vm.instantiate_with_imports(bindings).unwrap();
    assert_eq!(
        reproduced.call_export("f", &[I32(0), I32(0)]).unwrap_err(),
        error
    );
    assert_eq!(reproduced.memory_export("m"), live.memory_export("m"));
    replay.verify_complete().unwrap();
...
fn host_result_abi_errors_and_later_guest_traps_preserve_completed_effects() {
    for bad in [
        vec![],
        vec![WasmBoundaryValue::I64(1)],
        vec![I32(1), I32(2)],
    ] {
        let vm = vm(false, false);
        let mut bindings = imports(move |caller, _| {
            caller.write_memory(8, &[99])?;
            Ok(bad.clone())
        });
        let recording = bindings
            .record_calls(WasmHostTraceLimits::default())
    ...
    let mut reproduced = vm.instantiate_with_imports(bindings).unwrap();
    assert_eq!(
        reproduced.call_export("f", &[I32(0), I32(4)]).unwrap_err(),
        expected
    );
    assert_eq!(reproduced.memory_export("m"), live.memory_export("m"));
}
...
fn one_call(vm: &WasmNumericVm) -> WasmHostTranscript {
    ...
    let recording = bindings
        .record_calls(WasmHostTraceLimits::default())
        .unwrap();
    vm.instantiate_with_imports(bindings)
        .unwrap()
        .call_export("f", &[I32(0), I32(4)])
        .unwrap();
    recording.snapshot().unwrap()
}
...
fn different_arguments_memory_or_call_path_fail_before_replay_mutation() {
    ...
    for case in 0..3 {
        let mut bindings = imports(forbidden);
        let replay = bindings
            .replay_calls(transcript.clone(), WasmHostTraceLimits::default())
            .unwrap();
        let mut instance = vm.instantiate_with_imports(bindings).unwrap();
        if case == 1 {
            instance.call_export("set", &[I32(0), I32(100)]).unwrap();
        }
        ...
        assert_eq!(
            trace_error(instance.call_export(name, &args).unwrap_err()),
            WasmHostTraceError::Diverged { call: 0 }
        );
        assert_eq!(instance.memory_export("m").unwrap(), before);
        assert_eq!(
            replay.verify_complete(),
            Err(WasmHostTraceError::Unavailable)
        );
        // A divergence is sticky; retry cannot cherry-pick a matching suffix.
        assert_eq!(
            trace_error(instance.call_export("f", &[I32(0), I32(4)]).unwrap_err()),
            WasmHostTraceError::Unavailable
        );
    }
}
...
fn changed_binding_contract_or_vm_limits_cannot_reinterpret_a_trace() {
    ...
    for cost in [2, 4] {
        ...
        assert!(matches!(
            trace_error(instance.call_export("f", &[I32(0), I32(4)]).unwrap_err()),
            WasmHostTraceError::Diverged { .. }
        ));
    }
    let altered = WasmNumericVm::parse(
        &fixture(false, false),
        WasmNumericLimits {
    ...
    assert!(matches!(
        trace_error(
            altered
                .instantiate_with_imports(bindings)
                .unwrap()
                .call_export("f", &[I32(0), I32(4)])
                .unwrap_err()
        ),
...
fn replay_does_not_restore_revoked_capabilities_or_consume_denied_calls() {
    ...
    let mut bindings = imports(forbidden);
    let replay = bindings
        .replay_calls(one_call(&vm), WasmHostTraceLimits::default())
        .unwrap();
    let mut instance = vm.instantiate_with_imports(bindings).unwrap();
    assert!(instance.revoke_host_capability(RuntimeCapability::Builtin));
    assert!(matches!(
        instance.call_export("f", &[I32(0), I32(4)]),
    ...
    assert!(
        instance.memory_export("m").unwrap()[8..12]
            .iter()
            .all(|byte| *byte == 0)
    );
}
...
fn exhausted_trace_never_falls_back_to_native_execution() {
    ...
    assert_eq!(
        trace_error(instance.call_export("f", &[I32(0), I32(4)]).unwrap_err()),
        WasmHostTraceError::Exhausted { call: 1 }
    );
    assert_eq!(instance.memory_export("m").unwrap(), before);
    assert_eq!(
        replay.verify_complete(),
...
fn recording_quotas_refuse_before_callbacks_or_before_the_unrecorded_write() {
    ...
    let mut bindings = imports(move |_, _| {
        observed.fetch_add(1, Ordering::SeqCst);
        Ok(vec![I32(1)])
    });
    let recording = bindings
        .record_calls(WasmHostTraceLimits {
            max_calls: 0,
            ..WasmHostTraceLimits::default()
        })
        .unwrap();
    let mut instance = vm.instantiate_with_imports(bindings).unwrap();
    assert_eq!(
        trace_error(instance.call_export("f", &[I32(0), I32(0)]).unwrap_err()),
        WasmHostTraceError::LimitExceeded
    );
    assert_eq!(calls.load(Ordering::SeqCst), 0);
    assert_eq!(
        recording.snapshot().unwrap_err(),
        WasmHostTraceError::Unavailable
    );
    let mut bindings = imports(|caller, _| {
...
fn hashing_work_is_precharged_and_default_execution_metrics_are_unchanged() {
    ...
    let bounded = WasmNumericVm::parse(
        &fixture(false, false),
        WasmNumericLimits {
...
fn corrupted_truncated_unknown_schema_and_oversized_transcripts_are_rejected() {
    let vm = vm(false, false);
    let original = one_call(&vm)
        .to_json(WasmHostTraceLimits::default())
        .unwrap();
...
fn inspecting_inside_a_callback_does_not_deadlock_and_panics_poison_recording() {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        instance.call_export("f", &[I32(0), I32(0)])
    }));
...
fn configuration_cannot_replace_an_existing_recording_or_replay() {
    ...
    assert_eq!(
        bindings
            .record_calls(WasmHostTraceLimits::default())
            .unwrap_err(),
        WasmHostTraceError::AlreadyConfigured
    );
    assert_eq!(
        bindings
            .replay_calls(recorder.snapshot().unwrap(), WasmHostTraceLimits::default())
            .unwrap_err(),
        WasmHostTraceError::AlreadyConfigured
    );
    let mut bindings = imports(forbidden);
...
fn resolved_context() -> ResolutionContext {
    ResolutionContext::new(
        "trace-resolved-replay",
        "decision-resolved-replay",
        "policy-current",
    )
}
...
fn resolved(bytes: &[u8], path: &str, origin: &str, extra_capability: bool) -> WasmNativeModule {
    ...
    let mut definition = ModuleDefinition::wasm_binary(bytes, &WasmNumericLimits::default())
        .unwrap()
        .with_provenance(origin)
        .require_capability(RuntimeCapability::Builtin);
    if extra_capability {
        definition = definition.require_capability(RuntimeCapability::FsRead);
    }
    resolver
        .register_workspace_module(path, definition)
        .unwrap();
    resolver
        .load_wasm(
            &ModuleRequest::new(path, ImportStyle::Import),
            &resolved_context(),
            &resolved_policy(),
            WasmNumericLimits::default(),
        )
...
fn resolved_recording(module: &WasmNativeModule) -> WasmHostTranscript {
    ...
    let mut instance = module
        .instantiate_with_imports(&resolved_context(), &resolved_policy(), bindings)
        .unwrap();
    instance
        .call_export(
            "f",
...
fn resolved_recordings_pin_the_module_before_start_and_replay_real_state() {
    ...
    let mut live = module
        .instantiate_with_imports(&ctx, &policy, bindings)
        .unwrap();
    assert_eq!(recording.snapshot().unwrap().call_count(), 1);
    assert_eq!(
        recording.snapshot().unwrap().module_hash(),
        Some(module.resolution().module.content_hash)
    );
    let expected = live
        .call_export("f", &[I32(0), I32(4)], &ctx, &policy)
        .unwrap();
    let transcript = recording.snapshot().unwrap();
    ...
    let replay = bindings
        .replay_calls(decoded, WasmHostTraceLimits::default())
        .unwrap();
    let mut reproduced = module
        .instantiate_with_imports(&ctx, &policy, bindings)
        .unwrap();
    assert_eq!(
        reproduced.start_execution(&ctx, &policy).unwrap(),
        live.start_execution(&ctx, &policy).unwrap()
    );
    assert_eq!(
        reproduced
            .call_export("f", &[I32(0), I32(4)], &ctx, &policy)
    ...
    assert_eq!(
        reproduced.memory_export("m", &ctx, &policy).unwrap(),
        live.memory_export("m", &ctx, &policy).unwrap()
    );
    replay.verify_complete().unwrap();
}
...
fn matching_host_calls_cannot_replay_against_changed_source_name_or_provenance() {
    ...
    for (bytes, path, origin, extra) in [
        ...
        assert_ne!(
            changed.resolution().module.content_hash,
            module.resolution().module.content_hash
        );
        let mut bindings = imports(forbidden);
        let replay = bindings
            .replay_calls(transcript.clone(), WasmHostTraceLimits::default())
...
fn scoped_transcripts_cannot_escape_through_the_raw_numeric_vm() {
    ...
    let mut bindings = imports(forbidden);
    let replay = bindings
        .replay_calls(resolved_recording(&module), WasmHostTraceLimits::default())
        .unwrap();
    let vm = WasmNumericVm::parse(&bytes, WasmNumericLimits::default()).unwrap();
    assert_eq!(
        trace_error(vm.instantiate_with_imports(bindings).unwrap_err()),
        WasmHostTraceError::ModuleMismatch
    );
    assert_eq!(
        replay.verify_complete(),
...
fn scoped_empty_prefixes_require_identity_even_without_any_host_call() {
    ...
    assert_eq!(
        replay.verify_complete(),
        Err(WasmHostTraceError::ModuleMismatch)
    );
    module
        .instantiate_with_imports(&resolved_context(), &resolved_policy(), bindings)
        .unwrap();
    ...
    assert_eq!(
        trace_error(raw.instantiate_with_imports(bindings).unwrap_err()),
        WasmHostTraceError::ModuleMismatch
    );
}
...
fn current_policy_denials_and_permanent_host_revocation_still_gate_scoped_replay() {
    ...
    let before = instance
        .memory_export("m", &ctx, &policy)
        .unwrap()
        .unwrap()
        .to_vec();
    for rejected in [&denied, &denied_specifier] {
        assert!(
            matches!(instance.call_export("f", &[I32(0), I32(4)], &ctx, rejected),
        ...
        assert_eq!(
            instance.memory_export("m", &ctx, &policy).unwrap().unwrap(),
            before
        );
    }
    ...
    assert!(instance.revoke_host_capability(RuntimeCapability::Builtin));
    assert!(matches!(
        instance.call_export("f", &[I32(0), I32(4)], &ctx, &policy),
...
fn registry_updates_do_not_rebind_an_already_loaded_replay_identity() {
    let bytes = fixture(false, false);
    let mut resolver = DeterministicModuleResolver::new("/app");
    let definition = ModuleDefinition::wasm_binary(&bytes, &WasmNumericLimits::default())
        .unwrap()
        .with_provenance("release-a")
        .require_capability(RuntimeCapability::Builtin);
    resolver
        .register_workspace_module("/app/replay.wasm", definition.clone())
        .unwrap();
    let request = ModuleRequest::new("/app/replay.wasm", ImportStyle::Import);
    let ctx = resolved_context();
    let policy = resolved_policy();
    let old = resolver
        .load_wasm(&request, &ctx, &policy, WasmNumericLimits::default())
        .unwrap();
    let transcript = resolved_recording(&old);
    resolver
        .register_workspace_module("/app/replay.wasm", definition.with_provenance("release-b"))
        .unwrap();
    let new = resolver
        .load_wasm(&request, &ctx, &policy, WasmNumericLimits::default())
        .unwrap();
    ...
    assert_eq!(
        scope_error(
            new.instantiate_with_imports(&ctx, &policy, bindings)
                .unwrap_err()
        ),
        WasmHostTraceError::ModuleMismatch
    );
    ...
    let replay = bindings
        .replay_calls(transcript, WasmHostTraceLimits::default())
        .unwrap();
    old.instantiate_with_imports(&ctx, &policy, bindings)
        .unwrap()
        .call_export("f", &[I32(0), I32(4)], &ctx, &policy)
        .unwrap();
    replay.verify_complete().unwrap();
...
fn aliases_share_canonical_identity_but_their_current_denials_still_apply() {
    ...
    resolver
        .register_workspace_module(
            "/app/main.mjs",
            ModuleDefinition::new(ModuleSyntax::EsModule, "import './replay.wasm';"),
        )
        .unwrap();
    resolver
        .register_workspace_module(
            "/app/replay.wasm",
            ModuleDefinition::wasm_binary(&fixture(false, false), &WasmNumericLimits::default())
                .unwrap()
                .require_capability(RuntimeCapability::Builtin),
        )
        .unwrap();
    ...
    let direct = resolver
        .load_wasm(
            &ModuleRequest::new("/app/replay.wasm", ImportStyle::Import),
            &ctx,
            &policy,
            WasmNumericLimits::default(),
        )
    ...
    let alias = resolver
        .load_wasm(
            &ModuleRequest::new("./replay.wasm", ImportStyle::Import)
        ...
        .unwrap();
    assert_eq!(
        direct.resolution().module.content_hash,
        alias.resolution().module.content_hash
    );
    ...
    let replay = bindings
        .replay_calls(transcript, WasmHostTraceLimits::default())
        .unwrap();
    let mut instance = alias
        .instantiate_with_imports(&ctx, &policy, bindings)
        .unwrap();
    assert!(
        matches!(instance.call_export("f", &[I32(0), I32(4)], &ctx, &policy.clone().deny_specifier("./replay.wasm")),
        Err(WasmNativeLoadError::Resolution(error)) if error.code == ResolutionErrorCode::PolicyDenied)
    );
    assert_eq!(
        replay.verify_complete(),
...
fn transcript_scope_is_explicit_versioned_and_part_of_the_content_digest() {
    ...
    for change in 0..4 {
        ...
        match change {
            0 => {
                wire["data"].as_object_mut().unwrap().remove("scope");
            }
            1 => {
                wire["data"]["scope"] = serde_json::json!("Unscoped");
            }
            2 => {
                wire["data"]["scope"] = serde_json::json!({"Resolved": vec![0_u8; 32]});
            }
            3 => {
                wire["data"]["version"] = serde_json::json!(1);
            }
            _ => unreachable!(),
        }
        assert_eq!(

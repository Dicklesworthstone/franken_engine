...
fn slots(value: usize) -> NonZeroUsize {
    NonZeroUsize::new(value).unwrap()
}
fn context() -> ResolutionContext {
    ResolutionContext::new("schedule-trace", "schedule-decision", "schedule-policy")
}
...
fn fixture(host: bool) -> Vec<u8> {
    ...
    let bodies = [
        vec![
            0, 0x02, 0x40, 0x03, 0x40, 0x20, 0, 0x45, 0x0d, 1, 0x23, 0, 0x41, 1, 0x6a, 0x24, 0,
        ...
        if host {
            vec![0, 0x20, 0, 0x10, 0, 0x41, 1, 0x6a, 0x0b]
        } else {
            vec![0, 0x20, 0, 0x41, 1, 0x6a, 0x0b]
        },
    ];
...
fn load(host: bool, limits: WasmNumericLimits) -> WasmNativeModule {
    ...
    if host {
        definition = definition.require_capability(RuntimeCapability::Builtin);
    }
    resolver
        .register_workspace_module("/app/task.wasm", definition)
        .unwrap();
    resolver
        .load_wasm(
            &ModuleRequest::new("/app/task.wasm", ImportStyle::Import),
            &context(),
            &policy(host),
            limits,
        )
...
{
    let mut imports =
        WasmHostImports::new([RuntimeCapability::VmDispatch, RuntimeCapability::Builtin].into());
    imports
        .define(
            "h",
            "f",
...
fn three_real_instances_run_in_fifo_turns_and_match_unsliced_results_and_meters() {
    ...
    let handles = [
        scheduler
            .submit(a.begin_call("run", &[I32(1)], &ctx, &allow).unwrap())
            .unwrap(),
        scheduler
            .submit(b.begin_call("run", &[I32(2)], &ctx, &allow).unwrap())
            .unwrap(),
        scheduler
            .submit(c.begin_call("run", &[I32(3)], &ctx, &allow).unwrap())
            .unwrap(),
    ];
    for round in 0..2 {
    ...
    for _ in 0..500 {
        let Some(turn) = scheduler.run_next(work(3), &ctx, &allow) else {
            break;
        };
        match turn.outcome {
            ...
            WasmTaskOutcome::Complete(execution) => {
                let index = handles
                    .iter()
                    .position(|handle| handle.id() == turn.task_id)
                    .unwrap();
                let expected = module
                    .instantiate(&ctx, &allow)
                    .unwrap()
                    .call_export("run", &[I32(index as i32 + 1)], &ctx, &allow)
                    .unwrap();
...
fn an_infinite_guest_cannot_starve_a_useful_task() {
    ...
    let spin = scheduler
        .submit(spinner.begin_call("spin", &[I32(0)], &ctx, &allow).unwrap())
        .unwrap();
    let finite = scheduler
        .submit(useful.begin_call("run", &[I32(1)], &ctx, &allow).unwrap())
        .unwrap();
    let mut finished = false;
    for _ in 0..64 {
        let turn = scheduler.run_next(work(4), &ctx, &allow).unwrap();
        if let WasmTaskOutcome::Complete(result) = turn.outcome {
            assert_eq!(turn.task_id, finite.id());
            assert_eq!(result.results, [I32(1)]);
            finished = true;
            break;
        }
        assert!(matches!(turn.outcome, WasmTaskOutcome::Pending));
...
fn full_queue_returns_unexecuted_work_and_reuses_capacity_without_reusing_identity() {
    ...
    let old = scheduler
        .submit(a.begin_call("run", &[I32(0)], &ctx, &allow).unwrap())
        .unwrap();
    let error = scheduler
        .submit(b.begin_call("run", &[I32(2)], &ctx, &allow).unwrap())
        .unwrap_err();
    ...
    assert!(matches!(
        scheduler.run_next(work(100), &ctx, &allow).unwrap().outcome,
...
fn current_policy_is_rechecked_on_each_turn_and_denial_does_not_block_other_tasks() {
    ...
    let denied = scheduler
        .submit(a.begin_call("run", &[I32(3)], &ctx, &allow).unwrap())
        .unwrap();
    assert!(matches!(
        scheduler.run_next(work(9), &ctx, &allow).unwrap().outcome,
        WasmTaskOutcome::Pending
    ));
    let healthy = scheduler
        .submit(b.begin_call("run", &[I32(1)], &ctx, &allow).unwrap())
        .unwrap();
...
fn guest_faults_retire_only_the_faulting_task_and_preserve_its_completed_writes() {
    ...
    let mut scheduler = WasmNativeScheduler::new(slots(2));
    scheduler
        .submit(a.begin_call("trap", &[I32(0)], &ctx, &allow).unwrap())
        .unwrap();
    scheduler
        .submit(b.begin_call("run", &[I32(1)], &ctx, &allow).unwrap())
        .unwrap();
    ...
    drop(scheduler);
    assert_eq!(
        &a.memory_export("m", &ctx, &allow).unwrap().unwrap()[..4],
        &7_i32.to_le_bytes()
    );
    assert_eq!(
        a.call_export("run", &[I32(1)], &ctx, &allow)
            .unwrap()
            .results,
        [I32(1)]
    );
...
fn a_new_slice_does_not_replenish_the_vm_hard_invocation_budget() {
    ...
    scheduler
        .submit(
            instance
                .begin_call("run", &[I32(100)], &ctx, &allow)
                .unwrap(),
        )
        .unwrap();
    ...
    for _ in 0..30 {
        ...
        match turn.outcome {
            WasmTaskOutcome::Pending => {}
            WasmTaskOutcome::Failed(WasmNativeLoadError::Execution(
                WasmNumericVmError::InstructionBudgetExceeded { max: 20 },
            )) => {
                failed = true;
                break;
            }
...
fn yielding_never_repeats_a_host_effect_and_policy_denial_precedes_pending_callbacks() {
    ...
    scheduler
        .submit(
            instance
                .begin_call("host", &[I32(41)], &ctx, &allow)
                .unwrap(),
        )
        .unwrap();
    ...
    for _ in 0..32 {
        let turn = scheduler.run_next(work(1), &ctx, &allow).unwrap();
        if let WasmTaskOutcome::Complete(result) = turn.outcome {
            assert_eq!(result.results, [I32(83)]);
            complete = true;
            break;
        }
        assert!(matches!(turn.outcome, WasmTaskOutcome::Pending));
    ...
    scheduler
        ...
        .unwrap();
    assert!(matches!(
        scheduler.run_next(work(1), &ctx, &allow).unwrap().outcome,
        WasmTaskOutcome::Pending
    ));
    assert!(matches!(
        scheduler
            .run_next(work(100), &ctx, &policy(false))
            .unwrap()
            .outcome,
        WasmTaskOutcome::Failed(WasmNativeLoadError::Resolution(_))
    ));
...
fn task_cancellation_is_scoped_and_discards_results_but_not_completed_effects() {
    ...
    let handle = scheduler
        .submit(
            instance
                .begin_call("host", &[I32(0)], &ctx, &allow)
                .unwrap(),
        )
        .unwrap();
    *handle_slot.lock().unwrap() = Some(handle);
    ...
    drop(scheduler);
    assert_eq!(
        &instance.memory_export("m", &ctx, &allow).unwrap().unwrap()[8..12],
        &[1, 2, 3, 4]
    );
    // Cancellation targeted this invocation, not every future use of its instance.
    assert_eq!(
        instance
            .call_export("run", &[I32(1)], &ctx, &allow)
            .unwrap()
            .results,
        [I32(1)]
    );
...
fn cancellation_before_dispatch_never_executes_even_the_first_guest_store() {
    ...
    let handle = scheduler
        .submit(
            instance
                .begin_call("trap", &[I32(0)], &ctx, &allow)
                .unwrap(),
        )
        .unwrap();
    handle.cancel();
    ...
    drop(scheduler);
    assert_eq!(
        &instance.memory_export("m", &ctx, &allow).unwrap().unwrap()[..4],
        &[0; 4]
    );
}
...
fn withdrawal_transfers_the_live_continuation_without_resetting_its_meter() {
    ...
    let mut scheduler = WasmNativeScheduler::new(slots(1));
    let first = scheduler
        .submit(instance.begin_call("run", &[I32(3)], &ctx, &allow).unwrap())
        .unwrap();
    ...
    assert_eq!(
        ...
        first_turn.event.work_after.unwrap()
    );
    assert!(scheduler.is_empty());
    ...
    match scheduler.run_next(work(100), &ctx, &allow).unwrap().outcome {
        WasmTaskOutcome::Complete(result) => {
            let expected = module
                .instantiate(&ctx, &allow)
                .unwrap()
                .call_export("run", &[I32(3)], &ctx, &allow)
                .unwrap();
...
fn dropping_a_scheduler_releases_instances_without_undoing_prior_slices() {
    ...
    assert_eq!(
        instance.global_export("g", &ctx, &allow).unwrap(),
        Some(&I32(1))
    );
    assert_eq!(
        instance

...
fn revoked() -> WasmNumericVmError {
    WasmHostError::MemoryPoolRevoked.into()
}
fn work(n: u64) -> NonZeroU64 {
...
fn name(out: &mut Vec<u8>, value: &str) {
    out.extend(leb(value.len() as u32));
...
fn program(host: bool, start: bool, memory: Option<(u32, u32)>) -> Vec<u8> {
    ...
    if host {
        name(&mut exports, "host");
        exports.extend([0, 0]);
    }
    section(&mut out, 7, &exports);
    if start {
        section(&mut out, 8, &[base]);
    }
    ...
    let bodies = [
        ...
        if memory.is_some() {
            vec![0, 0x20, 0, 0x40, 0, 0x0b]
        } else {
            vec![0, 0x41, 0x7f, 0x0b]
        },
...
fn revocation_is_permanent_inherited_and_cannot_refund_or_widen_capacity() {
    ...
    assert!(matches!(
        grandchild.partition(0),
        Err(WasmHostError::MemoryPoolRevoked)
    ));
    assert_eq!(root.reserved_pages(), 8);
...
fn nesting_is_bounded_before_reservation_even_for_zero_page_partitions() {
    ...
    for _ in 0..WASM_MEMORY_POOL_MAX_DEPTH {
        deepest = deepest.partition(1).unwrap();
    }
...
fn revoked_admission_precedes_allocation_and_includes_memoryless_modules() {
    ...
    for memory in [None, Some((0, 0)), Some((1, 2))] {
        ...
        .unwrap();
        assert!(
            matches!(vm.instantiate_with_imports(imports(&root)), Err(error) if error == revoked())
        );
        assert_eq!(root.reserved_pages(), 0);
    }
    let mut bindings = imports(&root);
    assert_eq!(
        bindings.bind_memory_pool(WasmMemoryPool::new(10)),
...
fn existing_guest_state_stays_inspectable_and_charged_but_cannot_execute() {
    ...
    assert_eq!(
        instance.call_export("grow", &[I32(1)]).unwrap_err(),
        revoked()
    );
...
fn child_revocation_stops_existing_descendants_not_ancestors_or_siblings() {
    ...
    parent_guest.call_export("run", &[]).unwrap();
    b_guest.call_export("run", &[]).unwrap();
    root.revoke();
...
fn prior_budget_and_buffer_faults_keep_precedence_over_later_subtree_revocation() {
    ...
    for budget in [false, true] {
        ...
        let bindings = provider(&pool, move |caller, _| {
            let first = if budget {
                caller.charge_work(u64::MAX).unwrap_err()
            } else {
                caller.write_memory(u32::MAX, b"bad").unwrap_err()
            };
            signal.revoke();
...
fn explicit_execution_cancellation_and_established_exit_keep_their_precedence() {
    ...
    bindings
        .bind_execution_cancellation(token.clone(), "cancel-first")
        .unwrap();
    let instance = WasmNumericVm::parse(
        &program(false, false, Some((1, 2))),
        WasmNumericLimits::default(),
    )
    .unwrap();
    ...
    let signal = pool.clone();
    let mut guest = vm
        .instantiate_with_imports(provider(&pool, move |caller, _| {
            let exit = caller.exit(u32::MAX);
            signal.revoke();
            Err(exit)
        }))
        .unwrap();
    assert_eq!(
        guest.call_export("host", &[]).unwrap_err(),
        WasmHostError::ProcessExit { code: u32::MAX }.into()
    );
...
fn cooperative_start_cleans_up_on_revocation_and_never_repeats_effects() {
    ...
    let root = WasmMemoryPool::new(2);
    let child = root.partition(2).unwrap();
    ...
    assert_eq!(child.reserved_pages(), 0);
    assert_eq!(root.reserved_pages(), 2);
    assert_eq!(wake.0.load(Ordering::SeqCst), 1);
}
...
fn a_suspended_call_retains_writes_and_lease_without_entering_pending_host_work() {
    ...
    let mut instance = vm
        .instantiate_with_imports(provider(&pool, move |_, _| {
            calls.fetch_add(1, Ordering::SeqCst);
            Ok(vec![])
        }))
        .unwrap();
    let call = instance.begin_call("run", &[]).unwrap();
    let WasmCallStep::Pending(call) = call.resume(work(3)).unwrap() else {
        panic!("expected yield before host");
    };
    pool.revoke();
    assert!(matches!(call.resume(work(100)), Err(error) if error == revoked()));
    assert_eq!(seen.load(Ordering::SeqCst), 0);
    assert_eq!(
        &instance.memory_export("memory").unwrap()[..4],
        &7_i32.to_le_bytes()
    );
    assert_eq!(pool.reserved_pages(), 2);
    drop(instance);
...
fn another_thread_can_stop_a_hostless_guest_through_its_ancestor() {
    ...
    let worker_ready = barrier.clone();
...
fn a_successful_tape_cannot_supply_authority_to_a_revoked_live_pool() {
    ...
    let mut bindings = provider(&original, |caller, _| {
        caller.write_memory(0, b"tape")?;
        Ok(vec![])
    });
    ...
    let replay = bindings
        .replay_calls(observer.snapshot().unwrap(), limits)
        .unwrap();
...
fn recorded_revocation_replays_its_refusal_without_revoking_an_unrelated_live_pool() {
    ...
    assert_eq!(guest.call_export("host", &[]).unwrap_err(), revoked());
    let pool = WasmMemoryPool::new(2);
    let mut bindings = provider(&pool, |_, _| {
        panic!("a tape is not a live control-plane command")
    });
    let replay = bindings
        .replay_calls(observer.snapshot().unwrap(), limits)
        .unwrap();
...
mod interrupted_hosts {
    ...
    use frankenengine_engine::wasm_runtime_lane::scheduler::{
        WasmNativeScheduler, WasmTaskOutcome,
    };
    use frankenengine_engine::wasm_runtime_lane::{WasmNativeLoadError, WasmNativeModule};
    use std::num::NonZeroUsize;
    use std::panic::{AssertUnwindSafe, catch_unwind, panic_any};

    fn interrupted() -> WasmNumericVmError {
        WasmHostError::HostCallInterrupted.into()
    }
    fn context() -> ResolutionContext {
        ResolutionContext::new("unwind", "contain", "current")
    }
    ...
    fn module(host: bool, start: bool) -> WasmNativeModule {
        ...
        let definition =
            ModuleDefinition::wasm_binary(&program(host, start, Some((1, 2))), &limits)
                .unwrap()
                .require_capability(Builtin);
        resolver
            .register_workspace_module("/app/guest.wasm", definition)
            .unwrap();
        resolver
            .load_wasm(
                &ModuleRequest::new("/app/guest.wasm", ImportStyle::Import),
                &context(),
                &policy(),
                limits,
            )
    ...
    fn caught_native_panics_keep_payload_and_retire_every_reentry_surface() {
        ...
        for export in ["run", "indirect", "tail", "host"] {
            ...
            let mut instance = vm
                ...
                .unwrap();
            let payload =
                catch_unwind(AssertUnwindSafe(|| instance.call_export(export, &[]))).unwrap_err();
    ...
    fn interrupted_instance_does_not_revoke_siblings_or_release_live_memory() {
        ...
        let mut broken = vm
            .instantiate_with_imports(provider(&tenant, |caller, _| {
                caller.write_memory(0, b"kept")?;
                panic!("provider failed");
            }))
            .unwrap();
        let mut healthy = vm
            .instantiate_with_imports(provider(&tenant, |_, _| Ok(vec![])))
            .unwrap();
        ...
        assert_eq!(
            &healthy.memory_export("memory").unwrap()[4..8],
            &9_i32.to_le_bytes()
        );
        assert_eq!(broken.call_export("run", &[]).unwrap_err(), interrupted());
        drop(broken);
        assert_eq!(tenant.reserved_pages(), 2);
        let replacement = vm
            .instantiate_with_imports(provider(&tenant, |_, _| Ok(vec![])))
            .unwrap();
        drop(healthy);
        drop(replacement);
        assert_eq!(tenant.reserved_pages(), 0);
        assert_eq!(root.reserved_pages(), 4);
        drop(tenant);
        assert_eq!(root.reserved_pages(), 0);
    }
    ...
    fn trusted_provider_internal_recovery_is_not_engine_boundary_interruption() {
        ...
        let pool = WasmMemoryPool::new(2);
        let mut instance = vm
            .instantiate_with_imports(provider(&pool, |caller, _| {
                assert!(catch_unwind(|| panic!("caught inside trusted provider")).is_err());
                caller.write_memory(8, b"safe")?;
                Ok(vec![])
            }))
            .unwrap();
        for _ in 0..2 {
        ...
        assert_eq!(
            &instance.memory_export("memory").unwrap()[4..8],
            &9_i32.to_le_bytes()
        );
    }
    ...
    fn cooperative_unwind_cannot_leave_a_resumable_instance() {
        ...
        let pool = WasmMemoryPool::new(2);
        let mut instance = vm
            .instantiate_with_imports(provider(&pool, |_, _| panic!("sliced host")))
            .unwrap();
        let call = instance.begin_call("run", &[]).unwrap();
        let WasmCallStep::Pending(call) = call.resume(work(3)).unwrap() else {
            panic!("missing yield");
    ...
    fn incomplete_recording_cannot_be_escaped_by_guest_only_reentry() {
        ...
        let mut bindings = provider(&pool, |caller, _| {
            caller.write_memory(0, b"kept")?;
            panic!("recorded provider failed");
        });
        let observer = bindings
            .record_calls(WasmHostTraceLimits::default())
            .unwrap();
    ...
    fn startup_unwind_never_publishes_state_or_repeats_a_provider_on_repoll() {
        ...
        let mut future = Box::pin(vm.instantiate_cooperatively_with_imports(
            provider(&pool, move |_, _| {
                entered.fetch_add(1, Ordering::SeqCst);
                panic!("future startup");
            }),
            work(100),
        ));
    ...
    fn exit_then_unwind_does_not_masquerade_as_successful_exit_on_reentry() {
        ...
        let mut instance = vm
            .instantiate_with_imports(provider(&pool, |caller, _| {
                assert_eq!(
                    caller.exit(7),
                    WasmHostError::ProcessExit { code: 7 }.into()
                );
        ...
        assert_eq!(
            instance.call_export("host", &[]).unwrap_err(),
            interrupted()
        );
    ...
    fn resolved_scheduler_retires_panicked_work_not_healthy_sibling_instances() {
        ...
        let mut broken = module
            .instantiate_with_imports(
                &context,
                &policy,
                provider(&pool, |caller, _| {
                    caller.write_memory(8, b"kept")?;
                    panic!("scheduled host");
                }),
            )
            .unwrap();
        let mut healthy = module
            .instantiate_with_imports(&context, &policy, provider(&pool, |_, _| Ok(vec![])))
            .unwrap();
        let mut scheduler = WasmNativeScheduler::new(NonZeroUsize::new(2).unwrap());
        scheduler
            .submit(broken.begin_call("run", &[], &context, &policy).unwrap())
            .unwrap();
        let next = scheduler
            .submit(healthy.begin_call("run", &[], &context, &policy).unwrap())
            .unwrap();
        assert!(
            catch_unwind(AssertUnwindSafe(|| scheduler.run_next(
                work(100),
                &context,
                &policy
            )))
        ...
        drop(scheduler);
        assert!(
            matches!(broken.call_export("grow", &[I32(1)], &context, &policy),
            Err(WasmNativeLoadError::Execution(error)) if error == interrupted())
        );
        assert_eq!(
            &broken
                .memory_export("memory", &context, &policy)
                .unwrap()
                .unwrap()[8..12],
            b"kept"
        );
    ...
    fn owned_command_unwind_releases_private_state_without_revoking_tenant() {
        ...
        for start in [false, true] {
            ...
            let pool = WasmMemoryPool::new(2);
            let mut task =
                module.prepare_command(provider(&pool, |_, _| panic!("command provider")));
            ...
            assert!(matches!(
                replacement.resume(work(100), &context, &policy).unwrap(),
    ...
    fn interrupted_host_without_pool_memory_or_recording_is_still_terminal() {
        let vm = WasmNumericVm::parse(&program(true, false, None), WasmNumericLimits::default())
            .unwrap();
        ...
        bindings
            .define(
                ...
                BTreeSet::from([Builtin]),
                1,
                |_, _| panic!("unpooled host"),
            )
            .unwrap();
        let mut instance = vm.instantiate_with_imports(bindings).unwrap();
        let payload =
            catch_unwind(AssertUnwindSafe(|| instance.call_export("host", &[]))).unwrap_err();
        assert_eq!(payload.downcast_ref::<&str>(), Some(&"unpooled host"));
        assert!(instance.memory_export("memory").is_none());
        assert_eq!(
            instance.call_export("grow", &[I32(0)]).unwrap_err(),
            interrupted()
        );
        assert_eq!(
    ...
    fn subtree_revocation_retires_all_three_task_kinds_without_stopping_a_sibling() {
        ...
        let policy = policy();
        let root = WasmMemoryPool::new(8);
        let tenant = root.partition(6).unwrap();
        let sibling = root.partition(2).unwrap();
        let mut instance = running
            .instantiate_with_imports(&context, &policy, imports(&tenant))
            .unwrap();
        let mut healthy = running
            .instantiate_with_imports(&context, &policy, imports(&sibling))
            .unwrap();
        let WasmStartupStep::Pending(startup) = starting
            .prepare_startup_with_imports(imports(&tenant))
            .resume(work(1), &context, &policy)
            .unwrap()
        else {
            panic!("startup must yield");
        };
        let WasmCommandStep::Pending(command) = running
            .prepare_command(imports(&tenant))
            .resume(work(1), &context, &policy)
            .unwrap()
        else {
            panic!("command must yield before entry");
        };
        ...
        scheduler
            .submit(instance.begin_call("run", &[], &context, &policy).unwrap())
            .unwrap();
        let next = scheduler
            .submit(healthy.begin_call("run", &[], &context, &policy).unwrap())
            .unwrap();
        ...
        for _ in 0..3 {
            assert!(
                matches!(scheduler.run_next(work(100), &context, &policy).unwrap().outcome,

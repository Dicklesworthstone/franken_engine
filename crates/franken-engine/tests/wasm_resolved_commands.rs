...
fn fixture(start: Option<&[u8]>, command: &[u8], export: bool, result: bool) -> Vec<u8> {
    ...
    imports.extend(leb(wasi.len()));
    imports.extend(wasi);
    imports.extend([
        9, b'p', b'r', b'o', b'c', b'_', b'e', b'x', b'i', b't', 0, 1,
    ]);
    ...
    if export {
        section(
            &mut bytes,
            7,
            &[1, 6, b'_', b's', b't', b'a', b'r', b't', 0, 3],
        );
    }
    if start.is_some() {
...
fn load(bytes: &[u8], limits: WasmNumericLimits, declared: bool) -> WasmNativeModule {
    ...
    resolver
        .load_wasm(
            &ModuleRequest::new("/app/command.wasm", ImportStyle::Import),
            &context(),
            &policy(),
            limits,
        )
...
fn module(start: Option<&[u8]>, command: &[u8]) -> WasmNativeModule {
    load(
        &fixture(start, command, true, false),
    ...
    F: FnMut(&mut WasmHostCaller<'_, '_>, &[WasmBoundaryValue]) -> Outcome + Send + Sync + 'static,
{
    let mut imports = WasiPreview1Config::default()
        .into_imports(BTreeSet::from([Builtin, VmDispatch]))
        .unwrap();
    imports
        .define(
            "h",
...
) -> impl FnMut() -> Result<(ResolutionContext, CapabilityPolicyHook), WasmNativeLoadError> {
    move || {
        Ok((
            context(),
...
fn synchronous_commands_translate_only_real_exits_and_do_not_enter_after_startup_exit() {
    for exit in [0_u32, 17, u32::MAX] {
        ...
        for startup_exit in [false, true] {
            let module = if startup_exit {
                module(Some(&body), &[0x10, 0, 0x0b])
            } else {
                module(None, &body)
            };
            ...
            assert_eq!(
                module
                    .run_wasi_command(&context(), &policy(), counted(calls.clone()))
                    .unwrap()
                    .exit_code(),
                exit
            );
...
fn lazy_construction_and_mandatory_phase_yield_do_not_run_command_entry_early() {
    ...
    let mut future = Box::pin(module.run_wasi_command_cooperatively(
        counted(calls.clone()),
        work(100),
        reader(),
    ));
...
fn every_quantum_preserves_startup_state_and_exact_once_entry_effects() {
    ...
    for quantum in [1, 2, 3, 7, 64, u64::MAX] {
        ...
        let mut future =
            Box::pin(module.run_wasi_command_cooperatively(bindings, work(quantum), reader()));
        assert_eq!(
            finish(future.as_mut(), &Arc::new(Wakes::default()))
                .unwrap()
                .exit_code(),
            17
        );
...
fn invalid_or_missing_entry_fails_before_linking_allocation_and_startup() {
    for (export, result, body) in [
        (false, false, vec![0x0b]),
        (true, true, vec![0x41, 1, 0x0b]),
    ] {
        let module = load(
            &fixture(Some(&[0x10, 0, 0x0b]), &body, export, result),
            WasmNumericLimits {
                max_memory_pages: 0,
                ..WasmNumericLimits::default()
            },
        ...
        for cooperative in [false, true] {
            ...
            let outcome = if cooperative {
                finish(
                    Box::pin(module.run_wasi_command_cooperatively(bindings, work(1), reader()))
                        .as_mut(),
                    &Arc::new(Wakes::default()),
                )
            } else {
                module.run_wasi_command(&context(), &policy(), bindings)
            };
            assert!(matches!(
                outcome,
...
fn policy_precedes_entry_validation_and_provider_grants_cannot_supply_manifest_authority() {
    let bad = load(
        &fixture(None, &[0x0b], false, false),
        WasmNumericLimits::default(),
        true,
    );
    policy_denied(bad.run_wasi_command(
        &context(),
        &CapabilityPolicyHook::new(BTreeSet::new()),
        WasmHostImports::new(BTreeSet::new()),
    ));
    let undeclared = load(
        &fixture(Some(&[0x10, 0, 0x0b]), &[0x0b], true, false),
        WasmNumericLimits::default(),
        false,
    );
    let calls = Arc::new(AtomicUsize::new(0));
    let result = finish(
        Box::pin(undeclared.run_wasi_command_cooperatively(
            counted(calls.clone()),
            work(1),
            reader(),
        ))
...
fn final_policy_check_gates_normal_return_and_exit_in_both_phases() {
    for startup_exit in [false, true] {
        for exit in [false, true] {
            let body = if exit {
                vec![0x10, 0, 0x41, 17, 0x10, 1, 0x0b]
            } else {
                vec![0x10, 0, 0x0b]
            };
            let module = if startup_exit {
                module(Some(&body), &[0x0b])
            } else {
                module(None, &body)
            };
            let allowed = Arc::new(AtomicBool::new(true));
            ...
            policy_denied(finish(
                Box::pin(module.run_wasi_command_cooperatively(
                    bindings,
                    work(100),
                    live_reader(allowed),
                ))
...
fn traps_are_not_exit_statuses_and_keep_precedence_over_later_policy_changes() {
    ...
    let result = finish(
        Box::pin(module.run_wasi_command_cooperatively(bindings, work(100), live_reader(allowed)))
            .as_mut(),
        &Arc::new(Wakes::default()),
    );
    assert!(matches!(
    ...
    assert!(matches!(
        module.run_wasi_command(
            &context(),
            &policy(),
            imports(|_, _| Err(WasmHostError::trap("not an exit").into()))
        ),
...
fn startup_and_entry_exhaustion_are_errors_not_refunded_by_polling() {
    ...
    for start in [false, true] {
        let bytes = if start {
            fixture(Some(&spin), &[0x0b], true, false)
        } else {
            fixture(None, &spin, true, false)
        };
        let module = load(
            &bytes,
            WasmNumericLimits {
        ...
        assert!(matches!(
            result,
            Err(WasmNativeLoadError::Execution(
                WasmNumericVmError::InstructionBudgetExceeded { max: 12 }
            ))
        ));
    }
}
...
fn spinning_initialization_cannot_prevent_another_command_from_finishing() {
    ...
    let mut a = Box::pin(spinner.run_wasi_command_cooperatively(
        counted(Arc::new(AtomicUsize::new(0))),
        work(2),
        reader(),
    ));
    let mut b = Box::pin(useful.run_wasi_command_cooperatively(
        counted(Arc::new(AtomicUsize::new(0))),
        work(2),
        reader(),
    ));
    ...
    for _ in 0..20 {
        ...
        if let Poll::Ready(result) = poll(b.as_mut(), &wakes) {
...
fn live_execution_cancellation_is_never_translated_into_a_success_status() {
    ...
    bindings
        .bind_execution_cancellation(token, "command-cancel")
        .unwrap();
    let result = finish(
        Box::pin(module.run_wasi_command_cooperatively(bindings, work(100), reader())).as_mut(),
        &Arc::new(Wakes::default()),
    );
    assert!(matches!(
        result,
...
fn scoped_replay_covers_both_phases_without_repeating_provider_effects() {
    ...
    let calls = Arc::new(AtomicUsize::new(0));
    let mut live = counted(calls.clone());
    let recording = live.record_calls(WasmHostTraceLimits::default()).unwrap();
    assert_eq!(
        finish(
            Box::pin(module.run_wasi_command_cooperatively(live, work(1), reader())).as_mut(),
            &Arc::new(Wakes::default())
        )
        .unwrap()
        .exit_code(),
    ...
    assert_eq!(
        finish(
            Box::pin(module.run_wasi_command_cooperatively(replay, work(7), reader())).as_mut(),
            &Arc::new(Wakes::default())
        )
        .unwrap()
        .exit_code(),
...
fn completion_and_caught_provider_panics_cannot_restart_entry() {
    ...
    for panic_provider in [false, true] {
        ...
        let bindings = imports(move |_, _| {
            observed.fetch_add(1, Ordering::SeqCst);
            assert!(!panic_provider, "trusted provider panic");
            Ok(vec![])
        });
        let wakes = Arc::new(Wakes::default());
        let mut future =
            Box::pin(module.run_wasi_command_cooperatively(bindings, work(100), reader()));
        let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            finish(future.as_mut(), &wakes)
        }));
        assert_eq!(outcome.is_err(), panic_provider);
        assert_eq!(calls.load(Ordering::SeqCst), 1);
        assert!(
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| poll(
                future.as_mut(),
                &wakes
            )))
            .is_err()
...
fn command_future_can_move_between_executor_threads() {
    ...
    let future = module.run_wasi_command_cooperatively(
        counted(Arc::new(AtomicUsize::new(0))),
        work(1),
        reader(),
    );
...
fn cooperative_exit_retains_full_unsigned_status_and_its_original_phase() {
    use frankenengine_engine::wasm_runtime_lane::wasi_preview1::{
        WasiCommandOutcome, WasiCommandPhase,
    };
    for start in [false, true] {
        ...
        for quantum in [1, 2, 7, u64::MAX] {
            ...
            assert_eq!(
                actual,
                WasiCommandOutcome::Exited {
                    code: u32::MAX,
                    phase: if start {
                        WasiCommandPhase::Instantiation
                    } else {
                        WasiCommandPhase::Command
                    },
                }
            );
            assert_eq!(calls.load(Ordering::SeqCst), 0);

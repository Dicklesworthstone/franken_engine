...
const LOOP: &[u8] = &[
    0x41, 20, 0x21, 0, 0x02, 0x40, 0x03, 0x40, 0x20, 0, 0x45, 0x0d, 1, 0x23, 0, 0x41, 1, 0x6a,
    0x24, 0, 0x20, 0, 0x41, 1, 0x6b, 0x21, 0, 0x0c, 0, 0x0b, 0x0b, 0x41, 0, 0x23, 0, 0x36, 2, 0,
    0x0b,
];
...
impl Fixture {
    fn bytes(&self) -> Vec<u8> {
        ...
        if self.hosts != 0 {
            ...
            for index in 0..self.hosts {
                imports.extend([1, b'h', 1, b'f' + index, 0, 0]);
            }
        ...
        section(
            ...
            7,
            &[
                4, 3, b'g', b'e', b't', 0, self.hosts, 1, b'g', 3, 0, 1, b'm', 2, 0, 1, b't', 1, 0,
            ],
        );
        if self.start {
            section(
        ...
        let mut bodies = vec![
            if self.helper.is_some() { 3 } else { 2 },
            4,
            0,
            0x23,
            0,
        ...
        let mut body = if self.locals == 0 {
            vec![0]
        } else {
            let mut locals = vec![1];
            locals.extend(leb(self.locals));
            locals.push(0x7f);
            locals
        };
        body.extend_from_slice(&self.code);
        bodies.extend(leb(body.len() as u32));
        bodies.extend(body);
        if let Some(helper) = &self.helper {
            bodies.extend(leb(helper.len() as u32 + 1));
            bodies.push(0);
            bodies.extend(helper);
        }
        section(&mut bytes, 10, &bodies);
        let mut data = vec![2, 0, 0x41];
        data.extend(if self.invalid_data {
            vec![0x80, 0x80, 4]
        } else {
            vec![8]
        });
...
fn context() -> ResolutionContext {
    ResolutionContext::new("startup-trace", "startup-decision", "current-policy")
}
...
fn load(
    ...
    resolver
        .register_workspace_module(
            "/app/main.mjs",
            ModuleDefinition::new(ModuleSyntax::EsModule, "import './start.wasm';"),
        )
        .unwrap();
    let mut definition =
        ModuleDefinition::wasm_binary(&fixture.bytes(), &WasmNumericLimits::default()).unwrap();
    definition
        .required_capabilities
        .extend(declared.iter().copied());
    resolver
        .register_workspace_module("/app/start.wasm", definition)
        .unwrap();
    resolver
        .load_wasm(
            &ModuleRequest::new("./start.wasm", ImportStyle::Import).with_referrer("/app/main.mjs"),
            &context(),
            &policy(declared),
            limits,
        )
...
fn reader(
    current: &Arc<Mutex<CapabilityPolicyHook>>,
...
+ 'static {
    ...
    move || {
        ...
        Ok((context(), current.lock().unwrap().clone()))
    }
}
...
fn drive<F: Future>(mut future: Pin<&mut F>, wake: &Arc<WakeCount>) -> F::Output {
    for _ in 0..1_000_000 {
        if let Poll::Ready(output) = poll(future.as_mut(), wake) {
            return output;
        }
    }
...
fn host_fixture() -> Fixture {
    Fixture {
        hosts: 1,
        code: TWO_HOSTS.to_vec(),
        ..Fixture::default()
    }
}
...
fn lazy_resolved_startup_preserves_metrics_state_and_one_wake_per_pending_poll() {
    ...
    for work in [1, 2, 7, u64::MAX] {
        ...
        assert_eq!(
            ...
            wake.0.load(Ordering::SeqCst) + 2
        );
        assert_eq!(calls.load(Ordering::SeqCst), 2);
        assert_eq!(
            instance
                .start_execution(&context(), &policy(&[Builtin]))
                .unwrap(),
            baseline
                .start_execution(&context(), &policy(&[Builtin]))
                .unwrap()
        ...
        assert_eq!(
            instance
                .memory_export("m", &context(), &policy(&[Builtin]))
                .unwrap(),
            baseline
                .memory_export("m", &context(), &policy(&[Builtin]))
                .unwrap()
        ...
        assert_eq!(
            instance
                .call_export("get", &[], &context(), &policy(&[Builtin]))
                .unwrap()
                .results,
            [I32(9)]
        );
...
fn alias_and_canonical_denials_are_rechecked_while_startup_is_pending() {
    for name in ["./start.wasm", "/app/start.wasm"] {
        ...
        let mut future = Box::pin(module.instantiate_cooperatively_with_imports(
            counting(&calls),
            quantum(1),
            reader(&current, &Arc::new(AtomicUsize::new(0))),
        ));
...
fn last_callback_policy_changes_are_observed_before_instance_publication() {
    ...
    let reads = Arc::new(AtomicUsize::new(0));
    let calls = Arc::new(AtomicUsize::new(0));
    let revoke = Arc::clone(&current);
    let observed = Arc::clone(&calls);
    let provider = imports(move |_, _| {
        observed.fetch_add(1, Ordering::SeqCst);
        *revoke.lock().unwrap() = policy(&[]);
        Ok(vec![])
    });
    let mut future = Box::pin(module.instantiate_cooperatively_with_imports(
        provider,
...
fn no_start_modules_still_require_a_current_publication_decision() {
    ...
    denied(drive(future.as_mut(), &Arc::new(WakeCount::default())));
    let mut future =
        Box::pin(module.instantiate_cooperatively(quantum(1), || Ok((context(), policy(&[])))));
...
fn policy_reader_failure_is_terminal_and_releases_captured_provider_state() {
    ...
    for fail_on in [1, 2] {
        ...
        let lifetime = Lifetime(Arc::clone(&released));
        let provider = imports(move |_, _| {
            let _keep = &lifetime;
            Ok(vec![])
        });
        ...
        let mut future = Box::pin(module.instantiate_cooperatively_with_imports(
            ...
            move || {
                reads += 1;
                if reads == fail_on {
                    return Err(WasmNumericVmError::from(WasmHostError::trap(
                        "policy reader unavailable",
                    ))
                    .into());
                }
        ...
        assert!(matches!(
            drive(future.as_mut(), &Arc::new(WakeCount::default())),
...
fn original_execution_error_precedes_a_later_policy_reader_failure() {
    ...
    let mut future = Box::pin(module.instantiate_cooperatively(quantum(10), move || {
        if observed.fetch_add(1, Ordering::SeqCst) != 0 {
            return Err(
                WasmNumericVmError::from(WasmHostError::trap("later policy failure")).into(),
            );
        }
        Ok((context(), policy(&[])))
    }));
    ...
    for granted in [false, true] {
        for declared in [false, true] {
            for authorized in [false, true] {
                ...
                let mut provider = WasmHostImports::new(if granted {
                    [VmDispatch, Builtin].into()
                } else {
                    [VmDispatch].into()
                });
                provider
...
fn startup_work_slices_cannot_replenish_the_original_hard_budget() {
    ...
    let wake = Arc::new(WakeCount::default());
    assert!(matches!(
        drive(future.as_mut(), &wake),
        Err(WasmNativeLoadError::Execution(
            WasmNumericVmError::InstructionBudgetExceeded { max: 10 }
        ))
    ));
...
fn execution_cancellation_and_service_revocation_survive_policy_authorization() {
    for execution in [false, true] {
        ...
        if execution {
            provider
                .bind_execution_cancellation(token.clone(), "resolved-start")
                .unwrap();
        } else {
            provider
                .bind_capability_revocation(Builtin, token.clone(), "resolved-service")
                .unwrap();
        }
        let mut future = Box::pin(module.instantiate_cooperatively_with_imports(
            provider,
...
fn partially_started_instances_remain_isolated_and_cannot_repeat_completed_effects() {
    ...
    let mut a = Box::pin(module.instantiate_cooperatively_with_imports(
        counting(&first),
        quantum(1),
        || Ok((context(), policy(&[Builtin]))),
    ));
    let mut b = Box::pin(module.instantiate_cooperatively_with_imports(
        counting(&second),
        quantum(7),
        || Ok((context(), policy(&[Builtin]))),
    ));

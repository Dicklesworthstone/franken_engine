...
use frankenengine_engine::wasm_runtime_lane::numeric::{
    WasmHostCaller, WasmHostError, WasmHostImports, WasmNumericLimits, WasmNumericVmError,
    WasmStateError,
};
use frankenengine_engine::wasm_runtime_lane::wasi_preview1::{
    WASI_PREVIEW1_MODULE, WasiPreview1Config,
};
...
fn work(value: u64) -> NonZeroU64 {
    NonZeroU64::new(value).unwrap()
}
fn context() -> ResolutionContext {
    ResolutionContext::new("command-trace", "command-decision", "command-policy")
}
fn grants() -> BTreeSet<RuntimeCapability> {
    [Builtin, VmDispatch].into()
}
...
fn effect(value: i32) -> Vec<u8> {
    ...
    code.extend(signed(value));
    code.extend([0x10, 1, 0x0b]);
    code
}
fn exit(code: u32, opcode: u8) -> Vec<u8> {
    let mut bytes = vec![0x41];
    bytes.extend(signed(code as i32));
    if matches!(opcode, 0x11 | 0x13) {
        bytes.extend([0x41, 0, opcode, 1, 0]);
    } else {
        bytes.extend([opcode, 0]);
    }
...
fn load(
    ...
    resolver
        .register_workspace_module(
            "/app/main.mjs",
            ModuleDefinition::new(ModuleSyntax::EsModule, "import './command.wasm';"),
        )
        .unwrap();
    ...
    resolver
        .register_workspace_module("/app/command.wasm", definition)
        .unwrap();
    ...
    resolver
        .load_wasm(
            &ModuleRequest::new("./command.wasm", ImportStyle::Import)
...
where
    F: FnMut(&mut WasmHostCaller<'_, '_>, &[WasmBoundaryValue]) -> HostResult
        + Send
        + Sync
        + 'static,
{
    let mut imports = WasiPreview1Config::default()
        .into_imports(grants())
        .unwrap();
    imports
        .define(
...
fn recording_effects(events: Arc<Mutex<Vec<i32>>>) -> WasmHostImports {
    bindings(move |_, args| {
        let [I32(value)] = args else {
            panic!("validated callback ABI");
        };
...
fn one_command_retains_state_effect_order_and_work_across_both_phases() {
    let mut start = vec![0x41, 4, 0x41, 7, 0x3a, 0, 0];
    start.extend(effect(1));
    ...
    for quanta in [&[1][..], &[2, 7, 1, 99], &[3, 5]] {
        assert_eq!(run(quanta), expected);
    }
...
fn binary_start_and_entry_cannot_each_spend_a_fresh_hard_budget() {
    for (budget, expected_effects) in [(8, vec![1]), (9, vec![1, 2])] {
        ...
        let events = Arc::new(Mutex::new(Vec::new()));
        let error = finish(
            module.prepare_command(recording_effects(events.clone())),
            &[1, 2],
        )
        .unwrap_err();
        assert!(
            matches!(error, WasmNativeLoadError::Execution(WasmNumericVmError::InstructionBudgetExceeded { max }) if max == budget)
        );
    ...
    assert_eq!(
        finish(module.prepare_command(bindings(|_, _| Ok(vec![]))), &[1])
            .unwrap()
...
fn exit_during_binary_start_never_enters_the_command_entry() {
    ...
    assert_eq!(
        finish(module.prepare_command(bindings(forbidden)), &[1])
            .unwrap()
            .exit_code,
        73
    );
...
fn wrong_or_missing_entry_abi_refuses_before_initializer_effects_or_allocation() {
    for (entry, ty, export) in [
        (&[0x0b][..], 1, "_start"),
        (&[0x41, 0, 0x0b][..], 2, "_start"),
        (&[0x0b][..], 0, "other"),
    ] {
        let module = load(
            &fixture(Some(&effect(1)), entry, ty, export),
            &[Builtin],
            WasmNumericLimits {
                max_memory_pages: 0,
                ..WasmNumericLimits::default()
        ...
        let error = module
            .prepare_command(bindings(forbidden))
            .resume(work(1), &context(), &policy())
            .unwrap_err();
        assert!(matches!(
            error,
            WasmNativeLoadError::Execution(
...
fn broad_provider_and_policy_do_not_supply_undeclared_module_authority() {
    let module = load(
        &fixture(Some(&effect(1)), &[0x0b], 0, "_start"),
        &[],
        WasmNumericLimits::default(),
    );
    let error = module
        .prepare_command(bindings(forbidden))
        .resume(work(100), &context(), &policy())
        .unwrap_err();
    assert!(matches!(
        error,
        WasmNativeLoadError::Execution(WasmNumericVmError::State(WasmStateError::Host(
...
fn requested_and_canonical_deny_lists_remain_live_between_phases() {
    ...
    for specifier in ["./command.wasm", "/app/command.wasm"] {
        ...
        let task = pending(
            module
                .prepare_command(recording_effects(events.clone()))
                .resume(work(100), &context(), &policy())
                .unwrap(),
        );
        denied(
            task.resume(work(100), &context(), &policy().deny_specifier(specifier))
                .unwrap_err(),
        );
...
fn cancellation_and_selective_revocation_stop_a_pending_entry() {
    ...
    for execution in [false, true] {
        ...
        if execution {
            imports
                .bind_execution_cancellation(token.clone(), "command")
                .unwrap();
        } else {
            imports
                .bind_capability_revocation(Builtin, token.clone(), "command")
                .unwrap();
        }
        let task = pending(
            module
...
fn guest_traps_and_lookalike_host_messages_are_not_exit_statuses() {
    ...
    assert!(matches!(
        finish(
            module.prepare_command(bindings(|_, _| Err(WasmHostError::trap(
                "wasm guest exited with status 0"
            )
            .into()))),
            &[1]
        ),
...
fn one_scoped_transcript_replays_both_phases_without_provider_execution() {
    ...
    let transcript = observer.snapshot().unwrap();
    assert_eq!(transcript.call_count(), 3);
    let mut imports = bindings(forbidden);
    let replay = imports
        .replay_calls(transcript, WasmHostTraceLimits::default())
        .unwrap();
    assert_eq!(
        finish(module.prepare_command(imports), &[9999]).unwrap(),
        expected
    );
...
fn command_replay_rejects_another_pinned_module_before_any_effect() {
    ...
    imports
        .replay_calls(
            ...
            WasmHostTraceLimits::default(),
        )
        .unwrap();
    let error = b
        .prepare_command(imports)
        .resume(work(10000), &context(), &policy())
        .unwrap_err();
    assert!(matches!(
        error,
        WasmNativeLoadError::Execution(WasmNumericVmError::State(WasmStateError::Host(
            WasmHostError::Trace(_)
        )))
    ));
}
...
fn prior_budget_failure_cannot_be_disguised_by_a_later_exit() {
    ...
    assert!(matches!(
        finish(module.prepare_command(imports), &[1]),
        Err(WasmNativeLoadError::Execution(
...
fn provider_panic_consumes_the_task_and_leaves_independent_commands_usable() {
    ...
    assert!(
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(move || task.resume(
            work(100),
            &context(),
            &policy()
        )))
...
fn command_task_and_results_preserve_send_sync_contracts() {
    ...
    check::<WasmCommandTask<'static>>();
    check::<WasmCommandExecution>();
}
...
fn stdio_command_fixture() -> Vec<u8> {
    ...
    section(
        ...
        1,
        &[
            3, 0x60, 0, 0, 0x60, 1, 0x7f, 0, 0x60, 4, 0x7f, 0x7f, 0x7f, 0x7f, 1, 0x7f,
        ],
    );
    let mut imports = vec![2];
    ...
    let entry = [
        0x41, 2, 0x41, 0, 0x41, 1, 0x41, 8, 0x10, 1, 0x1a, 0x41, 9, 0x10, 0, 0x00, 0x0b,
    ];
    let mut code = vec![2];
    ...
    section(
        ...
        11,
        &[
            2, 0, 0x41, 0, 0x0b, 8, 64, 0, 0, 0, 3, 0, 0, 0, 0, 0x41, 0xc0, 0, 0x0b, 3, 255, 0, 128,
        ],
    );
    bytes
}
...
fn actual_wasi_streams_and_exit_share_the_resolved_cooperative_command_lifecycle() {
    ...
    let module = load(
        &stdio_command_fixture(),
        &[Builtin, RuntimeCapability::Console],
        WasmNumericLimits::default(),
    );
    let mut allowed = policy();
    allowed
    ...
    let make = || {
        WasiPreview1Config::default()
            .into_imports_with_stdio(
                [VmDispatch, Builtin, RuntimeCapability::Console].into(),
                vec![],
    ...
    let recording = imports
        ...
        .unwrap();
    let expected =
        finish_with_policy(module.prepare_command(imports), &[1, 2, 3000], &allowed).unwrap();
    assert_eq!(expected.exit_code, 9);
    let captured = output.take_output().unwrap();
    assert_eq!(captured.stdout, [255, 0, 128]);
    assert_eq!(captured.stderr, [255, 0, 128]);
    let (mut imports, output) = make();
    let replay = imports
        .replay_calls(
            recording.snapshot().unwrap(),
        ...
        .unwrap();
    assert_eq!(
        finish_with_policy(module.prepare_command(imports), &[9999], &allowed).unwrap(),
        expected
    );
    replay.verify_complete().unwrap();
    let captured = output.take_output().unwrap();
...
mod scheduling {
    ...
    fn infinite_command_cannot_starve_existing_exports_startup_or_finite_commands() {
        let spin = command(
            None,
            &[0x03, 0x40, 0x0c, 0, 0x0b, 0x0b],
            WasmNumericLimits::default(),
        );
        let fast = command(None, &[0x0b], WasmNumericLimits::default());
        let ctx = context();
        let allowed = policy();
        let mut instance = fast
            .instantiate_with_imports(&ctx, &allowed, bindings(forbidden))
            .unwrap();
        let mut queue = WasmNativeScheduler::new(slots(4));
        let spinner = queue
            .submit_command(spin.prepare_command(bindings(forbidden)))
            .unwrap();
        let call = queue
            .submit(instance.begin_call("_start", &[], &ctx, &allowed).unwrap())
            .unwrap();
        let startup = queue
            .submit_startup(fast.prepare_startup_with_imports(bindings(forbidden)))
            .unwrap();
        let finite = queue
            .submit_command(fast.prepare_command(bindings(forbidden)))
            .unwrap();
        ...
        for turn in 0..32 {
            ...
            if turn < 4 {
                assert_eq!(result.task_id, order[turn]);
            }
            match result.outcome {
                WasmTaskOutcome::Pending => {}
                WasmTaskOutcome::Complete(_)
                | WasmTaskOutcome::Initialized(_)
                | WasmTaskOutcome::Exited(_) => {
        ...
        assert_eq!(
            completed,
            BTreeSet::from([call.id(), startup.id(), finite.id()])
        );
        assert_eq!(queue.len(), 1);
        spinner.cancel();
        assert!(matches!(
            queue.run_next(work(1), &ctx, &allowed).unwrap().outcome,
    ...
    fn command_identity_and_cumulative_work_span_startup_and_unsigned_exit() {
        ...
        let handle = queue
            .submit_command(module.prepare_command(recording_effects(events.clone())))
            .unwrap();
        let mut before = 0;
        ...
        for _ in 0..20 {
            ...
            assert_eq!(turn.kind, WasmTaskKind::Command);
            assert_eq!(turn.event.event, "wasm_command_turn");
            assert_eq!(turn.event.work_before, before);
            ...
            match turn.outcome {
                ...
                WasmTaskOutcome::Exited(execution) => {
                    ...
                    assert_eq!(before, 9);
                    assert_eq!(turn.event.outcome, "exit");
                    complete = true;
                    break;
                }
    ...
    fn queue_full_returns_a_partially_executed_command_without_refunding_work() {
        ...
        assert!(matches!(
            queue.run_next(work(100), &ctx, &allowed).unwrap().outcome,
            WasmTaskOutcome::Complete(_)
        ));
        let new = queue.submit_command(task).unwrap();
        assert_eq!(new.id().get(), old.id().get() + 1);
        old.cancel();
    ...
    fn withdrawal_is_kind_safe_and_does_not_restart_a_command_phase() {
        ...
        let call = queue
            .submit(instance.begin_call("_start", &[], &ctx, &allowed).unwrap())
            .unwrap();
        assert!(queue.take_startup(cmd.id()).is_none());
        assert!(queue.take(cmd.id()).is_none());
        assert!(queue.take_command(startup.id()).is_none());
        assert!(queue.take_command(call.id()).is_none());
        assert_eq!(queue.next_task(), Some(cmd.id()));
    ...
    fn cancelled_admission_stops_before_allocation_and_cannot_be_transferred() {
        ...
        let mut queue = WasmNativeScheduler::new(slots(1));
        let handle = queue
            .submit_command(module.prepare_command(bindings(forbidden)))
            .unwrap();
        handle.cancel();
    ...
    fn revoked_policy_retires_only_the_selected_command_before_entry_effects() {
        ...
        assert!(matches!(
            queue
                .run_next(work(100), &context(), &policy())
                .unwrap()
                .outcome,
            WasmTaskOutcome::Pending
        ));
        ...
        assert_eq!(turn.event.work_before, 5);
    ...
    fn cancellation_in_the_final_callback_suppresses_command_completion_not_effects() {
        let module = command(None, &effect(2), WasmNumericLimits::default());
        let signal = Arc::new(Mutex::new(None::<WasmTaskHandle>));
        ...
        let mut queue = WasmNativeScheduler::new(slots(1));
        *signal.lock().unwrap() = Some(
            queue
                .submit_command(module.prepare_command(imports))
                .unwrap(),
        );
        assert!(matches!(
            queue
        ...
        let turn = queue.run_next(work(100), &context(), &policy()).unwrap();
        assert!(matches!(turn.outcome, WasmTaskOutcome::Cancelled));
        assert_eq!(turn.event.work_after, Some(4));
        assert_eq!(*events.lock().unwrap(), [1]);
        assert!(queue.is_empty());
    }
    ...
    fn cancellation_does_not_replace_a_real_command_budget_failure() {
        ...
        *signal.lock().unwrap() = Some(
        ...
        assert!(matches!(
            turn.outcome,
            WasmTaskOutcome::Failed(WasmNativeLoadError::Execution(
                WasmNumericVmError::InstructionBudgetExceeded { .. }
            ))
        ));
        assert_eq!(turn.event.work_after, None);
    ...
    fn a_panicked_command_is_removed_without_losing_other_ready_work() {
        ...
        assert!(
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| queue.run_next(
                work(100),
                &context(),
                &policy()
            )))

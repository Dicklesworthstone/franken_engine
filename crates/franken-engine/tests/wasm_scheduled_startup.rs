...
use frankenengine_engine::wasm_runtime_lane::scheduler::{WasmStartupStep, WasmStartupTask};
use frankenengine_engine::wasm_runtime_lane::{
    WasmBoundaryValue, WasmFunctionSignature, WasmNativeInstance, WasmNativeLoadError,
    WasmNativeModule,
};
...
fn program(depth: i32, mode: u8, hosts: u8, trap: bool, start: bool) -> Vec<u8> {
    ...
    section(
        &mut bytes,
        1,
        &[3, 0x60, 0, 0, 0x60, 0, 1, 0x7f, 0x60, 1, 0x7f, 0],
    );
    if imported != 0 {
        section(
            &mut bytes,
            2,
            &[1, 1, b'h', 4, b't', b'i', b'c', b'k', 0, 0],
        );
    }
    ...
    if start {
        section(&mut bytes, 8, &[imported + 2]);
    }
    section(&mut bytes, 9, &[1, 0, 0x41, 0, 0x0b, 1, imported]);
    let mut worker = vec![
        0, 0x23, 0, 0x41, 1, 0x6a, 0x24, 0, 0x41, 0, 0x23, 0, 0x36, 2, 0, 0x20, 0, 0x04, 0x40,
        0x20, 0, 0x41, 1, 0x6b,
    ];
    if mode & 1 == 0 {
        worker.extend([if mode == 2 { 0x12 } else { 0x10 }, imported]);
    } else {
        worker.extend([0x41, 0, if mode == 3 { 0x13 } else { 0x11 }, 2, 0]);
    }
    worker.extend([0x0b, 0x0b]);
    let getter = vec![0, 0x23, 0, 0x0b];
    let mut startup = vec![0, 0x41];
    startup.extend(signed(depth));
    startup.extend([0x10, imported]);
    for _ in 0..hosts {
        startup.extend([0x10, 0]);
    }
    if trap {
        startup.push(0);
    }
    startup.push(0x0b);
    let mut code = vec![3];
    for body in [&worker, &getter, &startup] {
...
fn quantum(work: u64) -> NonZeroU64 {
    NonZeroU64::new(work).unwrap()
}
fn context() -> ResolutionContext {
    ResolutionContext::new("startup-task", "startup-decision", "current")
}
...
fn load(bytes: &[u8], declared: bool, limits: WasmNumericLimits) -> WasmNativeModule {
    ...
    resolver
        .register_workspace_module(
            "/app/main.mjs",
            ModuleDefinition::new(ModuleSyntax::EsModule, "import './init.wasm';"),
        )
        .unwrap();
    let mut definition = ModuleDefinition::wasm_binary(bytes, &limits).unwrap();
    if declared {
        definition.required_capabilities.insert(Builtin);
    }
    resolver
        .register_workspace_module("/app/init.wasm", definition)
        .unwrap();
    resolver
        .load_wasm(
            &ModuleRequest::new("./init.wasm", ImportStyle::Import).with_referrer("/app/main.mjs"),
            &context(),
            &policy(),
            limits,
        )
...
fn module(bytes: &[u8]) -> WasmNativeModule {
    load(bytes, true, WasmNumericLimits::default())
}
...
fn run<'vm>(mut task: WasmStartupTask<'vm>, quanta: &[u64]) -> WasmNativeInstance<'vm> {
    ...
    for turn in 0..1_000_000 {
        ...
        match task
            .resume(quantum(quanta[turn % quanta.len()]), &context(), &policy())
            .unwrap()
        {
...
fn prepared_startup_is_lazy_and_denial_precedes_allocation() {
    ...
    denied(
        module
            .prepare_startup_with_imports(imports(effects.clone(), false))
...
fn variable_slices_match_synchronous_startup_state_and_metrics() {
    for mode in 0..4 {
        for quanta in [&[1][..], &[2, 7, 1, 64], &[u64::MAX]] {
            ...
            assert_eq!(
                actual.start_execution(&context(), &policy()).unwrap(),
                expected.start_execution(&context(), &policy()).unwrap()
            );
            assert_eq!(
                actual.memory_export("m", &context(), &policy()).unwrap(),
                expected.memory_export("m", &context(), &policy()).unwrap()
            );
            assert_eq!(
                actual.table_export("t", &context(), &policy()).unwrap(),
                expected.table_export("t", &context(), &policy()).unwrap()
            );
            assert_eq!(
                actual
                    .call_export("get", &[], &context(), &policy())
                    .unwrap()
                    .results,
                [I32(13)]
            );
...
fn no_start_publication_is_authorized_and_remains_once_only() {
    ...
    denied(module.prepare_startup().resume(
        quantum(1),
        &context(),
        &policy().deny_specifier("./init.wasm"),
    ));
...
fn tail_startup_retains_one_callee_activation_across_yields() {
    for mode in [2, 3] {
        let module = load(
            &program(20_000, mode, 0, false, true),
            true,
            WasmNumericLimits {
                max_call_depth: 2,
                max_live_values: 4,
        ...
        assert_eq!(
            instance.global_export("g", &context(), &policy()).unwrap(),
            Some(&I32(20_001))
        );
        assert_eq!(
            instance
    ...
    for work in [1, 7, 64] {
        ...
        for _ in 0..100 {
            match task.resume(quantum(work), &context(), &policy()) {
                Ok(WasmStartupStep::Pending(next)) => {
                    assert!(next.instructions_executed() <= 25);
                    task = next;
                }
                Err(WasmNativeLoadError::Execution(
                    WasmNumericVmError::InstructionBudgetExceeded { max: 25 },
                )) => {
                    exhausted = true;
                    break;
                }
...
fn every_current_policy_revocation_stops_the_next_startup_host_effect() {
    ...
    for capability in [RuntimeCapability::ModuleLoad, VmDispatch, Builtin] {
        ...
        revoked.granted_capabilities.remove(&capability);
...
fn dropping_partial_startup_keeps_external_effects_without_repeating_them() {
    ...
    assert_eq!(effects.load(Ordering::SeqCst), 1);
    let ready = run(
        module.prepare_startup_with_imports(imports(effects.clone(), false)),
        &[1],
    );
    assert_eq!(effects.load(Ordering::SeqCst), 3);
    assert_eq!(
        ready.global_export("g", &context(), &policy()).unwrap(),
        Some(&I32(1))
    );
}
...
fn declarations_and_provider_grants_cannot_be_manufactured_by_a_turn_policy() {
    ...
    assert!(matches!(
        module
            .prepare_startup()
            .resume(quantum(u64::MAX), &context(), &policy()),
        Err(WasmNativeLoadError::Execution(
...
fn live_execution_cancellation_survives_reset_between_startup_turns() {
    ...
    bindings
        .bind_execution_cancellation(token.clone(), "scheduled-start")
        .unwrap();
    let task = match module
        .prepare_startup_with_imports(bindings)
        .resume(quantum(7), &context(), &policy())
        .unwrap()
    {
        WasmStartupStep::Pending(task) => task,
    ...
    token.reset();
    assert!(matches!(
        task.resume(quantum(7), &context(), &policy()),
        Err(WasmNativeLoadError::Execution(WasmNumericVmError::State(
...
fn selective_revocation_before_the_first_resume_prevents_host_startup() {
    ...
    bindings
        .bind_capability_revocation(Builtin, token.clone(), "startup-service")
        .unwrap();
    let task = module.prepare_startup_with_imports(bindings);
    token.cancel();
    token.reset();
    assert!(matches!(
        task.resume(quantum(100), &context(), &policy()),
        Err(WasmNativeLoadError::Execution(WasmNumericVmError::State(
            WasmStateError::Host(WasmHostError::CapabilityDenied {
                capability: Builtin,
                ..
...
fn scoped_startup_replay_is_independent_of_quanta_and_never_reenters_providers() {
    ...
    let replay = bindings
        .replay_calls(tape, WasmHostTraceLimits::default())
        .unwrap();
    let reproduced = run(module.prepare_startup_with_imports(bindings), &[u64::MAX]);
    replay.verify_complete().unwrap();
    assert_eq!(effects.load(Ordering::SeqCst), 2);
    assert_eq!(
        live.start_execution(&context(), &policy()).unwrap(),
        reproduced.start_execution(&context(), &policy()).unwrap()
    );
    assert_eq!(
        live.memory_export("m", &context(), &policy()).unwrap(),
        reproduced
            .memory_export("m", &context(), &policy())
            .unwrap()
    );
}
...
fn a_later_start_trap_preserves_completed_callback_recording() {
    ...
    let recording = bindings
        ...
        .unwrap();
    assert!(matches!(
        module.prepare_startup_with_imports(bindings).resume(
            quantum(u64::MAX),
            &context(),
            &policy()
        ),
...
mod queue {
    ...
    use frankenengine_engine::wasm_runtime_lane::scheduler::{
        WasmNativeScheduler, WasmTaskAdmissionErrorKind, WasmTaskHandle, WasmTaskId, WasmTaskKind,
        WasmTaskOutcome,
    };
    ...
    fn spin_start() -> Vec<u8> {
        ...
        section(
            ...
            10,
            &[
                2, 7, 0, 0x03, 0x40, 0x0c, 0, 0x0b, 0x0b, 4, 0, 0x41, 7, 0x0b,
            ],
        );
        bytes
    }
    ...
    fn initialized<'call, 'vm>(
        scheduler: &mut WasmNativeScheduler<'call, 'vm>,
        work: u64,
    ) -> (WasmTaskId, WasmNativeInstance<'vm>) {
        for _ in 0..100_000 {
            let turn = scheduler
                .run_next(quantum(work), &context(), &policy())
    ...
    fn an_infinite_start_function_does_not_starve_ready_exports() {
        ...
        let call = scheduler
            .submit(
                instance
                    .begin_call("get", &[], &context(), &policy())
                    .unwrap(),
            )
            .unwrap();
        assert_eq!(scheduler.next_task_kind(), Some(WasmTaskKind::Startup));
        let turn = scheduler
            .run_next(quantum(2), &context(), &policy())
            .unwrap();
    ...
    fn initialized_instances_can_execute_exports_without_rerunning_startup() {
        ...
        let start = scheduler
            .submit_startup(module.prepare_startup_with_imports(imports(effects.clone(), false)))
            .unwrap();
        let (id, instance) = initialized(&mut scheduler, 7);
        assert_eq!(id, start.id());
        let expected_start = instance
            .start_execution(&context(), &policy())
            .unwrap()
            .cloned();
    ...
    fn startup_and_export_admission_share_one_capacity_limit() {
        ...
        let call = scheduler
            ...
            .unwrap();
        let error = scheduler
            .submit_startup(module.prepare_startup())
            .unwrap_err();
        ...
        assert!(matches!(
            scheduler
                .run_next(quantum(1), &context(), &policy())
                .unwrap()
                .outcome,
            WasmTaskOutcome::Cancelled
        ));
    ...
    fn withdrawal_preserves_partial_work_and_wrong_kind_does_not_remove_tasks() {
        ...
        assert!(matches!(
            scheduler
                .run_next(quantum(7), &context(), &policy())
                .unwrap()
                .outcome,
            WasmTaskOutcome::Pending
        ));
        let call = scheduler
            .submit(ready.begin_call("get", &[], &context(), &policy()).unwrap())
            .unwrap();
        ...
        assert_eq!(
            instance.start_execution(&context(), &policy()).unwrap(),
            module
                .instantiate(&context(), &policy())
                .unwrap()
                .start_execution(&context(), &policy())
                .unwrap()
        );
    ...
    fn denied_partial_startup_retires_independently_and_reports_current_context() {
        ...
        let healthy_id = scheduler
            .submit_startup(healthy.prepare_startup())
            .unwrap()
            .id();
        let changed = ResolutionContext::new("changed-trace", "changed-decision", "revoked-policy");
        let mut revoked = policy();
        revoked.granted_capabilities.remove(&Builtin);
        let turn = scheduler
            .run_next(quantum(100), &changed, &revoked)
            .unwrap();
    ...
    fn an_infinite_start_hits_its_hard_budget_without_retiring_other_tasks() {
        ...
        let mut scheduler = WasmNativeScheduler::new(slots(2));
        let slow_id = scheduler
            .submit_startup(slow.prepare_startup())
            .unwrap()
            .id();
        let healthy_id = scheduler
            .submit_startup(healthy.prepare_startup())
        ...
        for _ in 0..20 {
            let Some(turn) = scheduler.run_next(quantum(2), &context(), &policy()) else {
                break;
            };
            match turn.outcome {
                WasmTaskOutcome::Initialized(_) => {
                    assert_eq!(turn.task_id, healthy_id);
                    initialized_count += 1;
                }
                WasmTaskOutcome::Pending => assert_eq!(turn.task_id, slow_id),
                WasmTaskOutcome::Failed(WasmNativeLoadError::Execution(
                    WasmNumericVmError::InstructionBudgetExceeded { max: 9 },
                )) => {
                    assert_eq!(turn.task_id, slow_id);
                    refused = true;
                }
            ...
            bindings
                .define(
                    ...
                    move |_, _| {
                        ...
                        if trap {
                            return Err(WasmHostError::trap("first failure").into());
                        }
                        Ok(vec![])
                    },
                )
                .unwrap();
            let mut scheduler = WasmNativeScheduler::new(slots(1));
            *handle.lock().unwrap() = Some(
                scheduler
                    .submit_startup(module.prepare_startup_with_imports(bindings))
                    .unwrap(),
            );
            let turn = scheduler
                .run_next(quantum(u64::MAX), &context(), &policy())
                .unwrap();
            if trap {
                assert!(matches!(
                    turn.outcome,
    ...
    fn a_provider_panic_drops_only_the_current_startup_and_cannot_reenter_it() {
        ...
        let mut scheduler = WasmNativeScheduler::new(slots(2));
        scheduler
            .submit_startup(broken.prepare_startup_with_imports(bindings))
            .unwrap();
        let healthy_id = scheduler
            .submit_startup(healthy.prepare_startup())
            .unwrap()

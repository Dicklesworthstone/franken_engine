...
impl Default for Fixture {
    fn default() -> Self {
        Self {
            ...
            starts: vec![vec![0]],
            imports: vec![], tables: vec![], elements: vec![], memory: vec![], data: vec![],
...
impl Fixture {
    fn bytes(&self) -> Vec<u8> {
        ...
        if !self.memory.is_empty() { section(&mut module, 5, &self.memory); }
        section(&mut module, 6, &[1, 0x7f, 1, 0x41, 0, 0x0b]);
        section(&mut module, 7, &self.exports);
        for start in &self.starts { section(&mut module, 8, start); }
        ...
        if !self.data.is_empty() { section(&mut module, 11, &self.data); }
        module
    }
...
fn start_reads_initialized_data_and_publishes_its_memory_writes() {
    ...
    fixture.with_memory();
    fixture.functions[0].1 = vec![0x41, 0, 0x28, 2, 0, 0x41, 1, 0x6a, 0x24, 0, 0x41, 4, 0x23, 0, 0x36, 2, 0, 0x0b];
    let vm = fixture.vm();
    let mut instance = vm.instantiate().unwrap();
    assert_eq!(instance.global_export("g"), Some(&I32(42)));
    assert_eq!(&instance.memory_export("m").unwrap()[..8], &[41, 0, 0, 0, 42, 0, 0, 0]);
...
fn absent_start_is_distinct_from_an_empty_start_and_never_invokes_function_zero() {
    ...
    let vm = WasmNumericVm::parse(&fixture.bytes(), WasmNumericLimits { max_instructions: 0, ..WasmNumericLimits::default() }).unwrap();
    let instance = vm.instantiate().unwrap();
    ...
    let vm = fixture.vm();
    assert_eq!(vm.instantiate().unwrap().start_execution().unwrap().instructions_executed, 1);
}
...
fn start_indices_signatures_and_section_integrity_are_validated_before_execution() {
    for starts in [vec![vec![99]], vec![vec![1]], vec![vec![0, 0]], vec![vec![]], vec![vec![0x80]], vec![vec![0], vec![0]]] {
        let mut fixture = Fixture::default();
    ...
    assert!(matches!(WasmNumericVm::parse(&fixture.bytes(), WasmNumericLimits::default()), Err(WasmNumericVmError::InvalidModule { detail }) if detail.contains("[] -> []")));
    fixture.types[0].0.clear();
    fixture.starts = vec![vec![0x80, 0]]; // padded u32 index zero is valid
    assert_eq!(fixture.vm().instantiate().unwrap().global_export("g"), Some(&I32(1)));
}
...
fn a_trapping_start_never_returns_a_partial_instance_and_parse_does_not_execute_it() {
    ...
    let vm = fixture.vm();
    for _ in 0..3 {
        assert!(matches!(vm.instantiate(), Err(WasmNumericVmError::Unreachable { function_index: 0 })));
    }
    assert!(matches!(vm.call_export("f", &[]), Err(WasmNumericVmError::Unreachable { function_index: 0 })));
    fixture.starts.clear();
    assert_eq!(fixture.vm().instantiate().unwrap().global_export("g"), Some(&I32(0)));
}
...
fn segment_bounds_fail_before_start_code_is_entered() {
    ...
    fixture.functions[0].1 = vec![0x00, 0x0b];
    assert!(matches!(fixture.vm().instantiate(), Err(WasmNumericVmError::State(WasmStateError::DataSegmentOutOfBounds { .. }))));
    fixture.data.clear();
    fixture.tables = vec![1, 0x70, 0, 0];
    fixture.elements = vec![1, 0, 0x41, 0, 0x0b, 1, 0];
    assert!(matches!(fixture.vm().instantiate(), Err(WasmNumericVmError::State(WasmStateError::ElementSegmentOutOfBounds { .. }))));
}
...
fn start_uses_exact_instruction_and_call_depth_limits_without_unmetered_loops() {
    ...
    assert!(matches!(vm.instantiate(), Err(WasmNumericVmError::InstructionBudgetExceeded { max: 4 })));
    let vm = WasmNumericVm::parse(&fixture.bytes(), WasmNumericLimits { max_instructions: 5, ..WasmNumericLimits::default() }).unwrap();
    assert_eq!(vm.instantiate().unwrap().start_execution().unwrap().instructions_executed, 5);
    fixture.functions[0].1 = vec![0x03, 0x40, 0x0c, 0, 0x0b, 0x0b];
    let vm = WasmNumericVm::parse(&fixture.bytes(), WasmNumericLimits { max_instructions: 17, ..WasmNumericLimits::default() }).unwrap();
    assert!(matches!(vm.instantiate(), Err(WasmNumericVmError::InstructionBudgetExceeded { max: 17 })));
    ...
    assert!(matches!(vm.instantiate(), Err(WasmNumericVmError::CallDepthExceeded { max: 4 })));
}
...
fn start_memory_growth_uses_the_same_native_work_meter() {
    ...
    assert_eq!(instance.start_execution().unwrap().instructions_executed, 1028);
    let vm = WasmNumericVm::parse(&fixture.bytes(), WasmNumericLimits { max_instructions: 1027, ..WasmNumericLimits::default() }).unwrap();
    assert!(matches!(vm.instantiate(), Err(WasmNumericVmError::InstructionBudgetExceeded { max: 1027 })));
}
...
fn imported_start_function_does_not_acquire_ambient_host_authority() {
    ...
    fixture.exports = vec![1, 1, b'g', 3, 0];
    assert!(matches!(fixture.vm().instantiate(), Err(WasmNumericVmError::ImportedFunctionUnsupported { function_index: 0, module, name }) if module == "h" && name == "f"));
}
...
fn cli_runs_initialization_and_reports_start_and_export_work_separately() {
    ...
    stdin.write_all(serde_json::to_string(&request).unwrap().as_bytes()).unwrap();
    drop(stdin);
    ...
    assert_eq!(response["execution"]["results"], serde_json::json!([{"I32": 2}]));
}

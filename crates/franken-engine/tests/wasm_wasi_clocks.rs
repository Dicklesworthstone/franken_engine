...
fn fixture(memory: bool, start: bool) -> Vec<u8> {
    ...
    section(
        ...
        1,
        &[
            3, 0x60, 3, 0x7f, 0x7e, 0x7f, 1, 0x7f, 0x60, 2, 0x7f, 0x7f, 1, 0x7f, 0x60, 0, 0,
        ],
    );
    ...
    for (index, function) in ["clock_time_get", "clock_res_get"].iter().enumerate() {
        ...
        imports.extend([0, index as u8]);
    }
    section(&mut out, 2, &imports);
    section(&mut out, 3, if start { &[3, 0, 0, 2] } else { &[2, 0, 0] });
    section(&mut out, 4, &[1, 0x70, 0, 1]);
    if memory {
        section(&mut out, 5, &[1, 1, 1, 1]);
    }
    let mut exports = vec![4 + u8::from(memory)];
    for (index, function) in ["time", "resolution", "direct", "indirect"]
        .iter()
        .enumerate()
    {
        name(&mut exports, function);
        exports.extend([0, index as u8]);
    }
    if memory {
    ...
    for body in [direct.as_slice(), indirect.as_slice()]
        .into_iter()
        .chain(start.then_some(startup.as_slice()))
    {
    ...
    if memory {
        let mut data = vec![1, 0, 0x41, 0, 0x0b, 32];
        data.extend([0xaa; 32]);
...
fn vm(memory: bool, start: bool, work: u64) -> WasmNumericVm {
    WasmNumericVm::parse(
        &fixture(memory, start),
        WasmNumericLimits {
            max_instructions: work,
            ..Default::default()
        },
    )
    .unwrap()
...
{
    WasmHostImports::new(caps())
        .with_wasi_clock_source(source, limits)
        .unwrap()
}
fn scripted(limits: WasiClockLimits) -> (WasmHostImports, Arc<AtomicUsize>) {
    ...
    (
        imports(
            move |_| Ok(seen.fetch_add(1, Ordering::SeqCst) as u64 + 100),
            limits,
        ),
        count,
    )
...
fn time(
    ...
    let result = instance
        .call_export(name, &[I32(id), I64(precision as i64), I32(out)])
        .unwrap();
    let [I32(errno)] = result.results.as_slice() else {
        panic!("clock errno ABI");
    };
    *errno
}
fn resolution(instance: &mut WasmNumericInstance<'_>, id: i32, out: i32) -> i32 {
    let result = instance
        .call_export("resolution", &[I32(id), I32(out)])
        .unwrap();
    let [I32(errno)] = result.results.as_slice() else {
        panic!("resolution errno ABI");
    };
    *errno
}
fn memory<'a>(instance: &'a WasmNumericInstance<'_>) -> &'a [u8] {
    instance.memory_export("memory").unwrap()
}
...
fn all_clock_ids_preserve_unsigned_precision_and_timestamp_bits() {
    ...
    let provider = imports(
        move |request| {
            ...
            Ok(match request {
                WasiClockRequest::Resolution(clock) => 1 + clock as u64,
    ...
    for clock in [
        ...
        WasiClockId::ProcessCpu,
        WasiClockId::ThreadCpu,
    ] {
        assert_eq!(resolution(&mut instance, clock as i32, 3), 0);
        assert_eq!(word(&instance, 3), 1 + clock as u64);
        assert_eq!(time(&mut instance, "time", clock as i32, u64::MAX, 3), 0);
        assert_eq!(word(&instance, 3), u64::MAX - clock as u64);
        assert_eq!(memory(&instance)[2], 0xaa);
        assert_eq!(memory(&instance)[11], 0xaa);
    }
...
fn direct_indirect_and_exported_time_calls_use_one_owned_source() {
    let vm = vm(true, false, 10000);
    let (provider, count) = scripted(Default::default());
    let mut instance = vm.instantiate_with_imports(provider).unwrap();
    for (index, export) in ["time", "direct", "indirect"].iter().enumerate() {
        assert_eq!(time(&mut instance, export, 1, 0, 0), 0);
        assert_eq!(word(&instance, 0), 100 + index as u64);
    }
...
fn invalid_ids_and_bad_unsigned_extents_never_query_the_source() {
    ...
    for id in [4, -1, i32::MAX] {
        assert_eq!(resolution(&mut instance, id, 0), 28);
        assert_eq!(time(&mut instance, "time", id, 0, 0), 28);
    }
    for out in [-1, 65_529, 65_536] {
        assert_eq!(resolution(&mut instance, 1, out), 21);
        assert_eq!(time(&mut instance, "time", 1, 0, out), 21);
    }
    assert_eq!(memory(&instance), before);
    assert_eq!(count.load(Ordering::SeqCst), 0);
    assert_eq!(time(&mut instance, "time", 1, 0, 65_528), 0);
    assert_eq!(word(&instance, 65_528), 100);
}

#[test]
fn missing_memory_does_not_create_a_host_timestamp_buffer() {
    let vm = vm(false, false, 10000);
    let (provider, count) = scripted(Default::default());
    let mut instance = vm.instantiate_with_imports(provider).unwrap();
    assert_eq!(resolution(&mut instance, 1, 0), 21);
    assert_eq!(time(&mut instance, "time", 1, 0, 0), 21);
    assert_eq!(count.load(Ordering::SeqCst), 0);
}
...
fn invalid_zero_resolution_and_source_errors_leave_output_unchanged() {
    ...
    for result in [
        Ok(0),
        Err(WasiSourceError::Io),
        Err(WasiSourceError::WouldBlock),
        Err(WasiSourceError::Unsupported),
    ] {
        let provider = imports(
            move |_| result,
            WasiClockLimits {
        ...
        let expected = match result {
            Ok(_) | Err(WasiSourceError::Io) => 29,
            Err(WasiSourceError::WouldBlock) => 6,
            Err(WasiSourceError::Unsupported) => 58,
        };
        assert_eq!(resolution(&mut instance, 1, 0), expected);
        assert_eq!(memory(&instance), before);
    ...
    for id in [1, 2, 3] {
        ...
        let mut instance = vm
            .instantiate_with_imports(imports(
                move |_| Ok(readings.next().unwrap()),
                Default::default(),
            ))
            .unwrap();
        for (expected, value) in [(0, 10), (29, 10), (0, 10), (0, 11)] {
            assert_eq!(time(&mut instance, "time", id, 0, 0), expected);
            assert_eq!(word(&instance, 0), value);
        }
    }
}
...
fn realtime_can_jump_backwards_and_monotonic_zero_is_valid() {
    let vm = vm(true, false, 10000);
    let mut readings = [10, 0, 0].into_iter();
    let mut instance = vm
        .instantiate_with_imports(imports(
            move |_| Ok(readings.next().unwrap()),
            Default::default(),
        ...
        .unwrap();
    assert_eq!(time(&mut instance, "time", 0, 0, 0), 0);
    assert_eq!(word(&instance, 0), 10);
    assert_eq!(time(&mut instance, "time", 0, 0, 0), 0);
    assert_eq!(word(&instance, 0), 0);
    assert_eq!(time(&mut instance, "time", 1, 0, 0), 0);
    assert_eq!(word(&instance, 0), 0);
}
...
fn clock_domains_and_independent_registries_do_not_share_high_water_marks() {
    let vm = vm(true, false, 10000);
    let mut readings = [100, 1, 2, 100].into_iter();
    let mut a = vm
        .instantiate_with_imports(imports(
            move |_| Ok(readings.next().unwrap()),
            Default::default(),
        ...
        .unwrap();
    for id in [1, 2, 3, 1] {
        assert_eq!(time(&mut a, "time", id, 0, 0), 0);
    }
    let mut b = vm
        .instantiate_with_imports(imports(|_| Ok(0), Default::default()))
        .unwrap();
    assert_eq!(time(&mut b, "time", 1, 0, 0), 0);
    assert_eq!(word(&b, 0), 0);
}

#[test]
fn resolution_time_and_startup_share_one_nonrefundable_query_allowance() {
    let vm = vm(true, true, 10000);
    let (provider, count) = scripted(WasiClockLimits {
        max_calls: 2,
        ..Default::default()
    });
    let mut instance = vm.instantiate_with_imports(provider).unwrap();
    assert!(instance.start_execution().is_some());
    assert_eq!(word(&instance, 0), 100);
    assert_eq!(resolution(&mut instance, 1, 8), 0);
    assert_eq!(word(&instance, 8), 101);
    assert_eq!(time(&mut instance, "time", 1, 0, 0), 42);
    assert_eq!(word(&instance, 0), 100);
...
fn local_and_shared_work_refusal_precede_clock_query_and_preserve_exact_cost() {
    for use_pool in [false, true] {
        for work in [2, 3] {
            ...
            let (mut provider, count) = scripted(Default::default());
            let pool = WasmWorkPool::new(work);
            if use_pool {
                provider.bind_work_pool(pool.clone()).unwrap();
            }
            let mut instance = vm.instantiate_with_imports(provider).unwrap();
...
fn no_ambient_clock_and_builtin_cannot_substitute_for_timer_authority() {
    ...
    let provider = WasmHostImports::new(BTreeSet::from([
        RuntimeCapability::VmDispatch,
        RuntimeCapability::Builtin,
    ]))
    .with_wasi_clock_source(
        move |_| {
...
fn cancellation_or_timer_revocation_on_return_prevents_timestamp_publication() {
    for service in [false, true] {
        ...
        if service {
            provider
                .bind_capability_revocation(RuntimeCapability::Timer, token, "clock")
                .unwrap();
        } else {
            provider
                .bind_execution_cancellation(token, "clock")
                .unwrap();
        }
        let mut instance = vm.instantiate_with_imports(provider).unwrap();
        let before = memory(&instance).to_vec();
        assert!(
            instance
                .call_export("time", &[I32(1), I64(0), I32(0)])
                .is_err()
        );
        assert_eq!(memory(&instance), before);
    }
...
fn source_panic_cannot_resample_a_poisoned_clock_after_caught_unwind() {
    let vm = vm(true, false, 10000);
    let provider = imports(
        |_| -> Result<u64, WasiSourceError> { panic!("clock panic") },
        Default::default(),
    );
    let mut instance = vm.instantiate_with_imports(provider).unwrap();
    assert!(
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            || instance.call_export("time", &[I32(1), I64(0), I32(0)])
        ))
        .is_err()
    );
    assert_eq!(word(&instance, 0), 0xaaaa_aaaa_aaaa_aaaa);
    assert!(matches!(
        instance.call_export("resolution", &[I32(1), I32(0)]),
        Err(WasmNumericVmError::State(WasmStateError::Host(
            WasmHostError::HostCallInterrupted
        )))
    ));
}
...
fn clock_replay_reproduces_startup_and_results_without_sampling_new_time() {
    let vm = vm(true, true, 100000);
    let trace_limits = WasmHostTraceLimits::default();
    let (mut provider, count) = scripted(Default::default());
    let recording = provider.record_calls(trace_limits).unwrap();
    let mut original = vm.instantiate_with_imports(provider).unwrap();
    assert_eq!(resolution(&mut original, 1, 8), 0);
    assert_eq!(time(&mut original, "indirect", 1, u64::MAX, 16), 0);
    let (mut provider, replay_count) = scripted(WasiClockLimits {
        max_calls: 0,
        ..Default::default()
    });
    let replay = provider
        .replay_calls(recording.snapshot().unwrap(), trace_limits)
        .unwrap();
    let mut replayed = vm.instantiate_with_imports(provider).unwrap();
    assert_eq!(resolution(&mut replayed, 1, 8), 0);
    assert_eq!(time(&mut replayed, "indirect", 1, u64::MAX, 16), 0);
    replay.verify_complete().unwrap();
    assert_eq!(memory(&replayed), memory(&original));
    assert_eq!(count.load(Ordering::SeqCst), 3);
...
fn sliced_calls_and_duplicate_installation_do_not_create_extra_source_reads() {
    ...
    let mut call = instance
        .begin_call("direct", &[I32(1), I64(-1), I32(0)])
        .unwrap();
    loop {
    ...
    assert!(matches!(
        provider.with_wasi_clock_source(|_| Ok(1), Default::default()),
        Err(WasiPreview1Error::Binding(
            WasmHostError::DuplicateBinding { .. }
        ))
    ));
}
...
fn command_sources(
    ...
    let (provider, output) = WasiPreview1Config::default()
        .into_imports_with_stdio(command_grants(), Vec::new(), WasiStdioLimits::default())
        .unwrap();
    let provider = provider
        .with_wasi_random_source(
            |bytes: &mut [u8]| {
...
fn expected_command_output() -> Vec<u8> {
    ...
    for n in [1_u64, 1_u64 << 63, (1_u64 << 63) + 5] {
        result.extend(n.to_le_bytes());
    }
    result.extend(240_u8..=255);
    result
}
...
fn compiled_c_command_combines_clock_entropy_and_stdio_with_exact_u64_bits() {
    let vm = WasmNumericVm::parse(
        include_bytes!("fixtures/wasi_clock_smoke.wasm"),
        WasmNumericLimits::default(),
    )
    .unwrap();
    let (provider, output) = command_sources(false);
    assert_eq!(run_command(&vm, provider).unwrap(), 0);
    let captured = output.take_output().unwrap();
    assert_eq!(captured.stdout, expected_command_output());
...
fn compiled_command_replay_neither_reads_sources_nor_reemits_stdout() {
    ...
    let (mut provider, output) = command_sources(false);
    let recording = provider.record_calls(limits).unwrap();
    assert_eq!(run_command(&vm, provider).unwrap(), 0);
    assert_eq!(
        output.take_output().unwrap().stdout,
        expected_command_output()
    );
    let (mut provider, output) = command_sources(true);
    let replay = provider
...
fn resolved_scheduled_command_rechecks_policy_and_executes_both_source_families() {
    ...
    let context = ResolutionContext::new("clock", "decision", "policy");
    let mut grants = wasm_module_required_capabilities();
    grants.extend(command_grants());
    let policy = CapabilityPolicyHook::new(grants.clone());
    let mut definition =
        ModuleDefinition::wasm_binary(include_bytes!("fixtures/wasi_clock_smoke.wasm"), &limits)
            .unwrap();
    definition.required_capabilities = grants;
    let mut resolver = DeterministicModuleResolver::new("/app");
    resolver
        .register_workspace_module("/app/clock.wasm", definition)
        .unwrap();
    let module = resolver
        .load_wasm(
            &ModuleRequest::new("/app/clock.wasm", ImportStyle::Import),
            &context,
            &policy,
            limits,
        )
        .unwrap();
    let (provider, output) = command_sources(false);
    let command = module.prepare_command(provider);
    let WasmCommandStep::Pending(command) =
        command.resume(NonZeroU64::MIN, &context, &policy).unwrap()
    else {
        panic!("phase boundary");
    };
    let mut denied = policy.clone();
    denied
        .granted_capabilities
        .remove(&RuntimeCapability::Timer);
    assert!(matches!(
        command.resume(NonZeroU64::MIN, &context, &denied),
        Err(WasmNativeLoadError::Resolution(_))
    ));
    assert!(output.take_output().unwrap().stdout.is_empty());
    let (provider, output) = command_sources(false);
    let mut command = module.prepare_command(provider);
    let mut completed = false;
    for _ in 0..10000 {
        match command.resume(NonZeroU64::MIN, &context, &policy).unwrap() {
            WasmCommandStep::Pending(next) => command = next,
            WasmCommandStep::Complete(result) => {

...
mod command_exit {
    ...
    fn signed(mut n: i32) -> Vec<u8> {
        ...
        loop {
            ...
            bytes.push(low | if done { 0 } else { 128 });
            if done {
                return bytes;
            }
        }
    ...
    fn section(bytes: &mut Vec<u8>, id: u8, data: &[u8]) {
        bytes.push(id);
        bytes.extend(leb(data.len() as u32));
    ...
    fn exit_code(code: u32, instruction: &[u8]) -> Vec<u8> {
        ...
        body.extend(signed(code as i32));
    ...
    fn binary(body: &[u8], startup: Option<&[u8]>, command_type: u8, export_name: &str) -> Vec<u8> {
        ...
        section(
            ...
            1,
            &[
                4, 0x60, 1, 0x7f, 0, 0x60, 0, 0, 0x60, 4, 0x7f, 0x7f, 0x7f, 0x7f, 1, 0x7f, 0x60, 0,
                1, 0x7f,
            ],
        );
        ...
        for body in [body, mutation].into_iter().chain(startup) {
            code.extend(leb(body.len() as u32 + 1));
            code.push(0);
    ...
    fn providers() -> (WasmHostImports, WasiStdio) {
        WasiPreview1Config::default()
            .into_imports_with_stdio(
                BTreeSet::from([VmDispatch, Builtin, Console]),
                Vec::new(),
                WasiStdioLimits::default(),
            )
            .unwrap()
    }
    fn exited(code: u32) -> WasmNumericVmError {
        WasmHostError::ProcessExit { code }.into()
    }
    fn emit() -> Vec<u8> {
        vec![0x41, 1, 0x41, 0, 0x41, 1, 0x41, 16, 0x10, 1, 0x1a]
    }
            ...
            + 'static,
    {
        ...
        imports
            .define(
                ...
                1,
                |_, _| panic!("unexpected output callback"),
            )
            .unwrap();
        imports
    }
    ...
    fn proc_exit_preserves_full_unsigned_status_and_never_returns_to_guest() {
        for code in [0, 7, 255, 256, 0x8000_0000, u32::MAX] {
            let mut body = exit_code(code, &[0x10, 0]);
    ...
    fn indirect_and_tail_exits_use_the_same_terminal_host_boundary() {
        for instruction in [
            ...
            let mut body = exit_code(19, &instruction);
            body.extend([0x00, 0x0b]);
    ...
    fn exit_retains_prior_output_and_stores_and_disables_other_exports() {
        let mut body = emit();
        body.extend([0x41, 7, 0x24, 0]);
        body.extend(exit_code(23, &[0x10, 0]));
        body.extend([0x41, 9, 0x24, 0, 0x0b]);
        let vm = vm(&body);
        ...
        for export in ["mutate", "_start"] {
            assert_eq!(instance.call_export(export, &[]), Err(exited(23)));
        }
        assert_eq!(
            instance.call_export("proc_exit", &[I32(99)]),
            Err(exited(23))
        );
        assert_eq!(instance.global_export("g"), Some(&I32(7)));
    ...
    fn command_return_is_zero_but_traps_are_not_normal_exit_statuses() {
        ...
        assert!(matches!(
            run_command(&vm(&[0x00, 0x0b]), providers().0),
            Err(WasmNumericVmError::Unreachable { .. })
        ));
        let imports =
            custom(|_, _| Err(WasmHostError::trap("process exited with status 0").into()));
        assert!(matches!(
            run_command(&vm(&[0x41, 0, 0x10, 0, 0x0b]), imports),
    ...
    fn command_contract_is_checked_before_binary_start_has_effects() {
        let mut startup = emit();
        startup.push(0x0b);
        for (ty, body, export) in [
            (0, vec![0x0b], "_start"),
            (3, vec![0x41, 0, 0x0b], "_start"),
            (1, vec![0x0b], "missing"),
        ] {
            let vm = WasmNumericVm::parse(
                &binary(&body, Some(&startup), ty, export),
                WasmNumericLimits::default(),
            )
            .unwrap();
            let (imports, output) = providers();
    ...
    fn exit_in_binary_start_ends_command_without_publishing_an_instance() {
        let mut startup = emit();
        startup.extend(exit_code(12, &[0x10, 0]));
        startup.extend([0x00, 0x0b]);
        let vm = WasmNumericVm::parse(
            &binary(&[0x00, 0x0b], Some(&startup), 1, "_start"),
            WasmNumericLimits::default(),
        )
        .unwrap();
        let (imports, output) = providers();
        ...
        assert_eq!(output.take_output().unwrap().stdout, b"bye");
        assert!(
            matches!(vm.instantiate_with_imports(providers().0), Err(error) if error == exited(12))
        );
    }
    ...
    fn sliced_exit_consumes_the_continuation_and_never_runs_following_effects() {
        ...
        assert!(
            matches!(call.resume(NonZeroU64::new(1).unwrap()), Err(error) if error == exited(3))
        );
    }
    ...
    fn recording_and_replay_restore_exit_without_invoking_providers_again() {
        let mut body = emit();
        body.extend(exit_code(31, &[0x10, 0]));
        body.extend([0x00, 0x0b]);
        let vm = vm(&body);
        ...
        assert_eq!(
            instance.memory_export("memory"),
            original.memory_export("memory")
        );
    ...
    fn cancellation_before_exit_wins_but_later_cancellation_cannot_replace_exit() {
        ...
        for exit_first in [false, true] {
            ...
            let (mut imports, _) = providers();
            imports
                .bind_execution_cancellation(token.clone(), "wasi-exit-precedence")
                .unwrap();
            let mut instance = vm.instantiate_with_imports(imports).unwrap();
            if exit_first {
                assert_eq!(
                    instance.call_export("proc_exit", &[I32(13)]),
                    Err(exited(13))
                );
            }
            token.cancel();
            ...
            assert_eq!(
                ...
                if exit_first {
                    ...
                    WasmHostError::ExecutionCancelled.into()
                }
            );
    ...
    fn resolved_execution_keeps_module_policy_checks_and_terminal_guest_state() {
        ...
        let bytes = binary(
            &[0x41, 14, 0x10, 0, 0x41, 9, 0x24, 0, 0x0b],
            None,
            1,
            "_start",
        );
        ...
        let mut instance = module
            .instantiate_with_imports(&context, &policy, providers().0)
            .unwrap();
        assert!(
            matches!(instance.call_export("_start", &[], &context, &policy),
            Err(WasmNativeLoadError::Execution(error)) if error == exited(14))
        );
        let denied = policy.clone().deny_specifier("/app/command.wasm");
        assert!(matches!(
            instance.call_export("mutate", &[], &context, &denied),
            Err(WasmNativeLoadError::Resolution(_))
        ));
        assert!(
            matches!(instance.call_export("mutate", &[], &context, &policy),
            Err(WasmNativeLoadError::Execution(error)) if error == exited(14))
        );
        assert_eq!(

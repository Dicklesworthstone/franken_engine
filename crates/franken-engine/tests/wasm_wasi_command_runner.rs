...
use frankenengine_engine::wasm_runtime_lane::wasi_preview1::{
    WASI_PREVIEW1_MODULE, WasiCommandOutcome, WasiCommandPhase, WasiPreview1Config, WasiStdio,
    WasiStdioLimits, run_command_with_outcome as run_command,
};
...
fn grants() -> BTreeSet<RuntimeCapability> {
    BTreeSet::from([Builtin, Console, VmDispatch])
}
...
fn context() -> ResolutionContext {
    ResolutionContext::new("wasi-command", "command-decision", "command-policy")
}
...
fn resolved(hex: &str, declare_services: bool) -> WasmNativeModule {
    ...
    let mut definition =
        ModuleDefinition::wasm_binary(&bytes(hex), &WasmNumericLimits::default()).unwrap();
    if declare_services {
...
fn raw_proc_exit_is_typed_and_does_not_execute_the_following_store() {
    ...
    assert_eq!(
        ...
        Err(WasmHostError::ProcessExit { code: 42 }.into())
    );
...
fn command_exit_preserves_all_u32_bits_including_nonzero_statuses() {
    for (hex, code) in [
        ...
        (EXIT_7, 7),
        (EXIT_42, 42),
        (EXIT_2147483648, 0x8000_0000),
        (EXIT_4294967295, u32::MAX),
    ] {
        assert_exit(
            run_command(&vm(hex), providers().0).unwrap(),
            code,
...
fn ordinary_return_has_zero_status_and_real_separate_startup_metrics() {
    for hex in [RETURN, RETURN_START] {
        ...
        let WasiCommandOutcome::Returned { startup, execution } = result else {
            panic!("normal return");
        };
        assert!(execution.results.is_empty());
        assert_eq!(
            execution.instructions_executed,
            if hex == RETURN { 3 } else { 5 }
        );
        assert_eq!(
            startup
                .as_ref()
                .map(|execution| execution.instructions_executed),
...
fn startup_exit_does_not_publish_an_instance_or_enter_the_command_export() {
    ...
    assert_exit(
        run_command(&vm(START_EXIT), imports).unwrap(),
        42,
    ...
    for hex in [BAD_PARAM, BAD_RESULT, MISSING_ENTRY, MEMORY_ENTRY] {
        ...
        assert!(matches!(
            ...
            WasmNumericVmError::InvalidModule { .. }
                ...
                | WasmNumericVmError::ExportIsNotFunction { .. }
        ));
...
fn exact_exit_import_abi_is_linked_before_startup() {
    ...
    imports
        .define(
            ...
            BTreeSet::from([Builtin]),
            1,
            |_, _| panic!("invalid provider must not execute"),
        )
        .unwrap();
    assert!(matches!(
        run_command(&vm(START_EXIT), imports),
...
fn exit_work_is_charged_before_it_can_be_classified_as_normal_termination() {
    ...
    .unwrap();
    assert_exit(
        run_command(&exact, providers().0).unwrap(),
        42,
        WasiCommandPhase::Command,
    );
    let tight = WasmNumericVm::parse(
        &bytes(EXIT_42),
        WasmNumericLimits {
            max_instructions: 3,
            ..WasmNumericLimits::default()
...
fn ordinary_traps_are_not_process_statuses_and_keep_completed_output() {
    assert!(matches!(
        run_command(&vm(TRAP), providers().0),
        Err(WasmNumericVmError::Unreachable { .. })
    ));
    let (imports, output) = providers();
    assert!(matches!(
        run_command(&vm(WRITE_TRAP), imports),
        Err(WasmNumericVmError::Unreachable { .. })
    ));
    assert_eq!(output.take_output().unwrap().stdout, b"hello".to_vec());
    let imports =
        custom_exit(|_, _| Err(WasmHostError::trap("WASI command exited with status 0").into()));
    assert!(matches!(
        run_command(&vm(EXIT_0), imports),
        Err(WasmNumericVmError::State(WasmStateError::Host(
            WasmHostError::Trap { .. }
        )))
    ));
...
fn live_cancellation_and_capability_revocation_win_over_a_provider_exit() {
    for revoke in [false, true] {
        ...
        if revoke {
            imports
                .bind_capability_revocation(Builtin, token, "exit-revocation")
                .unwrap();
        } else {
            imports
                .bind_execution_cancellation(token, "exit-cancellation")
                .unwrap();
        }
        assert!(matches!(
            run_command(&vm(EXIT_0), imports),
...
fn command_output_is_retained_once_after_exit() {
    let (imports, output) = providers();
    assert_exit(
        run_command(&vm(WRITE_EXIT), imports).unwrap(),
        7,
        WasiCommandPhase::Command,
    );
    assert_eq!(output.take_output().unwrap().stdout, b"hello".to_vec());
...
fn typed_exit_round_trips_through_the_host_transcript_without_duplicate_io() {
    for (hex, code, phase) in [
        (WRITE_EXIT, 7, WasiCommandPhase::Command),
        (START_EXIT, 42, WasiCommandPhase::Instantiation),
    ] {
        let vm = vm(hex);
...
fn a_command_requires_neither_process_io_nor_a_memory_export_to_exit() {
    assert_exit(
        run_command(&vm(NO_MEMORY), providers().0).unwrap(),
        42,
        WasiCommandPhase::Command,
    );
    ...
    assert!(matches!(
        machine.export_signature("memory"),
        Err(WasmNumericVmError::ExportIsNotFunction { .. })
    ));
}
...
fn every_terminal_command_outcome_releases_owned_providers() {
    ...
    for hex in [RETURN, EXIT_0, TRAP, BAD_PARAM] {
        ...
        let guard = Released(releases.clone());
...
fn resolved_commands_enforce_manifest_and_current_policy_before_startup() {
    ...
    let (imports, output) = providers();
    assert!(matches!(
        module.run_wasi_command(&context(), &policy(), imports),
    ...
    for denied in [
        CapabilityPolicyHook::new(BTreeSet::from([ModuleLoad, VmDispatch])),
        policy().deny_specifier("/app/command.wasm"),
    ] {
        let (imports, output) = providers();
        assert!(matches!(
            module.run_wasi_command(&context(), &denied, imports),
    ...
    assert_exit(
        module
            .run_wasi_command(&context(), &policy(), providers().0)
            .unwrap(),
        42,
        WasiCommandPhase::Instantiation,
    );
...
fn resolved_command_replay_remains_bound_to_the_pinned_module() {
    let module = resolved(WRITE_EXIT, true);
    let limits = WasmHostTraceLimits::default();
    let (mut imports, _) = providers();
    let recording = imports.record_calls(limits).unwrap();
    assert_exit(
        module
            .run_wasi_command(&context(), &policy(), imports)
            .unwrap(),
        7,
        WasiCommandPhase::Command,
    );
    let tape = recording.snapshot().unwrap();
    assert!(tape.module_hash().is_some());
    let (mut imports, output) = providers();
    let replay = imports.replay_calls(tape.clone(), limits).unwrap();
    assert_exit(
        module
            .run_wasi_command(&context(), &policy(), imports)
            .unwrap(),
    ...
    assert!(output.take_output().unwrap().stdout.is_empty());
    let (mut imports, _) = providers();
    imports.replay_calls(tape, limits).unwrap();
    assert!(
        resolved(EXIT_0, true)
            .run_wasi_command(&context(), &policy(), imports)
            .is_err()
    );
}

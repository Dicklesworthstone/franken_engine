...
fn scripted(limits: WasiRandomLimits) -> (WasmHostImports, Arc<AtomicUsize>) {
    ...
    let mut next = 0_u8;
    let imports = WasmHostImports::new(caps())
...
fn call(instance: &mut WasmNumericInstance<'_>, export: &str, pointer: i32, length: i32) -> i32 {
    let result = instance
        .call_export(export, &[I32(pointer), I32(length)])
        .unwrap();
    let [I32(code)] = result.results.as_slice() else {
        panic!("errno ABI");
    };
    *code
}
...
fn direct_indirect_and_exported_random_imports_fill_exact_binary_ranges() {
    ...
    for (index, export) in ["random", "direct", "indirect"].iter().enumerate() {
        ...
        assert_eq!(memory(&instance)[63], 0xaa);
        assert_eq!(memory(&instance)[129], 0xaa);
    }
...
fn no_default_source_and_no_builtin_to_entropy_authority_substitution() {
    ...
    let mut grants = caps();
    grants.insert(RuntimeCapability::Builtin);
    assert!(matches!(
        vm.instantiate_with_imports(WasiPreview1Config::default().into_imports(grants).unwrap()),
    ...
    let imports = WasmHostImports::new(BTreeSet::from([
    ...
    assert!(matches!(
        ...
        Err(WasmNumericVmError::State(WasmStateError::Host(
            WasmHostError::CapabilityDenied {
                capability: RuntimeCapability::RandomRead,
                ..
            }
        )))
    ));
...
fn empty_requests_need_existing_memory_but_no_source_or_lifetime_quota() {
    for has_memory in [false, true] {
        ...
        let (imports, count) = scripted(WasiRandomLimits {
            max_request_bytes: 0,
            max_total_bytes: 0,
            max_calls: 0,
            ..Default::default()
        ...
        let mut instance = vm.instantiate_with_imports(imports).unwrap();
        assert_eq!(
            call(&mut instance, "random", 0, 0),
            if has_memory { 0 } else { 21 }
        );
        if has_memory {
...
fn source_failure_discards_partial_scratch_and_consumes_attempt_quota() {
    ...
    for (source_error, expected) in [
        (WasiSourceError::Io, 29),
        (WasiSourceError::WouldBlock, 6),
        (WasiSourceError::Unsupported, 58),
    ] {
        let imports = WasmHostImports::new(caps())
            .with_wasi_random_source(
                move |bytes: &mut [u8]| {
...
fn rejected_shared_work_does_not_consume_source_quota_or_hide_vm_failure() {
    ...
    let (mut imports, count) = scripted(WasiRandomLimits {
        max_calls: 1,
        ..Default::default()
    });
    imports.bind_work_pool(pool.clone()).unwrap();
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    assert!(matches!(
        instance.call_export("random", &[I32(0), I32(65)]),
        Err(WasmNumericVmError::State(WasmStateError::Host(
            WasmHostError::WorkPool(_)
        )))
    ));
...
fn return_boundary_controls_cannot_be_converted_to_errno_or_publish_entropy() {
    for control in 0..3 {
        ...
        let mut imports = WasmHostImports::new(caps())
            .with_wasi_random_source(
                move |output: &mut [u8]| {
                    output.fill(99);
                    signal.cancel();
        ...
        match control {
            ...
            1 => imports
                .bind_execution_cancellation(token, "random")
                .unwrap(),
            _ => imports
                .bind_capability_revocation(RuntimeCapability::RandomRead, token, "random")
                .unwrap(),
        }
        let mut instance = vm.instantiate_with_imports(imports).unwrap();
        let before = memory(&instance).to_vec();
    ...
    let mut imports = WasmHostImports::new(caps())
        .with_wasi_random_source(
            move |output: &mut [u8]| {
                seen.fetch_add(1, Ordering::SeqCst);
                output.fill(17);
                let _delegated = source_pool.partition(source_pool.remaining()).unwrap();
                Ok(())
            },
            Default::default(),
        )
...
fn replay_restores_startup_and_later_entropy_without_entering_a_source() {
    ...
    let (mut imports, original_calls) = scripted(Default::default());
    let recording = imports.record_calls(limits).unwrap();
    let mut original = vm.instantiate_with_imports(imports).unwrap();
    assert_eq!(call(&mut original, "direct", 192, 65), 0);
    let (mut imports, replay_calls) = scripted(WasiRandomLimits {
        max_calls: 0,
        ..Default::default()
    });
    let replay = imports
    ...
    assert_eq!(call(&mut replayed, "direct", 192, 65), 0);
...
fn source_panic_does_not_publish_or_allow_interrupted_instance_reentry() {
    ...
    let imports = WasmHostImports::new(caps())
        .with_wasi_random_source(
            |bytes: &mut [u8]| -> Result<(), WasiSourceError> {
                bytes.fill(42);
    ...
    assert!(memory(&instance)[..512].iter().all(|byte| *byte == 0xaa));
    assert!(matches!(
        instance.call_export("random", &[I32(0), I32(1)]),
        Err(WasmNumericVmError::State(WasmStateError::Host(
...
fn resolved_manifest_and_current_policy_cannot_manufacture_entropy_authority() {
    ...
    let context = ResolutionContext::new("random", "decision", "policy");
    let mut manifest = wasm_module_required_capabilities();
    let mut grants = manifest.clone();
    grants.insert(RuntimeCapability::RandomRead);
    let policy = CapabilityPolicyHook::new(grants.clone());
    let mut resolver = DeterministicModuleResolver::new("/app");
    for declared in [false, true] {
        if declared {
            manifest.insert(RuntimeCapability::RandomRead);
        }
        let mut definition = ModuleDefinition::wasm_binary(&fixture(true, false), &limits).unwrap();
        definition.required_capabilities = manifest.clone();
        resolver
            .register_workspace_module("/app/random.wasm", definition)
            .unwrap();
        let module = resolver
            .load_wasm(
                &ModuleRequest::new("/app/random.wasm", ImportStyle::Import),
                &context,
                &policy,
                limits.clone(),
            )
            .unwrap();
        let (imports, count) = scripted(Default::default());
        let result = module.instantiate_with_imports(&context, &policy, imports);
        if !declared {
        ...
        denied
            ...
            .remove(&RuntimeCapability::RandomRead);
        assert!(matches!(
            instance.call_export("random", &[I32(0), I32(1)], &context, &denied),
            Err(WasmNativeLoadError::Resolution(_))
        ));
        assert_eq!(count.load(Ordering::SeqCst), 0);
        assert_eq!(
            instance
                .call_export("random", &[I32(0), I32(1)], &context, &policy)
                .unwrap()
                .results,
            [I32(0)]
        );
...
fn compiled_c_command_reads_entropy_and_emits_it_through_real_wasi_stdio() {
    let vm = WasmNumericVm::parse(
        include_bytes!("fixtures/wasi_random_smoke.wasm"),
        WasmNumericLimits::default(),
    )
    .unwrap();
    let mut grants = caps();
    grants.extend([RuntimeCapability::Console, RuntimeCapability::Builtin]);
    let (imports, output) = WasiPreview1Config::default()
        .into_imports_with_stdio(grants, Vec::new(), WasiStdioLimits::default())
        .unwrap();
    let imports = imports
        .with_wasi_random_source(
            |bytes: &mut [u8]| {
    ...
    assert_eq!(captured.stdout, (0..65).collect::<Vec<u8>>());

...
fn fixture(start: bool) -> Vec<u8> {
    ...
    section(
        ...
        1,
        &[
            3, 0x60, 4, 0x7f, 0x7f, 0x7f, 0x7f, 1, 0x7f, 0x60, 2, 0x7f, 0x7f, 0, 0x60, 0, 0,
        ],
    );
    ...
    for (index, function) in ["fd_write", "fd_read", "set", "write", "read"]
        .iter()
        .enumerate()
    {
    ...
    for code in [set.as_slice(), write.as_slice(), read.as_slice()]
        .into_iter()
...
fn set(instance: &mut WasmNumericInstance<'_>, address: i32, value: i32) {
    assert!(
        instance
            .call_export("set", &[I32(address), I32(value)])
            .unwrap()
            .results
            .is_empty()
    );
...
fn returned(instance: &WasmNumericInstance<'_>) -> u32 {
    u32::from_le_bytes(
        instance.memory_export("memory").unwrap()[24..28]
            .try_into()
            .unwrap(),
    )
}
...
fn stdin_scatter_reads_advance_only_the_supplied_cursor_and_report_eof() {
    ...
    assert_eq!(returned(&instance), 5);
    assert_eq!(output.input_consumed().unwrap(), 5);
    assert_eq!(
        &instance.memory_export("memory").unwrap()[128..133],
        b"abcde"
    );
    assert_eq!(call(&mut instance, "read", [0, 0, 2, 24]), 0);
    assert_eq!(returned(&instance), 1);
    assert_eq!(output.input_consumed().unwrap(), 6);
    assert_eq!(
        &instance.memory_export("memory").unwrap()[128..133],
        b"fbcde"
    );
...
fn budget_refusal_precedes_stream_effects_and_exact_charges_are_shared() {
    for (export, fd, budget) in [("fd_write", 1, 10), ("fd_read", 0, 8)] {
        ...
        assert!(matches!(
            instance.call_export(export, &[I32(fd), I32(0), I32(2), I32(24)]),
        ...
        assert_eq!(
            instance
                .call_export(export, &[I32(fd), I32(0), I32(2), I32(24)])
                .unwrap()
...
fn output_aliasing_a_source_buffer_cannot_change_already_gathered_bytes() {
    ...
    assert_eq!(
        &instance.memory_export("memory").unwrap()[128..132],
        &5_u32.to_le_bytes()
    );
}
...
fn input_overwriting_descriptor_memory_uses_a_validated_iovec_snapshot() {
    ...
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    set(&mut instance, 0, 8);
    set(&mut instance, 4, 4); // overwrite NEXT descriptor
    set(&mut instance, 8, 128);
    set(&mut instance, 12, 4);
    ...
    assert_eq!(returned(&instance), 8);
    assert_eq!(output.input_consumed().unwrap(), 8);
}
...
fn only_the_three_explicit_standard_stream_descriptors_exist() {
    ...
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    for fd in [0, 3, -1] {
        assert_eq!(call(&mut instance, "fd_write", [fd, 0, 2, 24]), 8);
    }
    for fd in [1, 2, 3, -1] {
        assert_eq!(call(&mut instance, "fd_read", [fd, 0, 2, 24]), 8);
    }
    assert_eq!(output.input_consumed().unwrap(), 0);
    assert_eq!(returned(&instance), 0x7777_7777);
}
...
fn revoked_input_capability_does_not_consume_stdin_or_disable_output() {
    ...
    assert!(matches!(
        instance.call_export("read", &[I32(0), I32(0), I32(2), I32(24)]),
...
fn separate_registries_never_share_stream_positions_or_output() {
    ...
    assert_eq!(a_out.input_consumed().unwrap(), 5);
    assert_eq!(b_out.input_consumed().unwrap(), 0);
    assert_eq!(a_out.take_output().unwrap().stdout, b"12345");
    assert!(b_out.take_output().unwrap().stdout.is_empty());
...
fn replay_restores_guest_buffers_without_reconsuming_or_reemitting_host_streams() {
    ...
    let replay = imports
        .replay_calls(recorder.snapshot().unwrap(), limits)
        .unwrap();
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    assert_eq!(call(&mut instance, "read", [0, 0, 2, 24]), 0);
    assert_eq!(call(&mut instance, "write", [1, 0, 2, 24]), 0);
...
fn resolver_policy_and_manifest_bound_the_standard_services_before_execution() {
    ...
    let request =
        ModuleRequest::new("./command.wasm", ImportStyle::Import).with_referrer("/app/main.mjs");
    ...
    let mut definition =
        ModuleDefinition::wasm_binary(&fixture(false), &WasmNumericLimits::default()).unwrap();
    ...
    resolver
        ...
        .unwrap();
    resolver
        .register_workspace_module("/app/command.wasm", definition.clone())
        .unwrap();
    let module = resolver
        .load_wasm(&request, &context, &policy, WasmNumericLimits::default())
        .unwrap();
    let (imports, output) = providers(b"abcde", WasiStdioLimits::default());
    let mut instance = module
        .instantiate_with_imports(&context, &policy, imports)
        .unwrap();
    let mut revoked = policy.clone();
    revoked
        .granted_capabilities
        .remove(&RuntimeCapability::Console);
    assert!(matches!(
        instance.call_export(
            "write",
            &[I32(1), I32(0), I32(2), I32(24)],
            &context,
            &revoked
        ),
    ...
    let (imports, output) = providers(&[], WasiStdioLimits::default());
    assert!(matches!(
        module.instantiate_with_imports(&context, &policy, imports),
...
fn echo_module() -> Vec<u8> {
    ...
    name(&mut exports, "echo");
    exports.extend([0, 2]);
    name(&mut exports, "memory");
    exports.extend([2, 0]);
    section(&mut bytes, 7, &exports);
    let code = [
        1, 1, 0x7f, // one errno local
        0x02, 0x40, 0x03, 0x40, 0x41, 0, 0x41, 0, 0x41, 1, 0x41, 24, 0x10, 1, 0x22, 0, 0x04, 0x40,
        0x20, 0, 0x0f, 0x0b, 0x41, 24, 0x28, 2, 0, 0x45, 0x0d, 1, // leave on EOF
        0x41, 4, 0x41, 24, 0x28, 2, 0, 0x36, 2, 0, // write exactly nread
        0x41, 1, 0x41, 0, 0x41, 1, 0x41, 24, 0x10, 0, 0x22, 0, 0x04, 0x40, 0x20, 0, 0x0f, 0x0b,
    ...
    section(
        &mut bytes,
        11,
        &[1, 0, 0x41, 0, 0x0b, 8, 128, 0, 0, 0, 64, 0, 0, 0],
    );
    bytes
}
...
fn guest_echo_returns_capture_errors_instead_of_silently_dropping_output() {
    ...
    let mut instance = vm.instantiate_with_imports(imports).unwrap();
    assert_eq!(
        instance.call_export("echo", &[]).unwrap().results,
        [I32(51)]
    );

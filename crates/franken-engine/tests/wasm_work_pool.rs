...
fn fixture(start: bool, host: bool, large_locals: bool) -> Vec<u8> {
    ...
    if host {
        section(&mut out, 2, &[1, 1, b'h', 1, b'f', 0, 1]);
    }
    ...
    if start {
        section(&mut out, 8, &[base + 2]);
    }
    let mut unit = if large_locals {
        let mut v = vec![1];
        v.extend(leb(300));
        v.push(0x7f);
        v
...
{
    let mut registry =
        WasmHostImports::new([RuntimeCapability::Builtin, RuntimeCapability::VmDispatch].into());
    registry.bind_work_pool(pool.clone()).unwrap();
    registry
        .define(
            "h",
...
fn depleted(result: Result<impl std::fmt::Debug, WasmNumericVmError>) -> WasmWorkPoolExhausted {
    match result {
        Err(WasmNumericVmError::State(WasmStateError::Host(WasmHostError::WorkPool(error)))) => {
            error
        }
...
fn local_budget_refusal_does_not_debit_a_rejected_shared_charge() {
    ...
    let mut instance = vm.instantiate_with_imports(imports(&pool)).unwrap();
    assert!(matches!(
        instance.call_export("inc", &[]),
        Err(WasmNumericVmError::InstructionBudgetExceeded { max: 2 })
    ));
            ...
            threads.push(scope.spawn(move || {
                ...
                for _ in 0..250 {
                    match instance.call_export("unit", &[]) {
                        Ok(execution) => {
                            assert_eq!(execution.instructions_executed, 1);
                            success += 1;
                        }
                        error => {
                            assert_eq!(depleted(error).remaining, 0);
                        }
                    }
                }
                success
...
fn registry_cannot_replace_its_quota_before_linking() {
    let first = WasmWorkPool::new(1);
    let other = WasmWorkPool::new(100);
    let mut registry = imports(&first);
    assert!(matches!(
        registry.bind_work_pool(other.clone()),
        Err(WasmHostError::WorkPoolAlreadyBound)
    ));
    let vm = vm(false, false, false, 100);
...
fn host_fixed_charge_refuses_before_provider_entry_without_partial_debit() {
    ...
    let registry = host_imports(&pool, move |_, _| {
        counter.fetch_add(1, Ordering::SeqCst);
        Ok(vec![])
    });
    let mut instance = vm.instantiate_with_imports(registry).unwrap();
    assert_eq!(
        depleted(instance.call_export("host", &[])),
        WasmWorkPoolExhausted {
            requested: 5,
            remaining: 4
        }
    );
...
fn bulk_memory_work_refuses_atomically_and_keeps_unused_units() {
    ...
    let mut instance = vm.instantiate_with_imports(imports(&pool)).unwrap();
    assert_eq!(
        depleted(instance.call_export("fill", &[])),
        WasmWorkPoolExhausted {
            requested: 2,
            remaining: 1
        }
...
fn cancellation_and_slice_drop_do_not_reset_or_refund_shared_work() {
    ...
    registry
        .bind_execution_cancellation(signal.clone(), "quota-cancel")
        .unwrap();
    let mut instance = vm.instantiate_with_imports(registry).unwrap();
    let call = instance.begin_call("inc", &[]).unwrap();
    let WasmCallStep::Pending(call) = call.resume(NonZeroU64::new(4).unwrap()).unwrap() else {
        panic!("pending store");
    };
    assert_eq!(pool.remaining(), 96);
    signal.cancel();
    assert!(matches!(
        call.resume(NonZeroU64::new(4).unwrap()),
    ...
    let WasmCallStep::Pending(call) = instance
        .begin_call("inc", &[])
        .unwrap()
        .resume(NonZeroU64::new(2).unwrap())
        .unwrap()
    else {
        panic!("pending");
    };
...
fn replay_must_pay_live_quota_before_effects_without_reentering_providers() {
    ...
    let mut registry = host_imports(&pool, |caller, _| {
        caller.charge_work(3)?;
        caller.write_memory(0, b"ok")?;
        Ok(vec![])
    });
...
mod scoped {
    ...
    fn context() -> ResolutionContext {
        ResolutionContext::new("quota-trace", "quota-decision", "quota-policy")
    }
    ...
    fn load(start: bool) -> WasmNativeModule {
        ...
        resolver
            .register_workspace_module(
                "/app/quota.wasm",
                ModuleDefinition::wasm_binary(&fixture(start, false, false), &limits).unwrap(),
            )
            .unwrap();
        resolver
            .load_wasm(
                &ModuleRequest::new("/app/quota.wasm", ImportStyle::Import),
                &context(),
                &policy(),
                limits,
            )
    ...
    fn current_policy_denial_and_lazy_preparation_do_not_spend_or_replace_fuel() {
        ...
        let WasmCommandStep::Pending(command) =
            command.resume(work(100), &context(), &policy()).unwrap()
        else {
            ...
            loop {
                match command.resume(work(1), &context(), &policy()).unwrap() {
                    ...
                    WasmCommandStep::Complete(done) => {
                        ...
                        assert_eq!(done.instructions_executed, 10);
                        break;
                    }
                }
            }
    ...
    fn synchronous_and_future_commands_cannot_reset_fuel_between_phases() {
        ...
        for asynchronous in [false, true] {
            ...
            let result = if asynchronous {
                drive(
                    module.run_wasi_command_cooperatively(imports(&pool), work(1), || {
                        Ok((context(), policy()))
                    }),
                )
            } else {
    ...
    fn future_startup_yields_without_refilling_and_preserves_fuel_after_drop() {
        ...
        let pool = WasmWorkPool::new(20);
        let mut future =
            Box::pin(vm.instantiate_cooperatively_with_imports(imports(&pool), work(2)));
        ...
        assert_eq!(pool.remaining(), 18);
        let instance =
            drive(vm.instantiate_cooperatively_with_imports(imports(&pool), work(1))).unwrap();
        assert_eq!(instance.start_execution().unwrap().instructions_executed, 5);
    ...
    fn scheduler_withdrawal_requeue_and_task_cancellation_never_refund_spending() {
        ...
        let old = scheduler
            .submit_command(module.prepare_command(imports(&pool)))
            .unwrap();
        assert!(matches!(
            scheduler
                .run_next(work(2), &context(), &policy())
                .unwrap()
                .outcome,
        ...
        assert!(matches!(
            scheduler
                .run_next(work(1), &context(), &policy())
                .unwrap()
                .outcome,
            WasmTaskOutcome::Pending
        ));
    ...
    fn depleted_tenant_tasks_cannot_stop_an_independently_budgeted_command() {
        let module = load(false);
        let depleted_pool = WasmWorkPool::new(0);
        let healthy_pool = WasmWorkPool::new(5);
        let mut scheduler = WasmNativeScheduler::new(NonZeroUsize::new(2).unwrap());
        let denied = scheduler
            .submit_command(module.prepare_command(imports(&depleted_pool)))
            .unwrap();
        let healthy = scheduler
            .submit_command(module.prepare_command(imports(&healthy_pool)))
            .unwrap();
        let mut failed = false;
        let mut completed = false;
        for _ in 0..50 {
            let Some(turn) = scheduler.run_next(work(1), &context(), &policy()) else {
                break;
            };
            match turn.outcome {
                WasmTaskOutcome::Pending => {}
                WasmTaskOutcome::Failed(WasmNativeLoadError::Execution(
                    WasmNumericVmError::State(WasmStateError::Host(WasmHostError::WorkPool(_))),
                )) => {
                    assert_eq!(turn.task_id, denied.id());
                    failed = true;
                }
                WasmTaskOutcome::Exited(execution) => {
                    assert_eq!(turn.task_id, healthy.id());
    ...
    fn memory_capacity_returns_on_destruction_but_work_capacity_does_not() {
        ...
        let mut registry = imports(&work);
        registry.bind_memory_pool(memory.clone()).unwrap();
        let mut instance = vm.instantiate_with_imports(registry).unwrap();
        instance.call_export("inc", &[]).unwrap();
        ...
        let mut registry = imports(&work);
        registry.bind_memory_pool(memory).unwrap();
    ...
    fn typed_process_exit_keeps_all_prior_metered_work_spent() {
        ...
        assert!(matches!(
            instance.call_export("host", &[]),
            Err(WasmNumericVmError::State(WasmStateError::Host(
                WasmHostError::ProcessExit { code: u32::MAX }
            )))
        ));
        assert_eq!(pool.remaining(), 92);

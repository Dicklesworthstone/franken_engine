...
impl JournalBudget {
    ...
    pub(super) fn check_count(&self, count: usize) -> Result<(), HostEffectJournalError> {
        if count > self.limits.max_entries {
            return Err(HostEffectJournalError::Lifecycle {
                detail: format!("journal entry capacity exceeded (limit={})", self.limits.max_entries),
            });
        }
        Ok(())
    }
    ...
    pub(super) fn prepare_replacement<T: Serialize>(
        ...
        let retained = self.retained_bytes.checked_sub(previous_charge).ok_or_else(|| {
            HostEffectJournalError::Lifecycle {
                detail: "journal accounting does not cover its reservation".to_string(),
            }
        })?;
        let available = self.limits.max_encoded_bytes.checked_sub(retained).ok_or_else(|| {
            HostEffectJournalError::Lifecycle {
                detail: "journal retained bytes exceed configured capacity".to_string(),
            }
        })?;
        let mut counter = BoundedCounter { remaining: available, written: 0, exceeded: false };
        if serde_json::to_writer(&mut counter, payload).is_err() {
            return Err(HostEffectJournalError::Lifecycle {
                detail: if counter.exceeded {
                    format!("journal byte capacity exceeded (limit={})", self.limits.max_encoded_bytes)
                } else {
        ...
        Ok(Admission { charge: counter.written, retained_after: retained + counter.written })
    }
...
impl RequestRef<'_> {
    ...
    pub(super) fn matches(self, owned: &HostEffectJournalRequest) -> bool {
        match (self, owned) {
            (Self::HostIo(actual), HostEffectJournalRequest::HostIo(expected)) => actual == expected,
            (Self::ProcessSpawn(actual), HostEffectJournalRequest::ProcessSpawn(expected)) => actual == expected,
            _ => false,
        }
    }
...
pub(super) enum EntryRef<'a> {
    HostIo { request: &'a HostIoRequest, outcome: &'a HostIoOutcome },
    ProcessSpawn { request: &'a ProcessSpawnRequest, outcome: &'a ProcessSpawnOutcome },
}
    ...
    pub(super) fn into_owned(self) -> HostEffectJournalEntry {
        match self {
            Self::HostIo { request, outcome } => HostEffectJournalEntry::HostIo {
                request: request.clone(), outcome: outcome.clone(),
            },
...
mod tests {
    ...
    fn request() -> HostIoRequest {
        HostIoRequest::FsRead { path: "input.txt".into() }
    }

    fn outcome() -> HostIoOutcome {
        Ok(HostIoResponse::FsRead { bytes: b"data".to_vec() })
    }

    fn process_request() -> ProcessSpawnRequest {
        ProcessSpawnRequest::Run {
            launch: ProcessLaunch {
                executable: "/usr/bin/true".into(), argv: Vec::new(), env: BTreeMap::new(),
                cwd: None, shell: false, stdio: ProcessStdio::default(),
            },
            stdin: Vec::new(), timeout_millis: Some(100),
        }
    }

    fn limits(entries: usize, bytes: usize) -> HostEffectJournalLimits {
        HostEffectJournalLimits { max_entries: entries, max_encoded_bytes: bytes }
    }
    ...
    fn borrowed_entries_match_owned_wire_bytes_for_both_families_and_failures() {
        let request = request();
        for outcome in [outcome(), Err(HostIoError::Denied { reason: "denied\n\"quoted\"".into() })] {
            let borrowed = EntryRef::HostIo { request: &request, outcome: &outcome };
            assert_eq!(serde_json::to_vec(&borrowed).unwrap(), serde_json::to_vec(&borrowed.into_owned()).unwrap());
        }
        let request = process_request();
        let outcome = Ok(ProcessSpawnResponse::Run {
            exit: ProcessExit { success: true, code: Some(0), signal: None },
            stdout: vec![0, 127, 255], stderr: Vec::new(),
        });
        let borrowed = EntryRef::ProcessSpawn { request: &request, outcome: &outcome };
        assert_eq!(serde_json::to_vec(&borrowed).unwrap(), serde_json::to_vec(&borrowed.into_owned()).unwrap());
    }
    ...
    fn exact_byte_limit_allows_completion_and_replay_without_double_charging() {
        ...
        let replay = InMemoryHostEffectJournal::replaying_with_limits(entries.clone(), limits(1, entry_size())).unwrap();
    ...
    fn request_admission_obeys_exact_encoded_boundary() {
        ...
        for max_bytes in [size - 1, size] {
            ...
            journal.begin_execution().unwrap();
            assert_eq!(journal.reserve_host_io(&request()).is_ok(), max_bytes == size);
            assert_eq!(journal.retained_encoded_bytes(), if max_bytes == size { size } else { 0 });
        }
    }
    ...
    fn oversized_outcome_retains_a_hole_and_does_not_hide_later_completions() {
        ...
        let large = Ok(HostIoResponse::FsRead { bytes: vec![255; entry_size() * 4] });
        let error = journal.complete_host_io(first, &request(), &large).unwrap_err();
        assert_eq!(journal.retained_encoded_bytes(), before);
        journal.complete_host_io(second, &request(), &outcome()).unwrap();
        assert!(journal.attempt_entries().is_empty());
        assert!(matches!(journal.attempt_records().as_slice(), [
    ...
    fn process_rebinding_replaces_the_charge_instead_of_accumulating_it() {
        ...
        for _ in 0..10 {
            reservation = journal.bind_prepared_process_spawn(reservation, &original).unwrap();
        ...
        assert!(journal.bind_prepared_process_spawn(reservation, &enlarged).is_err());
        assert_eq!(journal.retained_encoded_bytes(), size);
    ...
    fn replay_rejects_oversized_inputs_before_becoming_usable() {
        let entry = HostEffectJournalEntry::HostIo { request: request(), outcome: outcome() };
        for policy in [limits(0, entry_size()), limits(1, entry_size() - 1)] {
            assert!(InMemoryHostEffectJournal::replaying_with_limits(vec![entry.clone()], policy).is_err());
        }
    }

    #[test]
    fn concurrent_reservation_admission_cannot_overrun_the_entry_limit() {
        let journal = Arc::new(InMemoryHostEffectJournal::recording_with_limits(limits(3, 100_000)));
        journal.begin_execution().unwrap();
        let reservations: Vec<_> = std::thread::scope(|scope| {
            ...
            handles.into_iter().filter_map(|handle| handle.join().unwrap().ok()).collect()
        });
        assert_eq!(reservations.len(), 3);
        for reservation in reservations {
            journal.complete_host_io(reservation, &request(), &outcome()).unwrap();
        }
    ...
    fn rejected_measurement_does_not_mutate_accounting() {
        ...
        assert!(budget.prepare_replacement(0, &"much too large a payload").is_err());

...
| `FE-CLAIM-025` | evidence | `README.md:2482` | `observed` | CEI H.2 reflexive soundness: the integrity capstone composes the A.1/A.3 lattice + H.1 Merkle ledger + wording gate + D.3 Test262 posture; 025's own row is ledger-committed, the A.5 adversarial corpus rejects over-promotion fixtures, and the G.3 no-mock drill reddens the capstone on any injected over-promotion; H.4 machine-checks the state<=ceiling(tier) meta-soundness lemma in Lean 4 (proofs/lean4/ClaimEvidenceSoundness.lean, scripts/run_cei_soundness_lean_proof.sh, sorryAx-free) | `bd-sde5e.8.2` |

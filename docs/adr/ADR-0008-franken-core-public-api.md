...
- `evidence_ledger`: re-export `SchemaVersion`; constants `LEGACY_UNSIGNED_EVIDENCE_SCHEMA_IDENTIFIER`, `EVIDENCE_LEDGER_STITCHING_BEAD_ID`, `EVIDENCE_LEDGER_STITCHING_COMPONENT`, `EVIDENCE_LEDGER_GRAPH_SCHEMA_VERSION`, `DECISION_SEMANTICS_LOG_SCHEMA_VERSION`, `ARTIFACT_LINEAGE_INDEX_SCHEMA_VERSION`, `EVIDENCE_QUERY_SURFACE_SCHEMA_VERSION`, `EVIDENCE_LEDGER_STITCHING_BUNDLE_SCHEMA_VERSION`, `EVIDENCE_LEDGER_STITCHING_TRACE_IDS_SCHEMA_VERSION`, `EVIDENCE_LEDGER_STITCHING_RUN_MANIFEST_SCHEMA_VERSION`; traits `SchemaVersionExt`, `EvidenceEmitter`; enums `DecisionType`, `EvidenceAuthorityClass`, `LedgerError`, `EvidenceGraphNodeKind`, `EvidenceGraphEdgeKind`; structs `CandidateAction`, `Constraint`, `Witness`, `ChosenAction`, `EvidenceKeyProvenance`, `EvidenceVerificationIdentity`, `RuntimeEvidenceAuthority`, `LabEvidenceAuthority`, `EvidenceSignatureEnvelope`, `LegacyUnsignedEvidenceEntryV1`, `EvidenceEntry`, `VerifiedEvidenceEntry`, `EvidenceEntryBuilder`, `InMemoryLedger`, `DecisionSemanticsAnnotations`, `ArtifactRecord`, `EvidenceGraphNode`, `EvidenceGraphEdge`, `EvidenceLedgerGraph`, `DecisionSemanticsRecord`, `ArtifactLineageRecord`, `EvidenceQueryRecord`, `EvidenceQuerySurfaceSnapshot`, `EvidenceLedgerStitchingBundle`, `StitchingTraceIdsArtifact`, `StitchingStructuredLogEvent`, `StitchingArtifactContext`, `StitchingBundleWriteReport`; functions `current_schema_version`, `render_stitching_summary`, `emit_default_stitching_bundle`.
...
`evidence_ledger::EvidenceEntry` is the authenticated schema-v2 surface.
Production builders, ledgers, and orchestrators require an explicit
`RuntimeEvidenceAuthority`; deterministic identities are available only through
lab-labelled APIs. Historical schema-v1 JSON is accepted solely by
`LegacyUnsignedEvidenceEntryV1::parse_json`, which returns an untrusted legacy
type and provides no conversion into authenticated evidence. Ledger storage and
normal orchestrator results expose non-deserializable `VerifiedEvidenceEntry`
values created only after validation against a separately supplied trust root.

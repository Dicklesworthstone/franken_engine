...
jq -e '
  ...
  .diagnostic.error_code == "OK" and
  .diagnostic.fallback_mode == "execute_native_react_pipeline" and
  ...
  .diagnostic.verification_lane == "react_compile_contract" and
  .compilation.language == "tsx" and
  .compilation.runtime_mode == "automatic" and
  (.compilation.generated_code | contains("main")) and
  .compilation.receipt.component == "react_compilation_pipeline"
' "${report_path}" >/dev/null

echo "verified react compile contract report:"
jq '{support_status, blocked, capability_id, generated_code: .compilation.generated_code, error_code: .diagnostic.error_code}' "${report_path}"

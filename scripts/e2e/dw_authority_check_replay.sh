#!/usr/bin/env bash
# dw_authority_check_replay.sh - replay verifier for the E5 authority/intake
# analyzer capstone bundle (bd-fqlfw.5.5).
#
...
case "$mode" in
  bundle)
    ...
    ;;
  rerun)
    DW_ARTIFACT_ROOT="artifacts/$CAP/replay" "./scripts/run_dw_authority_check.sh" ci "$@"
    ;;

#!/usr/bin/env bash
# dw_proof_spine_replay.sh - replay verifier for the E6 proof-spine capstone
# bundle (bd-fqlfw.6.6).
#
...
verify_bundle() {
  ...
  # When the live leg ran, the preserved FE-CLAIM-016 proof.json must still
  # bind exactly FE-CLAIM-016 (a fabricated v2-deferred claim id in a
  # preserved bundle is a tamper signal, not a formatting nit).
  local artifact="$dir/proof_spine_e2e/FE-CLAIM-016.proof.json"
  if [[ -f "$artifact" ]]; then
    local claims
    claims=$(jq -c '.claim_ids' "$artifact")
    if [[ "$claims" != '["FE-CLAIM-016"]' ]]; then
      echo "replay: preserved proof.json claim_ids drifted: $claims" >&2
      exit 1
    fi
  fi
  local outcome
...
case "$mode" in
  bundle)
    ...
    ;;
  rerun)
    DW_ARTIFACT_ROOT="artifacts/$CAP/replay" "./scripts/run_dw_proof_spine.sh" ci "$@"
    ;;

...
if ! command -v rch >/dev/null 2>&1; then
  echo "resource_certificate_formal_governance: rch is required for Cargo proof steps" >&2
  exit 2
fi
...
run_step "unit-formal-policy" "cargo-test-unit-formal-policy" cargo test -p frankenengine-engine --lib formal_policy -- --nocapture

run_step "integration-formal-policy" "cargo-test-integration-formal-policy" cargo test -p frankenengine-engine --test resource_certificate_governance_integration formal_policy -- --nocapture

run_step "enrichment-formal-policy" "cargo-test-enrichment-formal-policy" cargo test -p frankenengine-engine --test resource_certificate_governance_enrichment_integration formal_policy -- --nocapture

#!/usr/bin/env bash
# RGC signed-decision-receipt gate replay wrapper (bd-cixqu.1.4)
#
# Re-runs the FE-CLAIM-004 gate at
# scripts/run_rgc_signed_decision_receipt.sh against the pinned-or-latest
# previous bundle and compares verdict + the content-addressed required-surface
# set (sha256 per artifact).
#
# Source-bundle selection:
#   - RGC_SIGNED_DECISION_RECEIPT_REPLAY_RUN_DIR=<dir>  (pinned), else
#   - the latest complete bundle under the artifact root (auto-detect).
# Fails closed (exit 1) if no complete bundle is available.
#
# Usage:
#   scripts/e2e/rgc_signed_decision_receipt_replay.sh [ci|selftest]
#   RGC_SIGNED_DECISION_RECEIPT_REPLAY_RUN_DIR=path \
#     scripts/e2e/rgc_signed_decision_receipt_replay.sh
#
# Exit codes:
...
readonly MANIFEST_SCHEMA="franken-engine.rgc-signed-decision-receipt-gate.manifest.v1"
...
readonly PIN="${RGC_SIGNED_DECISION_RECEIPT_REPLAY_RUN_DIR:-}"
...
if ! RGC_SIGNED_DECISION_RECEIPT_REPLAY_RUN_DIR="${replay_run_dir}" \
...
jq -n \
  --arg schema "franken-engine.rgc-signed-decision-receipt-gate.replay-report.v1" \
...
{
  printf -- '# RGC signed-decision-receipt gate replay — %s\n\n' "${REPLAY_TS}"
  printf -- '- Mode: `%s`\n' "${mode}"
...
echo "rgc_signed_decision_receipt_replay_report=${REPLAY_DIR}/comparison_report.json"
echo "rgc_signed_decision_receipt_replay_summary=${REPLAY_DIR}/replay_summary.md"

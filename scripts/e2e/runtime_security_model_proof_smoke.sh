...
run_id="${RUNTIME_SECURITY_MODEL_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
run_dir="${RUNTIME_SECURITY_MODEL_RUN_DIR:-${artifact_root}/${run_id}}"
...
run_step() {
  ...
  if [[ "${exit_code}" -eq 0 ]]; then
    echo "runtime_security_model: ${step_id} passed (${duration_ms}ms)"
    record_event \
      "runtime_security_model.${step_id}" \
      "${step_id}" \
      "${command_id}" \
      "${log_path}" \
      "${exit_code}" \
    ...
    echo "runtime_security_model: ${step_id} failed (${duration_ms}ms); see ${log_path}" >&2
    ...
    record_event \
      ...
      "${duration_ms}" \
      "failed" \
      "Fix the failing invariant test or downgrade the matching claim wording."
  fi
}
...
jq -n \
  --arg schema_version "franken-engine.runtime-security-model-proof-smoke.v1" \
  --arg status "${status}" \
  --arg model_doc "docs/FORMAL_RUNTIME_SECURITY_MODEL_V1.md" \
  --arg ifc_module "crates/franken-engine/src/ifc_artifacts.rs" \
  --arg capability_module "crates/franken-engine/src/capability.rs" \
  ...
  '{
    ...
    model_doc: $model_doc,
    checked_modules: [$ifc_module, $capability_module],
    cargo_target_dir: $target_dir,
    checked_properties: [
      "IFC join/meet lattice laws",
      "IFC computed-label join upper bound",
      "IFC flow policy fail-closed precedence",
      "Capability subsumption and attenuation algebra",
      "Canonical authority partitions remain pairwise disjoint",
      "Unknown capability tags do not create authority"
    ]
  }' >"${source_report_path}"

proof_contract_write_standard_bundle \
  "${run_dir}" \
  "runtime_security_model_proof_smoke" \
  "${status}" \
  ...
  "${commands_path}" \
  "bd-1uk7a" \
  "FE-CLAIM-016,FE-CLAIM-015,FE-CLAIM-006" \
  "${failure_count}"

echo "runtime_security_model_manifest=${run_dir}/manifest.json"

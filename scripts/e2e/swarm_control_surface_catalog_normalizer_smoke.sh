...
contract_path="${root_dir}/docs/swarm_control_surface_catalog_contract_v1.json"
docs_path="${root_dir}/docs/SWARM_CONTROL_SURFACE_CATALOG.md"
fixtures_path="${root_dir}/scripts/testdata/swarm_control_surface_catalog/cases.json"
mode="${1:-check}"

record_pass() {
  printf 'PASS swarm-control-surface-catalog-normalizer %s\n' "$1"
}

record_failure() {
  printf 'FAIL swarm-control-surface-catalog-normalizer %s\n' "$1" >&2
  exit 1
}

usage() {
  cat >&2 <<'EOF'
Usage: ./scripts/e2e/swarm_control_surface_catalog_normalizer_smoke.sh [check|selftest]
EOF
}
...
run_real_contract_check() {
  ...
  tmp_root="${TMPDIR:-/tmp}/franken-engine-swarm-control-surface-catalog-real/$USER-$$-$(date -u +%Y%m%dT%H%M%SZ)"
  ...
  jq -e '
    .surface_count >= 13
    and (.decision == "pass" or .decision == "degraded")
    and (.artifact_paths.swarm_control_surface_catalog_json | test("swarm_control_surface_catalog.json$"))
  ' "${output_dir}/swarm_control_surface_catalog.json" >/dev/null \
    || record_failure "real contract catalog shape mismatch"
  jq empty "${output_dir}/catalog_findings.json" "${output_dir}/swarm_control_surface_catalog.json"
...
run_fixture_case() {
  ...
  tmp_root="${TMPDIR:-/tmp}/franken-engine-swarm-control-surface-catalog-fixtures/$USER-$$-$(date -u +%Y%m%dT%H%M%SZ)/${case_id}"
  ...
  jq -e --arg decision "$expected_decision" '.decision == $decision' "${output_dir}/swarm_control_surface_catalog.json" >/dev/null \
  ...
  jq empty "${output_dir}/swarm_control_surface_catalog.json" "${output_dir}/catalog_findings.json"
  [[ -s "${output_dir}/events.jsonl" ]] || record_failure "${case_id} missing events"
  grep -Fq './scripts/swarm_control_surface_catalog_normalizer.sh' "${output_dir}/commands.txt" \
...
run_check() {
  ...
  jq -e '.required_surface_fields | index("surface_id") and index("validation_commands")' "$contract_path" >/dev/null
  jq -e '.cases | length == 6' "$fixtures_path" >/dev/null
  grep -Fq "Any heavy Cargo example must start with \`rch exec -- env CARGO_TARGET_DIR=\`." "$docs_path" \

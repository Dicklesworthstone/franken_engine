...
# PERF-H6.4 (bd-o4cbn.4.4): End-to-end smoke for the H6 capacity-hint sweep.
#
# This is the lightweight E2E companion to the heavy statistical gate in
# `scripts/perf/h6_bench_validate.sh` (PERF-H6.3, bd-o4cbn.4.3). Same shape as
# the other per-track perf smokes: it proves the H6 pipeline is wired and
# behaves, then points at the validation script for the binding combined-drop
# number. A smoke answers "does the optimised path build, run, and keep its
# observable output?" — NOT "is the speedup statistically a win?" (that is the
...
#                  flags (capacity-hint sweep is in the build).
#   2. BENCH     — the two H6 target sub-benches (iterator_protocol_trace,
#                  scheduler_queue_commit) execute under a short Criterion
#                  budget and emit finite timings (the hot paths are live).
#   3. OUTPUT    — the H6.5 public-API golden test
#                  (perf_h6_capacity_hints_integration) passes: `frankenctl
#                  run` output is byte-identical to the pre-H6 golden, proving
#                  capacity hints are pure metadata.
#
# Emits under tests/artifacts/perf/h6_smoke/<ts>/ (gitignored — local evidence):
#   - build.log / bench_smoke.txt / integration.txt  raw logs
...
#
# Modes:
#   (default)       full smoke: build + bench-smoke + integration golden.
#   --quick         skip the release bench build; only the integration golden
#                   test + structural checks (still proves output-unchanged).
...
# Acceptance (bd-o4cbn.4.4): smoke is reproducible, records a run-id, and the
# binding combined-drop is produced by `scripts/perf/h6_bench_validate.sh`.
#
# Usage: scripts/perf/run_perf_h6_smoke.sh [--quick|--self-check]
...
INTEGRATION_TEST="perf_h6_capacity_hints_integration"
VALIDATE_SCRIPT="scripts/perf/h6_bench_validate.sh"
...
RUN_DIR="tests/artifacts/perf/h6_smoke/${RUN_TS}"
mkdir -p "$RUN_DIR"
echo "[h6.4] mode=$MODE run dir: $RUN_DIR"
...
PREREQS=(
    ...
    "crates/franken-engine/tests/fixtures/perf_h6/input.js"
    "crates/franken-engine/tests/fixtures/perf_h6/run_output.golden.json"
    "$VALIDATE_SCRIPT"
)
...
for p in "${PREREQS[@]}"; do
    if [[ ! -e "$p" ]]; then
        echo "[h6.4] MISSING prerequisite: $p" >&2
        PREREQ_OK=0
    fi
done
# The two target sub-benches must be present in the bench source.
for tgt in "${TARGETS[@]}"; do
    if ! grep -q "\"$tgt\"" "crates/franken-engine/benches/hot_paths.rs"; then
        echo "[h6.4] target sub-bench not found in bench source: $tgt" >&2
        PREREQ_OK=0
    fi
done
...
if [[ "$MODE" == "self-check" ]]; then
    if [[ "$PREREQ_OK" -eq 1 ]]; then
        echo "[h6.4] self-check PASS — all prerequisites present (run-id $RUN_TS)"
    else
        echo "[h6.4] self-check FAIL — missing prerequisites (run-id $RUN_TS)" >&2
    fi
    # Still emit a minimal summary/fingerprint so the run-id is durable.
    BUILD_STATUS="self-check"; BENCH_STATUS="self-check"; INTEG_STATUS="self-check"
fi

if [[ "$PREREQ_OK" -ne 1 && "$MODE" != "self-check" ]]; then
    echo "[h6.4] aborting: prerequisites missing" >&2
    exit 1
fi
...
# ---------------------------------------------------------------------------
if [[ "$MODE" == "full" ]]; then
    echo "[h6.4] building hot_paths bench (current linker-policy flags)..."
    if env -u CARGO_ENCODED_RUSTFLAGS \
        ...
        echo "[h6.4] BUILD FAILED — see $RUN_DIR/build.log" >&2
    fi
fi

# ---------------------------------------------------------------------------
# 2. BENCH — short-budget run of the two H6 targets (full mode only).
# ---------------------------------------------------------------------------
if [[ "$MODE" == "full" && "$BUILD_STATUS" == "pass" ]]; then
    ...
    if [[ -z "$HOT_BIN" ]]; then
        ...
        echo "[h6.4] bench binary not found after build" >&2
    else
        echo "[h6.4] bench binary: $HOT_BIN"
        # Short Criterion budget: this is a liveness smoke, not a statistical run.
        FILTER="${GROUP}/(${TARGETS[0]}|${TARGETS[1]})"
        if "$HOT_BIN" --bench --sample-size 10 --warm-up-time 0.5 \
            --measurement-time 1 "$FILTER" > "$RUN_DIR/bench_smoke.txt" 2>&1; then
            BENCH_STATUS="pass"
        else
            BENCH_STATUS="fail"; BENCH_RC=1
            echo "[h6.4] bench smoke FAILED — see $RUN_DIR/bench_smoke.txt" >&2
        fi
    fi
fi

# ---------------------------------------------------------------------------
# 3. OUTPUT — H6.5 public-API golden (full + quick modes).
# ---------------------------------------------------------------------------
if [[ "$MODE" == "full" || "$MODE" == "quick" ]]; then
    echo "[h6.4] running public-API golden test ($INTEGRATION_TEST)..."
    if RCH_CARGO_WRAPPER_BYPASS=1 CARGO_INCREMENTAL=0 \
        "$CARGO" test --test "$INTEGRATION_TEST" \
        > "$RUN_DIR/integration.txt" 2>&1; then
        INTEG_STATUS="pass"
    else
        INTEG_STATUS="fail"; INTEG_RC=1
        echo "[h6.4] integration golden FAILED — see $RUN_DIR/integration.txt" >&2
    fi
fi
...
python3 - "$RUN_DIR" "$BEAD" "$SCENARIO" "$MODE" "$VALIDATE_SCRIPT" \
    "$BUILD_STATUS" "$BENCH_STATUS" "$INTEG_STATUS" \
    "${TARGETS[0]},${TARGETS[1]}" <<'PYVERDICT'
import json, os, sys, time, hashlib

(run_dir, bead, scenario, mode, validate_script,
 build_status, bench_status, integ_status, targets_csv) = sys.argv[1:10]
...
checks = [("build", build_status), ("bench", bench_status), ("integration", integ_status)]
...
events.append({
    ...
    "note": ("combined-drop number is produced by the statistical gate "
             f"{validate_script} (PERF-H6.3); this smoke proves liveness + "
             "output-unchanged only"),
})
...
with open(os.path.join(run_dir, "summary.md"), "w") as f:
    f.write(f"# PERF-H6.4 E2E Smoke — {run_id}\n\n")
    ...
    f.write(f"| bench | {', '.join(targets)} execute (short budget) | {bench_status} |\n")
    f.write(f"| integration | `frankenctl run` output == pre-H6 golden | {integ_status} |\n")
    ...
    if fail_reasons:
        ...
        f.write("\n")
    f.write("## Scope\n\n")
    f.write("A smoke proves the H6 capacity-hint sweep **builds, runs, and "
            "preserves observable output**. The binding combined-drop number "
            f"is the statistical gate `{validate_script}` (PERF-H6.3), scored "
            "against the `What counts as a perf win` standard in "
            "`docs/PERFORMANCE_BASELINE.md`.\n")

print(f"[h6.4] smoke overall = {'PASS' if all_pass else 'FAIL'}  (run-id {run_id})")
for r in fail_reasons:
    print(f"[h6.4]   - {r}")
...
echo "[h6.4] artifacts written to $RUN_DIR"
ls -1 "$RUN_DIR"
exit $VERDICT_RC

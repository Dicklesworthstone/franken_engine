...
run_mode() {
  ...
  case "$mode" in
    ...
    test)
      ...
      ;;
    clippy)
      run_step "env -u CARGO_ENCODED_RUSTFLAGS RUSTC_WORKSPACE_WRAPPER=clippy-driver RUSTFLAGS='-Dwarnings -Clinker-features=-lld' cargo test -p frankenengine-engine ${test_targets[*]} --no-run" \

...
validate_input_and_detect_placeholders() {
  ...
  if [[ "$total_decision_count" -eq 0 ]]; then
    echo "Error: replay coverage input has no decisions" >&2
    input_validation_failure_count=$((input_validation_failure_count + 1))
  fi

  duplicate_decision_count="$(jq -r '[.decisions[].decision_id] as $ids | ($ids | length) - ($ids | unique | length)' < "$input_path")"
  if [[ "$duplicate_decision_count" -gt 0 ]]; then
    echo "Error: detected $duplicate_decision_count duplicate replay decision id(s)" >&2
    input_validation_failure_count=$((input_validation_failure_count + duplicate_decision_count))
  fi

  missing_decision_kind_count="$(jq -r '(["allow","deny","escalate"] - ([.decisions[] | select(.security_critical == true) | .decision_kind] | unique)) | length' < "$input_path")"
  if [[ "$missing_decision_kind_count" -gt 0 ]]; then
    echo "Error: replay coverage input does not cover allow, deny, and escalate security decisions" >&2
    input_validation_failure_count=$((input_validation_failure_count + missing_decision_kind_count))
  fi
  ...
  too_fast_verified_count="$(jq -r '[.decisions[] | select(.security_critical == true and .replay_verified == true and (.duration_ms < 10))] | length' < "$input_path")"
  if [[ "$too_fast_verified_count" -gt 0 ]]; then
    echo "Error: $too_fast_verified_count verified replay decision(s) use sub-10ms synthetic timing" >&2
    input_validation_failure_count=$((input_validation_failure_count + too_fast_verified_count))
  fi
  ...
  invalid_hash_count="$(jq -r '[.decisions[] | select((.expected_hash | test("^sha256:[0-9a-f]{64}$") | not) or (.actual_hash | test("^sha256:[0-9a-f]{64}$") | not) or (.replay_report_hash | test("^sha256:[0-9a-f]{64}$") | not))] | length' < "$input_path")"
  ...
  if [[ "$unverified_evidence_count" -gt 0 ]]; then
    echo "Error: $unverified_evidence_count security-critical replay decision(s) lack verified evidence" >&2
    input_validation_failure_count=$((input_validation_failure_count + unverified_evidence_count))
  fi

  if [[ "$uses_placeholder_data" == "true" ]]; then
    echo "⚠ DEFENSIVE: Input contains placeholder/synthetic replay data - gate fails closed"
    input_validation_failure_count=$((input_validation_failure_count + 1))
  fi

  export uses_placeholder_data
  export input_validation_failure_count
}
...
write_bundle() {
  ...
  # Calculate coverage from the decision artifact that will be emitted.
  coverage_denominator="$(jq -s -r '[.[] | select(.security_critical == true)] | length' "$decisions_path")"
  coverage_numerator="$(jq -s -r '[.[] | select(.security_critical == true and .replay_verified == true and .replay_exit_code == 0 and .expected_hash == .actual_hash and ((.verification_status // "") == "verified"))] | length' "$decisions_path")"
  ...
  if [[ "$input_validation_failure_count" -gt 0 ]]; then
    decision="fail"
    reason="replay_evidence_validation_failed"
    failure_count="$input_validation_failure_count"
  ...
  jq -c \
    ...
    '. as $decision
    | ($decision.security_critical == true
      and $decision.expected_hash == $decision.actual_hash
      and $decision.replay_exit_code == 0
      and $decision.replay_verified == true
      and (($decision.verification_status // "") == "verified")) as $covered
    | {
        schema_version: $schema_version,

...
CARGO_TARGET_DIR="${STDLIB_COMPLETENESS_E2E_CARGO_TARGET_DIR:-/tmp/rch_target_franken_engine_stdlib_completeness_$(date +%s)_$$}"
...
run_frankenctl() {
    ...
    local output_file
    output_file="$(dirname "$LOG_FILE")/test_stdlib_${step_name}.rch.log"

    timeout 30 "$RCH_BIN" exec -- env \
...
run_test() {
    ...
    if run_frankenctl "$test_name" run --input "$test_file" --out "$result_file" >/dev/null; then
        local end_time
        end_time=$(date +%s.%3N)
        local duration
